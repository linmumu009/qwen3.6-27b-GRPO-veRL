...
- 修复同题SFT流水线导出阶段未继承训练子进程Megatron-Bridge路径的问题：父进程显式使用固定版本，并在启动时检查实际模块来源，避免误用不支持模型架构的系统安装版本。
- 新增`POSTTRAIN_ONLY=true`恢复入口，跳过制数、八题模型补测和训练，重新校验第418/836步检查点后只做导出与正式评测；保留原失败日志，恢复日志独立保存，禁止覆盖已有恢复记录。
- 5号机恢复已启动，实测确认正确转换库和两个有效检查点，目前进入第418步导出；未重新训练。此次更新不宣称新模型评测已经完成。

## v1.12.73（2026-09-05）

- 按用户最新目标启动SC-bench/LogistikaBench同题得分专项，不再增加泛化测试。复用正式选择题提示、全部选项、关闭thinking的chat模板，直接监督零基答案JSON及EOS；成绩明确标注为训练过评测题后的同题成绩。
- 完成全量1,672题制数与实机门禁：LogistikaBench 1,446题、SC-bench知识部分226题；每遍1,126,158个输入token、11,516个受监督token，最大序列3,034 token，逐条核验正式prompt token一致与金答案解析回环。
- 已在5号机原容器启动顺序流水线：先补测两个八题模型的三轮正式选择题成绩，再从Step120连续训练1遍/2遍（第418/836步检查点），严格导出后按同一协议评测；仅保存model+extra，设置260GB训练前及95GB导出前磁盘门禁，不删除历史产物。此版本交付时尚无全量SFT得分。
- 新增无正文配对汇总与分数据集不退步的候选选择规则，40项相关测试通过。执行细节及状态边界见[同题优化协议](docs/logistics_mcq_same_item_fit_20260905.md)。

## v1.12.72（2026-09-05）

- 获得当前任务最多8条私有样本的明确查看授权，完成8条知识陈述忠实度复核：7条保留、1条删除章节引用；没有覆盖false判断题，也不构成独立事实核验。正文继续不进入Git。
- 完成同文本监督范围对照：两组分别从Step120模型态初始化fresh Adam，8题、32遍、64步，每组15,712个累计输入token；全文组与答案组分别监督每遍483/47个目标token，两组EOS均受监督。
- 统一39个答案token的NLL由基线`1.8344`降至全文组`9.95e-6`、答案组`1.22e-6`，完整金答案前缀均由`0/8`变为`8/8`。这证明小样本可拟合，不证明泛化或答案专属训练整体更优；答案组题干前缀NLL由`3.3729`升至`4.3157`，不能直接外推为通用能力退化。
- 两组最终模型、优化器及extra检查点和各15分片/1,199张量HF导出均通过校验；修复同一步训练/验证日志合并，保留冲突检测，新增最终汇总范围校验。31项相关测试通过，结果与限制见[八题实验报告](docs/logistics_reviewed8_mask_results_20260905.md)。

## v1.12.71（2026-09-05）

- 更正上一轮判断题制数与结论：历史实现是原命题后追加 true/false，未完成错误命题知识化；暂缓仅据全文 loss 和不显著评测就停止追加曝光的建议，改为检查答案写入、上下文依赖与监督位置。
- 新增同题双文本的答案专属 NLL、逐 token top-1、原样答案前缀生成，以及原 Parquet 训练块的精确答案范围计分；直接 CPT 在原训练块中的答案 NLL 从 `4.7873` 降至 `2.4873`，但独立题面仅从 `2.8990` 降至 `2.6632`，表明收益对上下文高度敏感，不能据全文 loss 认定答案已充分拟合。
- 完成 1,672 道题的新知识陈述候选生成与本地自动蕴涵检查：1,582 道自动通过、90 道留出；81 道判断题中 79 道自动通过。候选明确标记尚未实质抽查或独立事实核验；正文抽查因当前审阅目的地的传输授权未明确而被自动审批阻止，未启动新训练。
- 定位当前 veRL/Megatron 转换路径忽略通用 `optim.betas` 的原因，新增 `adam_beta1/adam_beta2` 显式覆盖并默认保持历史实际 `0.9/0.999`；新增答案边界、打包范围与配对一致性检查，25 项相关测试通过。

## v1.12.70（2026-09-04）

- 完成 SC-bench/LogistikaBench 金答案文本的两组受控污染诊断：直接题面组把题干与正确选项正文转换为无选项字母、无干扰项的 causal-LM 文本，改写组先对题干做严格语义保真改写；两组均从同一 Step120 模型态独立初始化 fresh optimizer，并各训练 4 遍、64 step。
- 两组 16-NPU 训练与 HF 导出均通过完整性校验；直接组/改写组每遍平均 loss 分别为 `3.8054→2.1378` / `3.4648→2.1423`，但三轮多数票仅由 `81.878%` 提升到 `82.596%` / `82.117%`。直接组净增 12/1,672（`p=0.0428`），改写组净增 4/1,672（`p=0.4545`）。
- 1,625 道严格通过的真实改写子集上，改写 CPT 仅 `+0.2462` 点且 `p=0.4545`；47 道恒等回退题贡献为零。结论是当前方案主要产生弱同题关联，未形成可靠语义迁移；结果只可用于污染机制诊断，不可作为交付分数或晋级依据，停止追加原题曝光并转向来源/概念隔离的物流训练集与盲测集。

## v1.12.69（2026-09-04）

- 完成 Step120 单书 CPT 的 2遍/4遍连续曝光实机复核：116 个 optimizer step 共处理 1,346,808 个含 EOS 的 sequence token，第58/116步检查点及两个 15 分片 HF 导出均通过严格完整性校验；逐遍平均 loss 为 `2.0447 → 1.9702 → 1.9233 → 1.9165`。
- 固定书内 NLL 相对 Step120 分别改善 `2.445% / 4.126%`，全权重相对 L2 达 `2.13e-4 / 3.08e-4`，证明更多曝光确实加强了书籍文本吸收；视觉编码器与 MTP 仍完全冻结，600 条固定续写未发现新增 10/20 词逐字复现。
- 独立书内知识题在 Step120、2遍、4遍上均为 `189/200` 且逐题完全相同；SC-bench 也均为 `189/226`。三轮多数票全量公开评测为 `1,369 → 1,366 / 1,371`（2遍/4遍），4遍仅净增 2/1,672、`+0.1196` 点且 McNemar `p=0.754`，远低于 `+3` 点门槛；两个候选均不晋级，停止继续重复单书原样 CPT。
- 新增 5 号机通用离线物流 MCQ 启动包装，统一加载昇腾环境、`spawn` 多进程和线程约束，并显式支持知识探针的 2,048 上下文与公开长题的 4,096 上下文，避免子进程初始化和长题预检差异。
...
- 新增严格运行合同、独占锁、禁止覆盖、每遍336,702 sequence token核验、逐遍 loss/LR/梯度汇总、两个 Megatron checkpoint 完整性门禁及顺序 HF 严格导出流水线；候选默认禁止晋级，后续必须依次通过固定NLL、逐字记忆、独立改写知识题、SC-bench/LogistikaBench与原Agent/通用能力回归。

## v1.12.66（2026-09-04）

- 新增单书 CPT 五项可复现诊断：固定 200 段/102,200 token 的 teacher-forced NLL、原 600 条逐字续写配对、200 道无连续 4 词原文重合的改写知识题、1,199 张量逐层权重差分，以及 1,672 道公开题的单副本三轮多数票评测；正文、题面和原始输出继续只保存在 5 号机私有目录。
- 实测确认 CPT 训练发生但吸收过弱：书内 NLL 仅相对改善 `0.398%`，低于 `2%` 门槛；改写知识题 `94.5% → 94.5%` 且无任何对错翻转；权重整体相对 L2 仅 `7.19e-5`，视觉编码器和 MTP 完全未变，未发现新增 10/20 词逐字续写。
- 用 TP8×DP1、温度 0、同 seed 三轮复测 SC-bench/LogistikaBench，并以逐题多数票消除异步调度波动：`81.878% → 81.938%`，4 道改对、3 道改错，净增 1/1,672（`+0.0598` 点，McNemar `p=1.0`）；候选继续不晋级，交付优先 RAG，参数训练转向 5M-token 多来源混合 CPT 金丝雀。
...
- 在同机同 vLLM-Ascend 配置下，对本地原生模型与 Step120 分别完成 32/64/128-token 前缀各 200 条全书续写筛查，共 1,200 次正式请求全部成功且无空回答；两模型均只有 3/600 条达到 10 token、0/600 达到 20 token，全部高命中长度一致，未发现 Step120 新增或书籍特异性的强逐字记忆证据。
- 修复本地 Qwen 评测必须同时关闭 chat-template thinking 的协议缺口，新增不含正文的逐样本配对与高命中重复性汇总；核验后决定继续把本书作为混合语料骨架，但不启动单书 CPT：正文约 37 万 Qwen token，且现有 Qwen3.5 Megatron-Bridge 明确标注 pretraining 未验证，需先补 5M-token 多来源语料与一步 causal-LM 门禁。

## v1.12.63（2026-09-03）

- 新增可断点续跑的 OpenAI 兼容物流知识评测器：统一零基选项 JSON、集合完全匹配、并发 64、私有原始结果与可提交安全摘要分离，并支持逐题 0→1/1→0、类别/题型/选项规模分层和 McNemar 精确检验。
- 在 5 号机完成百炼原生、本地原生和 Step120 共三次 1,672 题全量评测；同机同 vLLM 主比较为 `81.52% → 81.88%`（`+0.36` 点，6 个 0→1、0 个 1→0），LogistikaBench/SC-bench 知识题分别 `+0.35/+0.44` 点，低于预登记的 `+3` 点实用门槛。
- 定位并排除 vLLM thinking 模板导致的空 content 伪失败，记录 269 选项长匹配池、单选项题和 SC 判断题缺失选项等数据质量边界；结论是不做单书 CPT 或格式 SFT，优先补隐藏评测与 RAG，再以多来源授权语料做 5M-token 混合 CPT 金丝雀。两次临时推理容器均已清理，书籍正文未用于本轮评测或训练。

## v1.12.62（2026-09-03）

- 完成《The Handbook of Logistics and Distribution Management》第八版私有 TXT 的 CPT 前置审计：核验 44 章、430,858 个 Qwen token、正文 370,380 token，并量化 PDF 转文本占位符、重复行、参考文献/索引边界和训练权限制；安全汇总不包含书籍正文或评测题面。
- 对本地 LogistikaBench 1,446 题和 SC-bench 226 道知识题执行完整题干及 8/12/16/24 连续词重合扫描，均未发现逐字重合，LogistikaBench 列出的五个原始来源标题也未在书中出现；新增可复现审计脚本与回归测试。
- 只读核验 5 号机 Step120 开源续训权重、HF 导出、NPU、磁盘和训练容器：模型权重完整但 optimizer/RNG 缺失，后续只能从模型态初始化新优化器；当前决定不做单书纯 CPT，待书籍训练权确认后采用多来源物流语料、阅读理解式知识利用、一般能力回放与短 SFT 的分阶段门禁方案。

## v1.12.61（2026-09-02）

- 按汇报证据的领域相关性要求，删除物流语言模型首页中的医疗文本标注案例，替换为 Google Research 在发票、账单、税表、工资单等通用业务文档上的研究数据：含 25 个目标字段的文档，传统人工标注平均约 `6 分钟/份`。
- 保留 Google DeepMind / Google 对约 304 词文本偏好任务的人工标注成本估算 `$0.67/条`；页面短引用更新为 `Google Research · EMNLP 2023｜Google DeepMind · ICML 2024`，演讲者备注同步补充两篇论文的完整来源及“外部通用任务不等同于物流企业实测成本”的适用边界。
- 因上一版带证据文件被 PowerPoint 占用，保留原文件并新增 `物流语言模型_全链路业务架构可编辑版_通用领域数据证据.pptx`；已完成六页逐页渲染、模板保真、空占位符、医疗词残留和越界基线检查，新首页无新增越界，原第六页既有底部装饰条越界保持不变。

## v1.12.60（2026-09-02）

- 在物流语言模型汇报首页的传统人工建设区域新增两项论文证据：Google Research 医疗文本实验的人工初标加专家复核平均耗时 `26.67 分钟/份`，以及 Google DeepMind / Google 在约 304 词文本偏好任务中的人工标注成本估算 `$0.67/条`。
- 页面以可编辑 PowerPoint 原生文本呈现两项数字、任务口径和短引用，并在演讲者备注中补充完整论文信息、原始链接与适用范围，明确外部医疗/摘要任务证据不等同于物流行业实测成本。
- 因原可编辑版在本轮修改时被 PowerPoint 占用，保留原文件并新增 `物流语言模型_全链路业务架构可编辑版_含数据证据.pptx`；其余五页保持不变，已完成六页逐页渲染、模板保真、占位文字和越界基线检查，新首页无越界，原第六页既有底部装饰条越界保持不变。

## v1.12.59（2026-09-02）

- 参考《物流预测模型》第一页的信息层级重做物流语言模型汇报首页下半区，保留“模型需要什么、过去怎么建设、效果怎么量化、我们形成闭环”四个既有问题，将原先占据主体的大图缩小为辅助视觉。
- 左侧新增训练持续数据、独立验证标准和传统人工建设难持续三条编号说明；右侧新增持续生产训练数据、独立生成隐藏评测和支持模型持续迭代三张说明卡，并以底部结论明确“人工找数、人工造题”向“持续供数、独立评测”的升级价值。
- 新增信息均使用可编辑 PowerPoint 原生对象，原有其余五页保持不变；已完成六页逐页渲染、模板保真、占位文字和越界基线检查，新首页无越界，原第六页既有底部装饰条越界保持不变。

## v1.12.58（2026-09-02）

- 校正物流语言模型汇报第四页的产品边界：标题改为“一个物流模型，理解全链路业务”，明确物流垂域大模型才是本项目核心产品，避免把下游业务应用误解为本项目交付物。
- 将上层改为灰色弱化的“下游可支撑场景”，以客户服务、一线培训、知识问答和运营分析说明模型能力的潜在使用方向；下层改为“模型覆盖的物流业务范围”，强调六类领域是模型理解范围而非项目建设的业务系统。
- 模型能力同步调整为物流知识理解、业务语义推理、任务规划执行和工具调用；全部内容继续采用可编辑 PowerPoint 原生对象，已完成六页逐页渲染、模板保真和越界基线检查，原第六页既有底部装饰条越界保持不变。

## v1.12.57（2026-09-02）

- 将物流语言模型汇报第四页由整张架构图改为 PowerPoint 原生对象：应用卡片、模型能力、业务领域、文字和箭头均可分别选中编辑，页面中仅品牌 Logo 保留为图片。
- 将“客服助手 / 服务客户”等四组应用名称与说明拆为独立文本框，并统一增加垂直间距，解决上下两行过近的问题；同时保持三层架构关系和其余五页内容不变。
- 已完成六页逐页渲染、模板保真、占位文字和越界基线检查；新第四页无越界，原第六页既有底部装饰条越界保持不变。

## v1.12.56（2026-09-01）

- 重做物流语言模型汇报第四页，将原先左右挤压的“业务域 → 模型 → Agent”横向草图改为三层业务架构：底层六类物流业务领域沉淀数据与知识，中层由一个物流垂域大模型统一承接，上层复用为客服、培训、知识和运营四类 Agent 应用。
- 中间模型层明确展示领域知识、业务推理、流程编排和工具调用四项共享能力；统一各层宽度、间距和箭头关系，修复业务域与 Agent 卡片文字裁切、版面失衡和箭头说明压线问题。
- 原有其余五页保持不变，第四页权威参考继续保留在演讲者备注中；已完成六页逐页渲染、模板保真、占位文字和越界基线检查，新增页无越界，原末页既有底部装饰条越界保持不变。

## v1.12.55（2026-09-01）

- 在物流语言模型汇报材料中新增第四页“一个物流模型，服务全链路业务”，以单一横向链路展示六类物流核心业务域如何统一进入物流垂域大模型，并由模型支撑客服、培训、知识和运营四类 Agent 应用。
- 业务域划分综合参考 GB/T 18354-2021、ASCM SCOR DS、CSCMP 物流定义及本地数据沙箱对象，权威来源收录在该页演讲者备注中，主画面保持简洁，不堆砌框架名称。
- 原有五页内容和顺序均保留，整套扩展为六页；已完成逐页渲染、模板保真、占位文字和越界基线检查，新增第四页无越界，原第六页既有底部装饰条越界保持不变。

## v1.12.54（2026-09-01）

- 新增书籍逐字记忆审计脚本：支持从私有 UTF-8 正文或预制 JSONL 抽取续写片段，通过百炼 OpenAI 兼容接口并发调用模型，统计连续精确命中与 token F1；原始正文和模型输出仅写入私有产物，仓库汇总只保留哈希与聚合指标。
- 在 5 号机以并发 64 调用 `qwen3.6-27b`，完成《The Handbook of Logistics and Distribution Management》第八版官方公开预览 64 条续写预检，并以 Project Gutenberg 公版《双城记》著名开篇和全书随机片段各 64 条校准；目标预览连续 3/5/10/20 token 命中均为 0，著名开篇对照的 5/10 token 命中分别为 7.81%/6.25%。
- 明确证据边界：本轮只支持“未发现公开预览的逐字记忆正证据”，不证明完整正文未进入预训练；正式制数前仍须取得合规正文，完成跨章节非重叠续写、最小扰动原文识别和独立知识利用测试。

## v1.12.53（2026-09-01）

- 重做物流语言模型汇报首页，按“模型需要什么数据 → 过去怎么建设 → 效果怎么量化 → 数据沙箱如何形成闭环”组织单页叙事，明确训练侧需要事实、知识、任务与工具轨迹，评测侧需要独立任务、标准结果与过程规则。
- 将传统取数、脱敏、标注、出题和专家验收的慢、贵、长尾不足与标准不一，与数据沙箱“持续生产训练数据和隐藏评测、两者相互隔离、可回放、可追溯”的核心价值直接对照；首页移除重复的三组效果提升百分比，相关数字仅在第三页展开，后四页保持不变。
- 新增四阶段横向主视觉并保留全部中文标题为可编辑文本；完成五页逐页渲染、模板保真、占位文字和越界基线检查，新首页无新增越界，原第五页既有底部装饰条越界保持不变。

## v1.12.52（2026-09-01）

- 将物流语言模型汇报首页重构为一页式垂域数据闭环：左侧交代生产数仓、业务文档和专家经验如何经取数、脱敏、标注形成传统训练数据，中部展示数据沙箱如何以实体、状态、动作和规则重构可运行物流世界，右侧明确持续产出事实、知识、任务与工具轨迹并支撑 CPT、SFT、RL 和统一评测。
- 首页底部新增数据与模型双重效果量化，集中展示 36,101 条事实、1,587 个证据块、1,520 个任务，以及相对 Qwen3.6-27B 的运营分析、知识问答和客服培训任务提升；后四页保持不变，并完成逐页渲染、模板保真和原稿越界基线核验。

## v1.12.51（2026-09-01）

- 优化物流语言模型汇报材料首页：参考《物流预测模型》《物流时空模型》的问题导向表达，将背景收敛为数据、场景、验证三类边界，明确真实生产数据难以直接进入训练以及物流智能体缺少可复现业务环境的核心矛盾。
- 首页改用顺丰红灰扁平化链路视觉，并以“不复制生产明细，而是重构可训练、可验证、可复现的业务世界”承接后续数据沙箱方案；后四页内容和顺序保持不变，完成全稿逐页渲染、模板保真及版式基线核验。

## v1.12.50（2026-09-01）

- 新增物流语言模型汇报版 PPT：在原四页材料前补充“生产数据受限 → 物流数据沙箱 → 模型训练价值”的背景页，明确真实生产明细不可直接进入训练、沙箱以仿真数仓/业务文档/可验证任务重构业务环境，并落到 SFT/RL、统一评测与多模型轨迹采集。
- 新页复用原有顺丰科技红白母版与品牌元素，以单幅横向链路主视觉和一句结论替代密集文字；原四页内容、版式和顺序保持不变，并完成 5 页渲染、模板保真与越界基线核验。

## v1.12.49（2026-08-31）

- 将物流运营沙箱讲解收敛为固定 `4:3` 的 PPT 式分页画面，专门适配现有汇报页右下区域；页面无滚动、每次只讲一个真实阶段，阶段内部沿“上游内容 → 本步处理 → 下游产物”逐项显现，12 页总览始终可直接跳转。
- 全部示例改为中文业务概念优先：用“运单”依次讲清 23 个属性、20 个状态、状态更新动作、情境约束、`fact_waybill` 字段映射以及 `WAY000067 → ROU000651 华东干线 / CUS000669 广星德` 的真实连接，再延伸到 `task_000285` 数仓题、`doc_002` SLA、`KT-LOG-0080` 知识题和 `HT-0051` 混合合规题。
- 最后一页明确 Runner / 评测者边界并停在“沙箱生成完成”；`960×720` 与 `800×600` 实测页面、四节点详情、三类任务和冻结终态均无滚动、节点裁切或浏览器告警，重新播放可回到第一页，相关通用/物流动画回归共 `7 passed`。

## v1.12.48（2026-08-31）

- 按物流运营沙箱的实际形成过程重做示例动画，完全移除粒子球、数字增量栏和“文件生成文件”的主叙事；12 个阶段统一改成“一个真实上游样本 → 实际加工规则 → 真实下游内容 → 同样规则批量展开”的朴实讲解，并持续说明为什么能够进入下一步。
- 以运单 `waybill` 为主样本，展示 23 个业务属性、20 态生命周期和 `update_waybill_status` 动作如何映射为 25 字段 `fact_waybill`，再用真实记录 `WAY000067 → ROU000651 华东干线 / CUS000669 广星德` 说明事实世界如何连接；数仓任务、SLA 文档、知识任务和混合任务分别采用 `task_000257`、`doc_002`、`KT-LOG-0080`、`HT-0007` 的实际例子。
- 最终明确 Runner 的 DB / Docs / Schema 与评测者侧 Tasks / Hidden Gold 边界，停在“物流运营沙箱生成完成”状态；1280px 桌面逐步核验实体映射、真实记录、三类任务、冻结边界、重新播放与停止状态均正常，无横向溢出或浏览器告警，相关通用/物流动画回归共 `7 passed`。

## v1.12.47（2026-08-31）

- Prefix curriculum启动器在隔离Ray启动前、actor模型真正加载前分别重新核验5/6号机NPU进程表；任一主机探测异常或出现占卡进程即fail closed并保留只含计数的安全审计，不会抢占或停止外部任务。
- 资源阻塞状态由退出清理逻辑原样保留，便于区分资源竞争与训练故障；新增真实`npu-smi`设备行/进程行解析和双门调用顺序回归。数据包、tiered-v1、训练超参及全量训练锁均未改变。
...
- 粒子球按统一空间语法展开真实规模：蓝色事实半球由 64 个表锚点释放 36,101 个实例粒子和 300 条显式关系，紫色规则半球由 65 个文档锚点释放 1,587 个证据粒子；555 个数仓任务、465 个知识任务及 500 个混合任务作为世界外部的金色观察探针，明确任务不是世界内容。
- 最终绿色冻结壳只封装 Runner 可见的 DB、Docs 与 Schema，1,520 个任务和 Hidden Gold 保持在壳外；结束后粒子旋转和播放循环真正停止，画面保持成熟世界终态。相关通用/物流动画合计 `7 passed`，1280px 桌面逐阶段实测结构、分裂生长、双半球、外部任务轨道、冻结隔离、停止状态和重新播放均正常，浏览器无警告或错误。

## v1.12.42（2026-08-28）

- 按“结构逻辑清楚优先、详细过程其次、规模感再次、动效最后”的顺序重做物流运营沙箱动画：顶部固定展示 `业务理解 → 世界建模 → 事实/规则同源双分支 → 双源任务汇合 → 冻结交付`，中部保留 12 个真实执行阶段，当前阶段始终按“接住上一步 → 依次完成动作 → 世界新增内容 → 交给下一步”展开。
- 增加随实际动作增长的“世界累计规模”，明确区分 57 类实体与 36,101 条实例化记录，并持续展示 76 个状态、80 个动作、64 张表、65 份制度文档、1,587 个证据块和 1,520 个三类任务；详细阶段同时展示 15 个变化维度、115 个采样单元、27 条组合约束、300 条显式关系桥记录及 374 对文档引用关系等真实数量。
- 动效收敛为当前动作高亮、完成勾选和数字增长，不再使用与逻辑无关的粒子或场景装饰；最终保持静止的成熟物流世界规模总览，并明确 Runner/Raw 可见性隔离和 80分31秒真实生成耗时。
- 相关通用/物流动画合计 `7 passed`；1280px 桌面实测整体结构、12 阶段导航、Factor/Data/Hybrid/Freeze 关键步骤、累计规模增长、阶段交付、静止终态与重新播放均正常，无横向溢出，浏览器无警告或错误。

## v1.12.41（2026-08-28）

- 将物流沙箱动态图收敛为始终可见的一条因果主线：`业务描述 → 世界结构 → 现实实例 → 世界规则 → 观察任务 → 冻结沙箱`，用明确标注为非真实数据库行的示意运单 `W-DEMO-001` 贯穿实体、状态、动作、事实、规则和最终隔离，不再让 13 个工程阶段承担主叙事。
- 六幕画面分别回答“已经有什么、这一步做什么、世界因此获得什么”；重点重画任务产生逻辑：Step 5.1 数仓任务只观察“实际发生了什么”，Step 5.2 知识任务只观察“规则要求什么”，Step 5.3 混合任务将实际时效与 SLA 阈值放在同一判断中回答“现实是否符合规则”。
- 13 个真实阶段、耗时与落盘产物移入默认折叠的可审计证据区；最终明确分隔评测者保险库与模型可见 Runner，进入静止的“沙箱生成完成”状态，播放与下一步停止，仅保留重新观看入口。
- 相关通用/物流动画合计 `7 passed`；1280px 桌面逐幕实测无横向溢出，第二幕运单状态、第五幕三类任务分叉、冻结可见性边界、静止终态与重新播放均正常，浏览器无警告或错误。

## v1.12.40（2026-08-27）

- 针对上一版信息过密、世界层同时叠加导致难懂的问题，重做为 6 个连续故事章节：定义世界、建立运行机制、实例化事实世界、建立规则世界、生成混合判断、冻结并交付；13 个真实执行阶段仍完整保留，但不再作为一排细碎导航抢占注意力。
- 每个镜头固定回答三个问题：“上一刻的世界”“这一刻发生什么”“世界因此获得什么能力”；当前变化以大字号结论和真实结果居中显示，已有世界退为暗色上下文，只有当前相关的实体、状态环、动作束、情境轨道、事实流、规则层或观察任务被镜头聚焦。Step 5.1 数仓任务和 Step 5.3 混合任务的语义与编号继续明确。
- 文件清单默认折叠为“技术落盘凭证”，主画面只讲世界能力；新增六章节转场、镜头缩放、语义层生长、粒子爆发、可点击建模动作和颜色图例，最终保持静止的 `WORLD MODEL SEALED / 物流沙箱生成完成` 终态。
- 相关通用/物流动画合计 `7 passed`；1280px 桌面实测 6 个章节、三段式解释、聚焦层、操作跳转、终态停止与重播均正常，无横向溢出，控制台无警告或错误。
...
- 回归测试现在同时覆盖 32 条验证样本的列长不冲突，以及所有非字符串附加字段均可被 veRL 数值指标归约。
...
- [`docs/logistics_sandbox_generation_example_animation_20260827.html`](docs/logistics_sandbox_generation_example_animation_20260827.html)：桌面端电影化物流世界建模实录；以 6 个故事章节和“上一刻—这一刻—获得能力”三段式解释，聚焦展示世界边界、角色、实体、状态、动作、约束、事实、制度规则与三类观察任务怎样进入同一个可执行世界；文件默认折叠为技术落盘凭证。由 [`scripts/render_logistics_sandbox_example_animation.py`](scripts/render_logistics_sandbox_example_animation.py) 确定性生成。
...
- 修复双机非共享本地盘下rollout节点找不到正式私有调度文件的问题：监督器在启动Ray前确定性生成`approved43x4`与`sealed8`，以`0600`同步到6号机同路径，并以双机SHA256一致作为硬门。
- 首次正式启动在rollout actor读取数据阶段fail-closed退出，发生于任何轨迹、optimizer或checkpoint之前；失败run保留完整证据，修复后必须以新run从原始Qwen3.8基座重新启动，不复用已加载状态。
...
- 对800条历史轨迹重做CPU shadow：严格唯一final解析将正确标签从483降至289，识别118条歧义；43个批准组中36组为mixed、7组全错，所有mixed组错误轨迹正/非负advantage计数均为0，22/22表格修复题未复发。结构化硬门历史通过774/800，按每题最多16次补齐8条的模拟预计42.89/43组可用，超限组必须跳过。
- 新增仅从原始Qwen3.8-27B与冻结同源参考模型启动的43题×4曝光入口、批准包/模型哈希硬门、结构化`pi_tool_events`与EvidencePlan绑定、硬门重采、零陈旧度、actor侧KL loss和只保存最终模型合同；已在实际veRL容器副本通过静态合同及CPU门禁，正式训练、模型加载、rollout和optimizer仍保持关闭。
...
- 私有manifest逐题冻结奖励路由：21道numeric使用已验证最终数值口径，22道table必须使用`corrected_full_table_ordered_v1`并完成22/22重放；修复后全对5道、真实高难15道、模型工具/SQL错误5道和超时桶2道均明确排除。
- Git仅更新可复现构建/核验代码、自动测试与安全摘要；43行Parquet及其43行manifest保持0600权限并只留5号机，训练任务通过独立批准包身份读取，不修改原始数据门禁。

### v1.12.8 — 2026-08-22

- 冻结沙箱修复后训练为独立批准清单中的21道原始numeric mixed题；启动前硬核对21行/21唯一题、原始禁训标志、独立授权和sealed零重叠，只生成每题4次的84组/672条在线轨迹，明确拒绝22道旧表格奖励修复后的条件候选及原100题目录扩题。
- 新增严格最终正确性奖励：完整verifier继续记录数值容差和表格全行全列等指标，但标量奖励只取二值`acc`，过程奖励应用为0；固定Step120参考KL、陈旧度0，并在KL与优势计算后清零全错/全对组，整批无有效mixed时直接跳过actor优化器以阻断Adam动量更新。
- 新增5步逐步checkpoint/sealed门禁和双机监督器：5号机TP4×PP2×CP2训练、6号机TP4×DP4 rollout；任一步验证退化、KL/梯度/奖励异常或组记账失败即自动停机保留证据，全部通过才从Step5完整状态续到42步。相关奖励、数据、补丁和门禁回归测试38项通过。

### v1.12.7 — 2026-08-22

- 对Step120新100题运行中的21道mixed与47道全错完成逐题GRPO适用性审计：68/68 verification SQL与完整gold重放通过，题面/EvidencePlan/SQL、数据绑定、日期/分组/聚合/排序/TopN/占比、数值容差及544条范围内轨迹标签均已核验；原数据`training_allowed`保持不变。
- 21/21原mixed进入已审计候选；47道原全错经全行全列表格比较复核后，27道确认为旧奖励器假阴性，其中22道恢复为mixed条件候选、5道变为全对，剩余20道归因为15道真实高难与5道模型工具/SQL错误。可用非零方差题面因此为21道直接候选，加22道修复表格奖励路由后候选。
- 新增可复现的私有审计器与测试；逐题题面、gold、SQL和8条最终答案只保留5号机私有目录，Git仅保存不含敏感内容的安全聚合与输入哈希，所有候选继续`promotion_allowed=false`。

### v1.12.6 — 2026-08-21

- 完成Qwen3.8 Step70 mixed27四次曝光的全量退化诊断：864条轨迹中仅42条严格正确，108个GRPO组有78个八条全错，且78/78仍有奖励方差；54个优化步中26步完全没有正确轨迹，已记录的25个此类步骤全部产生非零梯度。
- 确认`banded-v2`能把正确轨迹稳定排在错误轨迹上方，但SQL证据通道为`0/864`，全错组实际按流程代理分持续更新；固定参考KL、KL loss和entropy均关闭，27题四次全参曝光与fully-async陈旧轨迹构成累计漂移的主要风险链。
- 新增隐私安全诊断脚本、自动测试、机器可读聚合和完整报告；维持Step54禁晋级，下一实验限定为回到Step70、mixed-correct-only、固定参考KL、零陈旧度、每步checkpoint与sealed止损的1–5步金丝雀。

### v1.12.5 — 2026-08-21

- `global_step_54` 的sealed6严格`2+2+2`评测完整结束：32条实际轨迹中31条完整、1条完整轨迹超时、0个runtime error，监督器与队列退出码均为0，6号机NPU已释放。
- 同一道sealed6在训练前为`6/6`题至少一次正确、6个mixed、`6/31`条完整轨迹正确；训练后降为`1/6`题至少一次正确、1个mixed、`1/31`条完整轨迹正确，其余5题最多6次均无正确答案。v15保留1题，v20三题与v21一题均未保留正确轨迹。
- 该小而同题的密封门给出强负向信号，继续保持`promotion_allowed=false`，不据此追加训练或把sealed题回流训练；新增安全报告与机器可读聚合，不含题面、gold、身份、工具输出或服务器路径。

### v1.12.4 — 2026-08-21

- 新增 mixed27 四次曝光最终 `global_step_54` 的 sealed6 独立评测监督器：先验证唯一 checkpoint 与冻结数据哈希，再导出并校验 HF 模型，只复制到6号机并以16卡 TP4×DP4运行严格 `2+2+2`。
- 评测保持 `reasoning_effort=medium`、94,208上下文、30分钟完整轨迹超时及原采样参数；安全结果只聚合训练前/后正确任务、mixed任务、正确/完整/超时轨迹和分版本/难度计数，题面、gold、任务身份与工具输出不进入Git。
- sealed6始终保持禁训、禁自动晋级；0号机不参与这6道小评测，避免为很小负载支付额外模型复制与集群启动时间。
