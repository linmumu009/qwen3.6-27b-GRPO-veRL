...
## v1.13.173（2026-09-23）

- 完成[单书CPT“Logistika提升9点”调查](docs/book_cpt_eval_investigation_20260923.md)：找到9月20日评测与8月31日基线，混用新旧评分可重现+9.75点；统一评分实际1088→1087，净少1题。
- 旧A–J解析器低估基线，原回答重评分净增142题（+9.82点）；30K派生目录预测与8K相同但基线汇总未同步，仍显示旧分数。
- 下载36份必要证据并核验指纹，主比较2,892条回答独立复算、全1,446题与本项目输入对齐；9月3日1179→1177结果复核一致。只读远端，未启动推理或改动对方评分。

## v1.13.172（2026-09-23）

- 完成[去前缀31道变化题来源核验](docs/cpt_neutral_change_audit_20260923.md)：隔离审阅18修复与13损失，32处归档引用定位和指纹复验通过；新增调用、训练均0。
- 5道损失的原前缀确实提供优缺点分类框架，2道涉及同义候选；明确区分索引得失与知识变化，保留原正式评分及全部915次结果。
- 追加完整私有台账、安全统计与3项规范化测试；下一步优先全305题任务框架/候选关系清点，尚不采用处理或训练。

## v1.13.171（2026-09-23）

- 完成[全部305题统一去主题前缀验证](docs/cpt_neutral_prefix_results_20260923.md)：915次新调用＋915次历史复用，原224题正确变229，修复18、损失13、净增5；不自动采用，不训练。
- 提示构建不读取标签/预测，不借领域配对或缩候选；定义与269项顺序保持，全部305边界及协议经隔离审阅。旧25目标仅修复8，未复现借领域提示时19题收益。
- 58批及独立复算一致，7项测试通过；明确来源A只有原错子集、保持标签未全部复核及历史基线限制，原正式成绩不变。

## v1.13.170（2026-09-23）

- 完成[144次多选完整性对照](docs/cpt_selection_diagnostic_results_20260923.md)：同场索引集合14/16、真假向量12/16、单项重建10/16；两种新形式各损失一个保持组合，未采用。
- 独立登记与协议审阅通过；单项模式保留完整上下文、明确披露4倍调用成本。错误仍为漏判，完整布尔输出也会判假，削弱“只漏写答案”的解释。
- 新增严格布尔评分、完整集合重建及逐项稳定性台账，4项针对性测试通过；原始内容私有，训练/正式重评0，保留P1。

## v1.13.169（2026-09-23）

- 完成[操作矩阵批次](docs/cpt_matrix_trial_results_20260923.md)：先审规格，再隔离编写与审阅20教学＋8开发题，质量全部通过；共同事实与完整跨分区操作核验通过。
- 固定80次P1准入完成：UNCTAD两条件均4/4稳定正确，运输均2/4且有重复波动；完整来源未改善。按登记关闭本批，训练/正式重评0，保留P1。
- 新增审阅链绑定、隐藏答案导出、固定调用与复算工具，12项检查通过；原始内容私有，报告明确有规则提示、状态曝光不等及开发测量限制。

## v1.13.168（2026-09-23）

- 接受仅Logistika两家族小训练范围并完成[隔离制数与审核](docs/cpt_two_family_results_20260923.md)：24教学＋8迁移题，首稿4组跨集复用，经一次生成前修订后复用清零。
- 第二次独立盲审30/32通过；两道T题未体现条件辨析，教学对照未成立，本批关闭。未释放训练包、学生调用0、训练0；停止原因是数据设计，不是SC来源或运行权限。
- 新增准入、轮换、审核链绑定与训练预算工具，修复旧验证指纹/审核后篡改风险，4项测试通过；实际token与远端训练尚未运行，原始材料私有。

## v1.13.167（2026-09-23）

- 完成[呈现交叉诊断](docs/cpt_presentation_results_20260923.md)：25目标保留269项换领域前缀后19题多数正确，换原小候选集后22题正确；25保持题分别损失0/1，不能作为正式分数提升。
- 完成[来源/等义规则对照](docs/cpt_small_choice_results_20260923.md)：隔离审阅4题放行、1题待裁定；闭卷0/4，完整原文及结构化规则均4/4三次全对，未显示结构化额外收益。
- 两项合计新增336次、复用312次，批次预算/恢复与提示指纹核验通过；补充[训练准备缺口](docs/cpt_training_readiness_20260923.md)，原阶段C未达准入、无新增训练。代码及安全结果提交，原始数据私有。

## v1.13.166（2026-09-23）

- 完成[P1全量错误决策台账](docs/cpt_p1_error_map_20260923.md)：287题全部关联，补核验18题，依据充分52、部分100、缺背景72、待裁定63。
- 重建303对同定义/同答案的历史对照：71道大候选集错、小候选集对，其中25道来源充分；优先核查呈现与术语选择，不将结果当选项数量的因果效应。
- 独立复算预测、原始CSV及配对计数一致，3项针对性测试通过；新增调用/训练0，原题、标签、正式成绩和P1保持不变，原始数据私有。

## v1.13.165（2026-09-23）

- 完成[剩余来源筛选与局部验证](docs/cpt_remaining_operations_results_20260923.md)：10项预筛、20题隔离制题/盲审，最终1组进入80次诊断，全部完成复算。
- Step120与P1闭卷均4/4稳定正确，P1给来源3/4；32次闭卷重复无变化，未发现登记要求的知识缺口与训练价值信号，本轮关闭，停止当前范围追加制题。
- 新增冻结输入、批次恢复及独立复算工具，5项针对性测试通过；原始数据私有，P1和原阶段C门槛保留，无新增训练或正式集重跑。

## v1.13.164（2026-09-22）

- 完成[制题前判断操作盘点](docs/cpt_operation_feasibility_20260922.md)：39个能力组分流，4类材料隔离审阅，16项抽象操作与39处引文核验。
- 历史对照发现四组均包含既有判断，尚未认证完整新组；保留两条待核验线索，明确8组尚未深入，避免将局部未放行误称来源穷尽。
- 新增安全复算台账；本轮新题、学生调用及训练均0，原始来源与审阅保持私有，P1和原阶段C门槛保留。

## v1.13.163（2026-09-22）

- 完成[仅Logistika隔离制题与审阅](docs/cpt_logistika_independent_results_20260922.md)：32题、152项盲审判断；26题内容合并通过、4个完整内容组，历史操作核验后0组放行。
- 21份历史文件的词面排重未命中，但主审发现具体条件组合复用；另记录依据不足和测量设计问题，未修改草稿或追加制题。
- 新增题干/选项排重与安全复算工具，5项测试通过；本轮关闭，学生诊断、正式评测及训练均0，P1与原双集门槛保留。

## v1.13.162（2026-09-22）

- 另立[仅Logistika制题与筛选登记](docs/cpt_logistika_independent_registration_20260922.md)，冻结8组来源、32个草稿位置，潜在诊断上限432次；不训练、不改原双集门槛。
- 实现只含来源的作者输入、隐藏作者答案的审阅导出及不可覆盖的评分冻结；3项隔离/证据校验测试通过。
- 当前完成执行准备，尚待确定隔离子代理或人工方式；草稿、审阅及学生模型调用均0，旧自动制题服务保持停止。

## v1.13.161（2026-09-22）

- 完成[SC来源补查与恢复条件](docs/cpt_sc_source_readiness_20260922.md)：复核9条部分依据/争议项目，将31条未解决记录整理为29组资料验收清单。
- 补查税制适用范围和标签标准线索，未新增合格组；SC仍1来源组，原阶段C门槛及旧判定保留。
- 新增可复算的来源依赖汇总，明确停止重复检索及恢复触发条件；模型调用、训练和制题均0，原始资料私有。

## v1.13.160（2026-09-22）

- 完成[完整规则候选规格](docs/cpt_structured_candidate_plan_20260922.md)：8组来源交接包及逐组覆盖/拒收条件，收窄铁路装卸和拣选范围。
- 复核来源指纹及269条来源台账；阶段B仍0合格组、SC仅1来源组，保留原阶段C门槛与自动制题停止状态。
- 新增可复算的私有交接构建器及安全门槛汇总；新增题目、模型调用和训练均0，原始材料不入Git。

## v1.13.159（2026-09-22）

- 完成[三项规则判别对照](docs/cpt_rule_disambiguation_results_20260922.md)：新增38次回答、复用4次，匿名评分冻结与42条复算通过，无训练或正式评测。
- K分类普通0/3、逐类3/3，R均0/3；两模型载重原说明0/3、等长度中性及删除均3/3；铁路单轮聚焦未形成稳定调用。
- 按预设判定收窄为完整分类表述和竞争说明干扰两个候选信号，保留P1、旧分数及阶段C门槛；原始记录私有，本轮关闭。

## v1.13.158（2026-09-22）

- 完成[四单元前文对照](docs/cpt_context_bridge_results_20260922.md)：16次规则回答、16次概率评分，匿名评分冻结后揭示、独立复算通过；无训练或正式集重跑。
- 铁路规则K两种前文均通过、R均未通过；组合载重两模型均在来源前文通过、K既有前文未完整支持。未观察到正确前文普遍修复所有缺口。
- 新增冻结构建器、可恢复协调器、匿名评分汇总器及安全台账；原始记录私有，P1保留，本轮关闭不晋级。

## v1.13.157（2026-09-22）

- 完成[逐事实调用缺口复盘](docs/cpt_source_gap_results_20260922.md)：全部26项必要事实的来源片段概率均改善，包括K自由回答始终缺失的18项；仅为正确前文下的预测信号，不宣称已理解。
- 关联原回答，发现标题正确仍缺事实、回答长于来源仍遗漏条件的反例；收窄下一步为内容选择与前文依赖，不直接增加训练或输出预算。
- 新增离线关联脚本、安全台账与104项独立复算记录；新增模型调用和训练均0，保留P1及全部冻结评分，原始内容私有。

## v1.13.156（2026-09-22）

- 完成[固定来源概率检查](docs/cpt_source_nll_results_20260922.md)：四模型24请求、1,428监督token，K六段正文均优于三个对照，排除仅标题/结束符改善的解释。
- 联合已有自由调用结果，将诊断更新为“原文条件预测改善与不完整调用并存”；不据此宣称规则理解、迁移或晋级。
- 新增冻结登记、概率协调器、逐token复算器和安全台账，原始记录私有；无新增训练或正式评测，24个接口解码token丢弃。

## v1.13.155（2026-09-22）

- 完成[72次原训练提示对照](docs/cpt_source_recall_results_20260922.md)，匿名逐事实评分先冻结后揭示，调用与指纹独立核验通过。
- K仅内河客船单元3/3完整通过，其余5单元未通过；其他三模型54条均在384token截断，明确严格分数不能归因于纯知识增益。
- 更新提示依赖与学习缺口结论，保留P1、旧K/R分数及预算；不补跑、不训练、不晋级，原始结果保持私有。

## v1.13.154（2026-09-22）

- 启动用户批准的[72次原训练提示调用对照](docs/cpt_source_recall_execution_20260922.md)，四模型身份、精确训练消息、推理代码及设备门禁通过；Step120已完成18次，P1运行中。
- 新增独立协调器和匿名评分摘要器，保留调用预登记与冻结评分保护；不训练、不重复正式评测，不扩展原K/R预算。

## v1.13.153（2026-09-22）

- 完成[规则学习环节离线核验](docs/cpt_mechanism_learning_audit_20260922.md)：重建冻结采样顺序并逐步验证两臂222批token，确认6段来源各3次曝光；新增来源监督占K总量8.07%。
- 明确混合批次loss不能证明逐规则学习、旧探针未覆盖原训练提示调用；准备独立72次原提示测量登记，尚未运行，新增训练及生成均0。
- 保留K/R原结果和P1，不追加原轮预算；新增可复算审计脚本、安全台账及下一步判定。

## v1.13.152（2026-09-21）

- 在客户训练手册开头补充新服务器解压后进入目录、首次导入离线镜像的命令，说明镜像只导入一次、容器由训练入口自动管理及安装目录可调整；同步5号机交付目录。

## v1.13.151（2026-09-21）

- 将客户运行手册简化为放数据、改参数、运行命令三步，明确完整轮次训练所需的步数限制、保存频率和超时设置；部署、验收和导出内容另存运维说明。
- 移除数据预检对示例答案中Answer和calculator字样的硬编码要求，保留监督范围检查，允许同格式普通业务SFT数据；交付包版本更新为0.2.3。

## v1.13.150（2026-09-21）

- 将用户指定的CPT导出模型独立复制到5号机交付包model/CPT，保留来源目录，核对权重索引、张量形状及逐文件SHA256回读。
- 交付包更新为0.2.2，补齐模型来源和验证说明，更新运行手册、快速开始及验收状态；默认训练基座保持原始模型，未重新执行训练或推理。

## v1.13.149（2026-09-21）

- 新增[业务方运行手册](delivery/business_training_delivery/docs/业务方运行手册.md)，覆盖异机部署、离线镜像导入、五类一行训练、成功判定、模型导出、数据替换和故障处理。
- 按现有入口核对命令和环境要求，明确宿主机驱动准备、单步验证范围及正式训练限制；同步5号机交付目录和文件校验清单。

## v1.13.148（2026-09-21）

- 按用户要求清理5号机业务训练交付包的验收检查点、HF测试导出、运行日志、缓存和本次临时文件，保留原始模型、五类示例数据、训练入口、共享框架及离线镜像。
- 保留清理前的验证汇总并明确标注产物已删除，更新导出操作说明、清理记录和交付文件校验清单；交付包版本更新为0.2.1。
...
- K目标修复1/7，但完整规则调用0/36；对P1的SC净-2、Logistika净+2、合计持平。K优于R仅为待确认配方信号，保留P1，不追加试验或晋级。
- 提交来源—监督—调用—正式得失安全台账及30行独立复算，更新总计划；原题、回答和评分保持私有。

## v1.13.145（2026-09-21）

- 将5号机业务训练交付目录移动至 `/data3/llin/business_training_delivery`，同步更新交付说明；确认旧目录已移除、原始模型链接仍可访问。

## v1.13.144（2026-09-21）

- 在5号机整理业务训练交付目录，涵盖五类训练、模型预留、共享框架、环境、工具和输出；原始Qwen3.6-27B以软链接接入。
- 新增[交付方案](delivery/business_training_delivery/docs/交付方案.md)、历史实现索引及五类自编格式样例；本轮仅目录整理，启动脚本和训练验收待后续完成。

## v1.13.143（2026-09-20）

- 核实K臂首次初始化因NPU通信端口占用失败：16rank均在加载权重阶段退出，零训练步、无检查点，72次基线调用指纹保持一致；[恢复记录](docs/cpt_mechanism_validation_20260920.md)。
- 在锁与进程核验后完整归档失败目录，保持冻结02包和训练配方不变；等待器要求持续120秒空闲检查再接续，不更换通信端口抢占设备。
- 新增零步故障恢复保护和等待窗口测试，18项通过。本次跟进新增生成0次，训练收益仍待验证。

## v1.13.142（2026-09-20）

- 实施[来源知识机制验证](docs/cpt_mechanism_validation_20260920.md)：复算9组45,144条正式预测，关联10个来源单元、实际监督位置及历史知识探针，固定6个单元、7道目标题；保留阶段B失败及阶段C范围门槛。
- K/R各保留P1原315条并追加18条，各111步；真实token、监督掩码、安装加载器、相同采样顺序及基线身份/协议门禁通过。72次基线调用已保存，设备占用时被动等待，K/R尚未开训。
- 新增按批保留进度与调用预登记、匿名事实评分和目标/非目标得失汇总；12项测试通过。开训前明确固定引擎和采样种子1；新增生成上限10,176次，不调参、不追加第三臂、不晋级模型；原始资料保持私有。

## v1.13.141（2026-09-20）

- 完成[阶段B两组质量小试](docs/cpt_stage_b_two_group_pilot_20260920.md)：8次制题、7题完整1题截断；不同模型本地盲审先冻结判断再比较作者依据，2题内容与结构通过但0个完整组，诊断/训练调用0次。
- 发现正确来源ID仍伴随船型指代错误、未给出的港口选择条件及无关事实反驳；按预先停止条件停止扩大当前自动制题，不补题凑组。
- 完成2785条历史记录词面排重并披露语义重叠；19项测试通过，提交安全登记、结果、质量与排重台账，原始题目保持私有。

## v1.13.140（2026-09-20）

- 完成[阶段B引用及内容核验](docs/cpt_stage_b_citation_review_20260920.md)：26/32题引用片段可定位，12题进入本地内容审阅；4题语义暂缓、2题设计暂缓、6题答案有依据但未放行，仍0个完整能力组。
- 识别位置检测误报，以及原盲审已通过拣选题的过强推论；不把来源定位或同模型一致性当作标签正确证明，保留原判。
- 新增带原文偏移和指纹的来源段落ID、逐选项三态证据校验及安全审计台账；14项测试通过，本轮新增模型调用0次，无诊断或训练。

## v1.13.139（2026-09-20）

- 阶段B首批制题结束：32次生成、2次盲审，2/32题通过初审、0/8组完整通过，未放行诊断或训练。
- 归档原始结果并复算：25条引用拒绝中20条含省略号拼接、3条仅空白差异、2条其他非精确引用；区分制题格式契约失败与知识内容错误，不将未盲审题判为能力失败。
- 提交安全结果与可复算审计程序；下一步先修复来源引用表示并核验逐选项依据，保留本批原判与调用台账。

## v1.13.138（2026-09-20）

- 修复制题token输入：当前tokenizer的chat模板返回BatchEncoding，旧程序误将映射交给推理引擎；改为渲染文本后显式编码，并逐项检查整数token及真实窗口长度。
- 推理引擎异常现在立即失败，不再被归入题目质量拒绝；9项测试通过。此前run03实际0次调用、0道生成题，32条均为执行错误，不能计为制题或审阅失败率。
- 修复版在独立v3目录启动，保留失败记录，来源及64次制题/审阅上限不变。当前未进行诊断或训练。

## v1.13.137（2026-09-20）

- 修复阶段B设备等待误判：实测无运行进程时基础显存仍约2.9—3.1GB，原2GiB阈值阻止启动；改为全部芯片低于4GiB且8个NPU明确无运行进程，保留项目锁。
- 8项测试通过，旧等待任务零调用退出；独立目录部署修复版，13:33确认进入模型加载。来源、题目范围和64次制题/审阅预算不变，尚无诊断或训练结果。

## v1.13.136（2026-09-20）

- 按已确认的范围调整启动[阶段 B Logistika 首批筛选](docs/cpt_stage_b_logistika_screen_20260920.md)：冻结8个来源能力组，最多32道候选题，制题/审阅最多64次、后续诊断最多432次；SC待源及阶段C门槛不变。
- 部署来源隔离制题与盲审程序，实际模型身份、分词窗口及上传指纹检查通过；制题进程正在等待设备空闲，最多等待6小时，生成后停在本地质量与排重检查。
- 7项本地协议测试通过；归档安全规格和状态快照。目前无生成结果、诊断分数或新增训练，4/6选项筛选不代表长候选任务已解决。

## v1.13.135（2026-09-20）

- 完成[阶段 A 第三批19条候选核验](docs/cpt_stage_a_methods_20260920.md)：MIT原课件补齐1条完整多选来源，Logistika累计48条、保守去重39组；其余17条部分依据、1条待裁定不放行。
- 核验实际训练中7条仓储题的相邻监督，区分已有主题任务与完整分类覆盖；提交安全增量台账，原题与来源正文保持私有。
- 7页原PDF提取对照、3页视觉核验、10,032条预测复算及重复重建通过；SC资料门槛未变化，本轮无新增模型调用或训练。

## v1.13.134（2026-09-20）

- 完成[阶段 A 第二轮补证](docs/cpt_stage_a_followup_20260920.md)：新增5条Logistika来源合格项，累计47条、保守去重38组；SC仍1条，训练门槛不变。
- 逐文件核验SC最新公开包与本地一致，226条记录指向的上游文件未包含；区分论文435题与当前发布范围，登记资料依赖及两个来源侧候选规格。
- 保存第二轮安全台账、来源指纹和复算程序；重建一致、首次结果兼容、3项来源故障检查通过。新增模型调用和训练均为0。

## v1.13.133（2026-09-20）

- 执行[阶段 A 来源与覆盖审计](docs/cpt_stage_a_audit_20260920.md)：完成269条共同错误基础判定及P1/P4共10,032条原始预测复算，提交安全逐题台账和复算程序。
- SC严格确认1条；Logistika确认42条、保守去重36条，其中17条已有明确规则监督。SC来源门槛未通过，进入资料补全，不以来源数量代替训练价值。
- 核验363条实际训练消息及315条P1前缀，重建一致、5项故障注入拒绝；更新实验计划执行状态，本轮新增模型调用和训练均为0。

## v1.13.132（2026-09-20）

- 新增[下一阶段实验计划](docs/cpt_next_experiment_plan_20260920.md)：先核实共同错误的可修复空间与候选训练价值，再比较直接知识监督和迁移导向监督。
- 明确数据隔离、能力匹配保持、token预算、两臂固定正式后测、停止条件及最多一次种子确认；初始推理上限14,464次，含确认最多21,864次，制题/审阅另行登记。
- 本次仅完成方案文档，未启动制题、模型推理或训练；保留P1及196/1224目标，不以局部小试替代正式评测。

## v1.13.131（2026-09-20）

- 完成 P1/P4 全部 43 条正式正误变化审阅及三模型 15,048 条原始预测复算：18 增益、25 损失，净 -7；保留官方标签和评分。[逐题复盘与下一步](docs/cpt_p4_error_audit_20260920.md)。
- 核对实际训练消息与来源：25 条损失中 2 条已有明确规则监督、6 条只有相关覆盖；缺图、企业流程和可能等价标签单独记录。269 选项损失涉及的任务与此前设备小试不对应，停止同类扩量。
- P1/P4 正确集合即使理想合并也仅为 SC 194、Logistika 1209，仍未达标；后续需同时扩大有效覆盖并验证条件迁移和保持。本轮没有新增训练或推理，提交安全台账和复算程序，原始题目留在本地私有目录。

## v1.13.130（2026-09-18）

- 完成第二批开放数据资产备份：40G训练评测主库、人工核验/SFT/物流精选包以及0号机FEV与GiftEval时序评测包均已归档、双端校验并上传百度网盘。
- 新增随包恢复清单，记录源路径、文件数、原始与归档规模和SHA-256；云端确认4个文件上传完成后，已删除本地及两台服务器临时归档，源数据保持不变。
- 更新[长期清单](docs/open_source_dataset_catalog_20260918.md)和[机器可读目录](docs/open_source_dataset_catalog_20260918.json)，将本批资产状态改为已沉淀并记录准确网盘位置。

## v1.13.129（2026-09-18）

- 完成设备边界固定768次及本地复算：Step120为252/256，P1/P4均256/256；零无效/截断，同提示两次零答案变化。[完成结果与判断](docs/cpt_equipment_boundary_20260918.md)。
- Step120的4次错误均来自同一道驶入式货架题：正确项首位时误选后推式、末位时正确；来源复核保留标签。两种列表长度没有成对退化，仍未区分P1/P4或解释正式回退。
- 停止简单设备分类题扩量，不追加训练；后续转回已用正式集的错误机制及来源覆盖核对，尚未启动。全部预测、输入指纹与模型身份核验通过，远端锁已释放。[安全结果](docs/cpt_equipment_boundary_result_20260918.safe.json)。

## v1.13.128（2026-09-18）

- 完成5号机、0号机开放数据资产盘点，按“必须备份、选择性沉淀、仅登记来源”三级整理路径、规模、用途、重复关系和合规边界。[长期清单](docs/open_source_dataset_catalog_20260918.md)。
- 确认已备份的5.2G LLM RAW只是40G主库的子集；主库另有13类约28G原始数据及7.4G统一加工数据，列为下一批最高优先级资产。
- 新增机器可读目录，记录百度网盘现有备份、校验值、候选资产与禁止直接上传个人网盘的内部数据边界。[JSON目录](docs/open_source_dataset_catalog_20260918.json)。

## v1.13.127（2026-09-18）

- 启动16道物料搬运相近设备辨析：输送、车辆、货架、起重机四族，4/16选项、概念/原样查找、首末位置、两次同批重复，Step120/P1/P4固定768次。[冻结设计及停止规则](docs/cpt_equipment_boundary_20260918.md)。
- 四个MHI来源页面归档核验、2,496个问题签名排重及8项相关测试通过；短列表保留同族竞争选项，全部题目只评估，不训练。来源新归档不代表模型或正式集未见过相关知识。
- 三模型身份核验后启动，结果待收齐；若再次全对，停止同类诊断扩量，不自动追加训练。[来源、审计与启动登记](docs/cpt_equipment_boundary_launch_20260918.safe.json)。

## v1.13.126（2026-09-18）

- 完成短/长列表固定1,728次测量及本地独立复算。Step120、P1、P4各576/576全对，4/32/128选项和所有位置均全对，同提示三次零答案变化，零无效/截断。[完成结果与判断](docs/cpt_list_probe_20260918.md)。
- 小试没有复现缺口；8个熟悉概念存在满分上限，不能据此排除269选项正式任务的格式影响，也不支持启动索引专项训练。后续转向来源独立的相近概念和边界辨析，尚未启动。
- 修复本地Windows核验器的远端路径序列化，远端协议和原始预测不变；所有输入指纹、登记、成对统计和结束后模型身份核验通过，实验锁已释放。[安全结果](docs/cpt_list_probe_result_20260918.safe.json)。

## v1.13.125（2026-09-18）

- 启动来源自编的短/长候选列表小试：8个概念，概念识别/原样查找两模式，4/32/128选项、4种位置、3次相同批次重复，Step120/P1/P4固定1,728次。[冻结设计与判读规则](docs/cpt_list_probe_20260918.md)。
- 8条归档证据核验、2,488个问题签名排重、输入窗口及选项映射检查通过；提供高位索引解析测试和完整原始结果复核器。全部题目仅作评估，不训练、不重测正式集。
- 冻结模型指纹、脚本与数据包，远端已启动；结果尚待收齐。128选项和8个熟悉概念不能代表269选项正式题。[启动登记](docs/cpt_list_probe_launch_20260918.safe.json)。

## v1.13.124（2026-09-18）

- 完成88题、792次多来源四顺序基线和盲审，全部输入token及原始预测本地复算通过；P1训练176/192、开发83/96、保持59/64。[结果与决策](docs/cpt_broad_packet_20260918.md)。
- 盲审78/88通过，10条模型矛盾由Codex逐项裁定，标签不变；47道来源合格训练候选中40道已四顺序全对、7道不稳定。隔离1道公式题，不整批追加训练。[候选释放](docs/cpt_broad_release_20260918.safe.json)。
- 既有正式预测分层发现P4在269选项组比P1净少对7题，短选项组净多对5题；格式与类别混杂，不能作因果结论。下一步转向来源独立的短/长列表受控测量，尚未启动；本轮无新训练或正式集重测。[分层证据](docs/cpt_format_gap_20260918.safe.json)。

## v1.13.123（2026-09-18）

- 构建多来源 v2 候选：48训练、24开发、16保持，覆盖12项能力、21个训练来源单元和6个连通组，其中8个来源未用于P1。[范围与测量](docs/cpt_broad_packet_20260918.md)。
- 2,400个问题签名排重、69项等式、2,112次排列答案映射及29项相关测试通过；预先隔离1道直接给出公式的训练候选，保留历史开发来源分区。[来源与审计](docs/cpt_broad_sources_20260918.safe.json)。
- 启动固定792次Step120/P1四顺序基线与隐藏标签盲审；Step120已完成，P1及后续裁定待收齐。没有新训练或正式集重测。

## v1.13.122（2026-09-18）

- 完成 P4 的 121 步累计 SFT 和固定 5,284 次后测。正式 SC 191/226、Logistika 1187/1446，较 P1 净 -1/-6，未达目标，不替换 P1。[完整复盘与下一步](docs/cpt_p4_cumulative_20260918.md)。
- 新训练/开发为 20/32、11/16，但旧迁移与保持回退；Logistika 主要损失来自物料搬运。后续转回多来源类别覆盖、规则迁移与顺序稳定性，不自动追加 P4 调参或 CPT。
- 全部新预测、10,032 条正式对照、1,940 组输入 token、396 条数据双端核验通过，模型完整导出检查通过；零无效/截断，26 项相关测试通过。保留统计与预算限制。[安全结果](docs/cpt_p4_result_20260918.safe.json)。

## v1.13.121（2026-09-18）

- 启动单次 P4 累计 SFT：保留 P1 原 315 条消息与 token，追加已释放 16 题的 48 次曝光，共 363 条、121 步；冻结 99,038 序列 token 与 14,710 监督 token。[方案与运行记录](docs/cpt_p4_cumulative_20260918.md)。
- 远端实际加载器及本地全部 396 条记录重建通过；保持开发/保持题隔离。训练后固定执行 180+88+5,016 次推理，不因局部得分取消正式测量或自动追加训练臂。[启动登记](docs/cpt_p4_launch_20260918.safe.json)。
- 当前环境、对照模型记录、正式脚本及输入 token 与既有 Step120/P1 对照一致；已观察到实际训练步骤，尚未取得本轮效果结论。

## v1.13.120（2026-09-17）

- 完成近期实验全局复盘：P1保持当前正式点估计领先，P3的局部保持优势不能代理正式效果；121步方案定位为一次累计迁移验证，建议事前纳入完整双集测量。[方向复盘](docs/cpt_recent_direction_20260917.md)。
- 复算P1剩余错误：SC履约/协同占24/34，Logistika搬运/运输/仓储占235/253；新24题的27次错误分为漏选10、多选8、替换9，不能简化为格式问题。[证据与指纹](docs/cpt_recent_direction_20260917.safe.json)。
- 后续主线为来源充分的覆盖扩展与类别保持，再做同SFT数据的CPT配对；本轮无新推理、训练或自动任务，不改变既有实验门槛。

## v1.13.119（2026-09-17）

- 完成16道新训练、8道开发、20道分层保持检查的来源编写；修订四处前提/措辞，70项精确等式和2,353个历史问题签名排重通过。[数据与审核](docs/cpt_remediation_packet_20260917.md)。
- 完成固定240次前测及来源复核。最终P1为训练12/32、开发9/16、保持39/40，Step120为14/32、6/16、36/40；批量成本仍是主要缺口。全部结果双端重建、独立解析和成对提示token核验通过。
- 盲复核36/44通过，另8条模型矛盾/推断错误保留并逐项裁定；仅释放16道训练题，28道评估题禁止训练。14项相关测试通过；本轮未执行新训练或正式集重测。[释放范围](docs/cpt_remediation_release_20260917.safe.json)。

## v1.13.118（2026-09-17）

- 完成P1同16题、双选项顺序的32次对照：23/32，对照Step120为25/32；改对1、新增错3，新增错误均漏选一个正确项。两个顺序都答对为10/16，保持8/8。[完整结果](docs/cpt_p1_authoring_compare_20260917.md)。
- 确认批量成本干预、截止时间、运力逆推及拣选指标的具体残留缺口，冻结最多16道新训练候选、8道开发候选的来源编写规格；尚未生成或释放新训练数据，不把本组诊断题用于补训。
- 模型记录和全部32条输入token匹配，双端重建及独立解析通过，零无效/截断；10项相关检查通过。无新训练、正式重测或提示搜索。

## v1.13.117（2026-09-17）

- 收齐58次制题及32次盲复核：23道过滤通过，仅5道可作事实素材、18道需改写或剔除；2314个历史问题签名排重及原始输出重建通过，原批次不进入训练。[逐项审计](docs/cpt_expansion_draft_audit_20260917.safe.json)。
- 完成16道来源题的小试：双顺序闭卷25/32，训练13/16、开发4/8、保持8/8；来源复核14/16，另2道出现解释算对但答案索引矛盾。追加32次答案组织对照仅到26/32，改对2、新增错1，双顺序均正确仍11/16，不采用为主要修复方案。[协议与结果](docs/cpt_source_authoring_pilot_20260917.md)。
- 累计40项相关检查通过；无新训练、无正式集重测，不将素材数量或同模型过滤冒充数据释放。

## v1.13.116（2026-09-17）

- 完成 1672 题当前得失台账：P1 的 25 道回退有 23 道不在旧登记，23 道为三轮持续回退，11 道表现为只漏选；保持正式评分不变。[分析与范围](docs/cpt_expansion_20260917.md)。
- 冻结 21 个扩展能力、36 个来源单元，其中 17 个未进入 P1 领域或回放训练；保留历史开发隔离，212 条来源引用指纹核对通过。[来源卡片](docs/cpt_expansion_sources_20260917.safe.json)。
- 修复新草稿不能全选的限制，答案数量对盲复核隐藏，旧包行为保留；30 项检查通过。已启动固定 58 次生成及最多 237 次复核，仅产出待审草稿，不启动训练。[回退审阅](docs/cpt_expansion_regression_review_20260917.safe.json)。

## v1.13.115（2026-09-17）

- 完成固定 15,048 次正式双集测量：当前 Step120 为 190/1180，P1 为 192/1193，P3 为 189/1185；目标 196/1224 均未达到。[完整结果与决策](docs/cpt_formal_transfer_20260917.md)。
- P1 对当前基线改对/改错为 SC 4/2、Logistika 36/23；P3 为 5/6、40/35。保留 P1 配方供覆盖扩展，停止以算术保持小试替代正式效果判断；点估计收益尚未证明稳定泛化改善。
- 全部原始输出双端复算及本地独立解析通过，1 条无效回答照原规则计错、0 截断；补充可复现核验脚本和 Windows 路径兼容修复。[安全汇总](docs/cpt_formal_transfer_result_20260917.safe.json)。

## v1.13.114（2026-09-17）

- 执行全局复审的正式目标测量：冻结 P1/P3，并因历史缺完整运行环境指纹加入当前 Step120 对照，固定 15,048 次回答；三个模型的全部提示 token 均与历史一致。[方案与状态](docs/cpt_formal_transfer_20260917.md)。
- 直接复用原双集三次严格多数评测程序，不新增训练、不改 P3 原判定；目标仍为 196/226 与 1224/1446。29 项相关检查通过，持锁流程已启动。[登记](docs/cpt_formal_transfer_launch_20260917.safe.json)。

## v1.13.113（2026-09-17）

- 完成全局路线复审：重算历史正式三轮20,064条记录、P1—P3分层及来源关联，确认近期小试尚未测正式目标，10规则族的充分目标覆盖仍未建立。[全局分析](docs/cpt_global_review_20260917.md)。
- 保留P3原门槛未通过的结论，修正“探索未通过即可否定方法”的决策含义；建议下一步冻结P1/P3做一次有界目标测量，再按实际收益决定扩覆盖与CPT配对，不再先开P4扫参。[重算证据](docs/cpt_global_review_20260917.safe.json)。
- 本轮仅分析与前瞻建议，无新模型调用、训练、正式评测或定时任务。

## v1.13.112（2026-09-17）

- P3完成105步和固定后测：训练47/60、新表达10/20、新条件12/20、原保持52/80；原保持相对P2改对5题、改错0，首次达到本轮52门槛。[完整结果](docs/cpt_transfer_p3_result_20260917.md)。
- 新算术检查为46/60，低于预登记47门槛（Step12044、P247）；两个乘积求和8/15。整体未通过，停止当前内容替换路线，不追加扫参或正式评测。
- 360条新回答加360条复用基线全部双端重解析和token重建通过，零无效/截断；74,573实际训练token、检查点及1,199张量/15分片导出核验通过。[核验](docs/cpt_transfer_p3_result_20260917.safe.json)。

## v1.13.111（2026-09-17）

- 按P3预登记生成并冻结60通用算术训练题和60检查题，四种操作各15题，独立精确整数运算、标签及交换等价排重通过；39项相关测试通过。
- P3替换60额外回放位置，其余255消息逐字不变，实际加载为74,573序列/8,770监督token，较P2分别-4,029/0。已启动持锁的两模型算术前测→唯一105步训练→固定后测流程；无定时自动任务，不重跑旧诊断。[登记](docs/cpt_transfer_p3_launch_20260917.safe.json)。

## v1.13.110（2026-09-17）

- 完成P2后的回放覆盖审查：135旧题含88定义、34边界、13应用，明确数值计算仅1题；80保持题中72题为数值任务。P2新增60次回放中55次仍属定义/边界，未实质补齐计算练习。[分析](docs/cpt_replay_coverage_20260917.md)。
- 完成三模型405次旧训练题拟合诊断：Step120为123/135，P1为128/135，P2为131/135；相对基线分别改对5/8题且均无新增错题。405回答双端重解析和token重建通过，32项测试通过；这不是保持或泛化成绩。[核验](docs/cpt_replay_fit_result_20260917.safe.json)。
- 预登记P3内容替换对照：固定P2配比、255个原位置及105步，仅将60次额外旧题回放替换为独立通用算术练习；禁止保持题/改写进入训练，不再搜索回放比例。P3尚未训练。[设计](docs/cpt_transfer_p3_design_20260917.md)。

## v1.13.109（2026-09-17）

- P2完成105步训练和唯一一次后测：训练42/60、新表达10/20、新条件11/20、保持47/80；相对两个已有基线，保持分别净降4、5题，未达到预登记52/80门槛。[核验结果](docs/cpt_transfer_p2_result_20260917.safe.json)。
- 180条新回答及360条复用基线在服务器和本地独立重解析，540条输入提示重建一致，零无效/截断；训练token、检查点及1199张量/15分片导出核验通过。停止该混合配比路线，不追加扫参、重复训练或正式评测。[分析](docs/cpt_transfer_p2_result_20260917.md)。

## v1.13.108（2026-09-17）

- P2确定性混合数据通过逐条独立核验和实际训练器加载检查：120新任务曝光加195历史回放，共315条，255位置不变；实际78,602序列/8,770监督token，较P1减少8,738/3,432，不宣称token预算相同。
- 33项测试通过；SSH恢复后启动唯一P2训练，105步、1遍、同Step120初始化和学习率，后测复用两个既有基线分别比较，保持门槛52/80，封存及正式评测不运行。[运行登记](docs/cpt_transfer_p2_launch_20260917.safe.json)。

## v1.13.107（2026-09-16）

- P1唯一追加配对复测完成：训练18→51、新表达6→11、新条件7→11、保持52→50；保持净降2题方向复现，原主结果51→49保持不变。[独立核验](docs/cpt_transfer_p1_stability_result_20260916.safe.json)。
- 360条回答独立重解析及token重建一致，零无效/截断；基线2题、P1一题正确性跨运行变化，不能等同独立训练seed或稳定总体遗忘。停止同样复测，预登记只调整保持采样占比的P2，尚未训练。[P2设计](docs/cpt_transfer_p2_design_20260916.md)。

## v1.13.106（2026-09-16）

- P1追加复测因入口覆盖Ascend运行库路径而在初始化阶段失败，未产生回答；已保留失败目录，新增保留环境路径及acl导入检查的入口，在02目录恢复同一次复测。[恢复记录](docs/cpt_transfer_p1_stability_recovery_20260916.safe.json)。
- 模型、180题顺序和推理参数不变，无新增训练；不将失败尝试计为完成的追加测量。

## v1.13.105（2026-09-16）

- 按P1结果决定启动一次有上限的配对运行稳定性检查：固定Step120/P1两份模型、原180题顺序及推理配置，各追加一轮，共360回答；不训练、不覆盖主结果。[启动记录](docs/cpt_transfer_p1_stability_launch_20260916.safe.json)。
- 复测协调器全程持共享资源锁，实际输入token与预登记提示逐条比对；30项已有P1测试通过。追加结果待核验，不作为新训练seed或取高分依据。

## v1.13.104（2026-09-16）

- 完成P1跨类迁移小试：10规则族/6类，315训练记录、105步、87,340 token，使用同一Step120 HF初始化；190题/760选项完成来源与计算检查，84项测试通过。[小试合同与结果](docs/cpt_transfer_p1_20260916.md)。
- 前后测：训练19→51/60，新表达6→11/20，新条件7→11/20，保持51→49/80。两层迁移达到预登记探索信号，但保持净退步2题，暂停扩量和C/D；没有正式双集收益声明。[完整核验](docs/cpt_transfer_p1_result_20260916.safe.json)。
- 360条原始回答和全部分词提示独立核验通过，零无效/截断，10道封存题未查询。修复旧分布式检查点缺失的初始化路径，复用冻结前测，不改数据、标签或判断门槛。[登记](docs/cpt_transfer_p1_registration_20260916.safe.json)、[运行记录](docs/cpt_transfer_p1_launch_20260916.safe.json)。

## v1.13.103（2026-09-16）

- 将两道待修表达题另立版本，改为逐资产完整分类证明的选择，替代仅换列分隔符的汇总题；原18条其余拆分逐字保留，旧题包不改。[新版核验](docs/cpt_premise_expression_result_20260916.safe.json)。
- 68项测试通过，80选项复算与来源绑定通过；两题仍待完整结构隔离审查，不把新题面当新增知识规则或训练收益。训练发布0，无新增模型请求。

## v1.13.102（2026-09-16）

- 改为先建来源前提表和逐选项判据再生成题面，实际完成两类铁路规则20题、80个选项判定；另一套计算实现、9条来源绑定与历史开发组排除检查通过，67项测试通过。[本轮结果](docs/cpt_premise_task_audit_20260916.md)。
- 下游核对14份历史题文件；两道新表达候选仅改变格式，标为待修，其余仍为候选。保留原始版本与审查记录；训练发布0，未启动新训练，28候选和保持集缺口未变。[核验摘要](docs/cpt_premise_task_result_20260916.safe.json)。

## v1.13.101（2026-09-16）

- 来源约束小批完成：12个计划设计中3条通过形式与模型复核；独立审核后1条暂留、2条待修，仍未达到扩量质量门槛。[审核结果](docs/cpt_transfer_constraint_audit_20260916.safe.json)。
- 两条生成、三条复核的原始谱系与token输入均核验通过；训练发布0、完整6/3/1单元0。下一步转为先列完整前提和逐选项判据再构造题面，不直接扩量当前自由生成器。[谱系核验](docs/cpt_transfer_constraint_result_20260916.safe.json)。

## v1.13.100（2026-09-16）

- 为首批两个铁路来源单元建立11条逐字来源绑定的制题约束，覆盖统计框架、建设类型、适用条件、例外和计数对象；新增问题范围词检查，旧请求提示不变。[约束登记](docs/cpt_transfer_constraint_pilot_20260916.safe.json)。
- 14项制数测试通过，已在确认设备空闲后启动两请求、最多12条训练草稿的小批修复，PID3697562。旧稿保持，输出仍隔离，尚无训练或完整数据发布。[启动记录](docs/cpt_transfer_constraint_launch_20260916.safe.json)。

## v1.13.99（2026-09-16）

- 核验6题12次回答形式对照，并完成14个新规则情境的56次筛选：闭卷19/28、来源辅助25/28，全部原始回答及68条分词提示核验通过。新增7个候选后合并为28个规则族、8类，仍非训练收益。[覆盖审查](docs/cpt_coverage_expansion_audit_20260916.safe.json)、[新候选表](docs/cpt_transfer_candidate_registry_v2_20260916.safe.json)。
- 实际生成首批48条训练草稿：修复过严措辞过滤后22条进入语义审核，3条暂可保留、14条待修、5条剔除，全部仍隔离。暂停剩余批量生成，不以同模型盲审替代来源与条件核验。[制数审查](docs/cpt_transfer_draft_pilot_audit_20260916.safe.json)。
- 新增来源白名单制数、分拆任务设计、原始断点复用和独立核验工具，30项测试通过。新目标主题已从保持来源中排除，保持题计划114条仍不足120；未启动新训练、未改正式评分。[数据分配](docs/cpt_transfer_draft_plan_v2_20260916.safe.json)。

## v1.13.98（2026-09-16）

- 将四份训练材料的语料形态改为实际训练记录样例，分别展示正文、指令问答、工具调用轨迹和强化学习任务，注明来源记录与省略内容。
- 逐项核对样例与原始记录，完成四份 Word 共13页的排版检查，同步更新可复制的 Markdown 版本。

## v1.13.97（2026-09-16）

- 新增CPT、通用开源SFT、沙箱Agentic SFT和GRPO四份数据与运行说明Word，覆盖来源/构成、字段样例、实际脚本入口与环境前置条件，并提供可复制的Markdown版本。
- 核对本地脚本与5号机开源SFT代码和保存参数，区分模型、数据及训练版本；完成13页视觉检查、JSON样例与格式核验，未启动训练。

## v1.13.96（2026-09-16）

- 完成32道多条件情境的128次闭卷/来源证据与选项换位对照。原始闭卷35/64、证据53/64；1题因能力边界未写全待定，其余31题闭卷35/62、证据53/62，其中10题两顺序均错、5题给来源后两顺序均对。[结果及审查](docs/cpt_composed_boundary_audit_20260916.safe.json)。
- 追加31题372次逐条判断：闭卷完整正确17→16/31、证据25→22/31，未改善，不支持仅靠拆分回答解决错误；原始回答与全部500条分词提示均已独立核验。[对照结果](docs/cpt_atomic_boundary_result_20260916.safe.json)。
- 新17个有闭卷错误的情境与旧7个来源规则合并3处重叠，登记21个候选规则族、8类；不等于21个稳定独立缺口，未达到训练门槛。新增推理和核验工具17项测试通过，无新训练或正式评测收益。[候选登记](docs/cpt_transfer_candidate_registry_20260916.safe.json)。

## v1.13.96（2026-09-16）

- 已手工构造并逐项审核6道多条件来源情境，涉及5个来源单元、3个历史连通组；数值答案以精确分数独立核算，题目与来源均不读取官方评测。[登记](docs/cpt_reviewed_scenario_probe_20260916.safe.json)。
- 已在空闲设备启动12请求回答形式对照，PID3680359，保持同一96 token预算；仅作缺口和输出形式诊断，不是训练或正式评测。两道绩效题重叠，不能把6题计成6个独立缺口。[启动](docs/cpt_reviewed_scenario_launch_20260916.safe.json)。

## v1.13.95（2026-09-16）

- 第二批119请求完成并核验：46道模型初筛题基线暂答对44道，零无效/截断；两道WMS疑似错误存在定义范围或选项前提问题，均待定，新增确认知识缺口0个。[结果](docs/cpt_transfer_screen_round2_result_20260916.safe.json)。
- 原始谱系、独立答案重解析、119条服务器分词提示指纹全部通过；累计仍7个规则/5类，未开新训练。下一步改为完整来源支持、先逐项审核的多条件情境，停止按当前方式继续批量生成简单定义题。[审查记录](docs/cpt_transfer_screen_round2_audit_20260916.safe.json)。

## v1.13.94（2026-09-16）

- 扩展来源筛选已启动：24个未进入上一批的来源片段，覆盖6类、11个历史连通组，优先仓储、定量规则及业务系统；最多72道新探针，旧开发组与主题继续排除。[来源登记](docs/cpt_transfer_screen_round2_20260916.safe.json)。
- 筛选器支持显式登记来源数量，原64条默认不变，13项测试通过；已核对设备空闲并启动PID3667214，目前加载模型，无新训练。[启动记录](docs/cpt_transfer_screen_round2_launch_20260916.safe.json)。片段数不等于独立缺口数，预测与海关规则后续需合并去重。

## v1.13.93（2026-09-16）

- 广域筛选完成316次请求；118道模型初筛题中基线答对102道。16道疑似错题逐项复核后保留7个来源规则、覆盖5类，另9道待定或剔除，未达到新训练的数据门槛。[筛选结果](docs/cpt_transfer_screen_result_20260916.safe.json)。
- 原始谱系、答案解析及316条实际分词提示指纹核验通过；本轮无新训练、无正式评测收益。下一步扩充来源覆盖并重制缺条件情境，保留筛选题隔离。[分词与审查摘要](docs/cpt_transfer_screen_token_audit_20260916.safe.json)。

## v1.13.92（2026-09-16）

- 确认共享NPU空闲后，已在独立目录接续广域来源筛选，PID3659070；复用已核验的前32条生成记录，继续剩余来源及后续盲审、基线。[恢复状态](docs/cpt_transfer_screen_recovery_20260916.safe.json)。
- 保留旧失败目录及输出；当前为数据筛选推理，尚未开始新训练。

## v1.13.91（2026-09-16）

- 广域筛选在落盘32个来源后因推理工作进程退出中断；新增带来源顺序、文件和提示指纹校验的生成前缀恢复，13项测试通过。[恢复登记](docs/cpt_transfer_screen_recovery_20260916.safe.json)。
- 已取回并核验32条记录；同设备当前被另一数学训练占用，恢复暂未启动，保留失败日志及原始拒绝记录，不干预其他任务。

## v1.13.90（2026-09-16）

- 已启动广域来源筛选：64个候选来源单元、8类知识、18个历史来源组，最多192道边界/条件组合/情境变化题，依次进行来源制题、盲审及原Step120闭卷基线。[运行登记](docs/cpt_transfer_screen_20260916.safe.json)。
- 制题输入仅包含历史审核来源，冻结开发组和主题排除；筛选题不进入训练、迁移开发或封存确认。模型初筛标签仍需独立语义核验，候选来源数不等于已验证知识缺口数。
- 新增来源筛选、原始输出追溯及独立解析工具，12项合同测试通过。当前运行仅为推理，尚无新训练或正式评测提升；两训练臂的实际保存预算已核算，分盘落点仍待登记。

## v1.13.89（2026-09-15）

- 以完整来源页继续裁定4道题，新增3道充分证据题，累计17道；两道铁路分类题共享同一来源页，不按独立单元重复计数。[增量记录](docs/cpt_source_review_round3_20260915.safe.json)。
- 另1道历史时点题保留版本证据缺口；尚有92道未建立充分唯一证明，官方标签与训练数据不变。

## v1.13.88（2026-09-15）

- 继续核对6道待定题的来源，原页核验新增1道充分证据题，累计14道；另1道缺少原定义范围限定、4道现有检索上下文不足。[增量记录](docs/cpt_source_review_round2_20260915.safe.json)。
- 保留原13题试验及官方标签，尚有95道未建立充分唯一证明；审查包仅用于诊断，不进入训练制数。

## v1.13.87（2026-09-15）

- 完成6个非开发来源主题、12请求的回答形式配对诊断：仅答索引5/6、简短规则后作答6/6，唯一改对为盈亏平衡计算，零截断；12条原始输出独立重解析通过。[结果](docs/cpt_broad_format_probe_20260915.safe.json)。
- 固定原Step120及96 token预算；仅作首批难度检查，单题信号不计训练收益、不替代30–50单元门槛。首次启动换行问题已修复并保留失败日志，暂无新训练。

## v1.13.86（2026-09-15）

- 按实验路径分析推进迁移验证：重解析Step120/实验16/S4/S5共20,064条正式回答及1,012条来源回答，归档分层修复/退步交集与66题正确性变化。三候选修复并集SC3题、Logistika34题，仍不足目标所需7/44题。[路线与结论](docs/cpt_transfer_route_20260915.md)。
- 完成13题37请求证据诊断：Logistika闭卷0/11、短证据10/11、原页9/11；SC算术0/2→1/2。追加两题四请求对照，原页0/2、去章节编号2/2；均独立复核，仅作诊断、不计训练收益。
- 新增原始回答核验、分层审查包与证据对照工具，13项测试通过；冻结下一步广域迁移数据及CPT/SFT配对门槛，官方评分和旧开发划分不变。

## v1.13.85（2026-09-15）

- 完成300道历史错题与S5来源单元的范围核对：四类局部任务只对应已登记的4道Logistika错题、0道SC错题；这不是完整语义覆盖上限，不能据此预测间接迁移。[核对结果](docs/cpt_local_scope_audit_20260915.safe.json)。
- 下一检验转向多个来源主题的新情境迁移与回答形式对照；不重复已完成的22题诊断，不启动另一轮仅四类任务训练。

## v1.13.84（2026-09-15）

- 删除[实验汇总Word报告](reports/物流模型CPT与SFT实验汇总_20260915.docx)22个一级实验标题前重复的“1.”，保留“实验N：名称”，其余内容和排版要求不变。

## v1.13.83（2026-09-15）

- 统一[实验汇总Word报告](reports/物流模型CPT与SFT实验汇总_20260915.docx)正文首行缩进2字符、全文1.5倍行距及零段前段后间距，关闭段落网格对齐。
- 内容不变，26页重新渲染检查通过，保留宋体小四正文、黑体标题与无底色三线表。

## v1.13.82（2026-09-15）

- 修订[实验汇总Word报告](reports/物流模型CPT与SFT实验汇总_20260915.docx)格式：正文宋体小四、标题黑体，26张表格改为无底色三线表。
- 报告内容与实验结果保持不变；重新渲染并检查26页排版，更新文件校验记录。

## v1.13.81（2026-09-15）

- 按实际执行顺序整理22项物流CPT/SFT实验、32组检查点或对照结果，按目的、数据、方法、双评测与已有分析生成[Word报告](reports/物流模型CPT与SFT实验汇总_20260915.docx)。
- 补核服务器原分块及关联分块两臂已完成的结果，明确污染诊断与历史评测口径限制；26页逐页排版检查通过，附来源及核算归档。

## v1.13.80（2026-09-15）

- S5 来源迁移对照完成，三模型66条原始回答及冻结输入独立复核；S5原题5/9、换位6/9、措辞1/4，与Step120各组总分相同。[复核结果](docs/cpt_s5_transfer_result_20260915.safe.json)。
- 双评测3个百分点目标仍未达成，当前没有训练运行；后续重审整体知识覆盖及训练形式，来源诊断不作为独立泛化结论。

## v1.13.79（2026-09-15）

- S5 完成一遍训练及双评测，10032 条原始回答独立复核：SC189/226、Logistika1186/1446，较基线+0/+0.415个百分点，未达标。[结果](docs/cpt_s5_result_20260915.safe.json)。
- 新训练题4→23/32，但9条来源迁移仍5/9；已自动接续冻结22条选项轮换及措辞对照，未晋级模型。

## v1.13.78（2026-09-15）

- S5 协调器已启动：补齐竞争分配规则、流量与成本组合、方向及正负边界；231 条记录、41429 输入 token、77 步，一遍训练后自动固定双评测。[实验登记](docs/cpt_s5_registration_20260915.safe.json)。
- 数据审核及 14 项测试通过；S5 独立检查仅保存模型和随机状态，不支持优化器续训，按实际挂载盘检查容量，保留旧实验及原有完整检查。

## v1.13.77（2026-09-15）

- 来源迁移对照已完成并复核，仅部分题对位置/措辞敏感；核实S4缺少流量比例干扰项、加权训练仅有等流量的覆盖限制。
- 准备32个S5竞争规则对比候选，尚待训练前审核；记录新实验仅保存模型方案的容量依据，不删除旧产物。[准备摘要](docs/cpt_s5_preparation_20260915.safe.json)。

## v1.13.76（2026-09-15）

- 核对S4逐题错误：9题正确性无翻转，训练残错集中于负节省符号和分配方向；启动原模型/S4来源题的选项顺序及措辞对照。[诊断注册](docs/cpt_s4_transfer_diagnostic_20260915.safe.json)。
- 仅来源推理诊断，不改变官方评测、不启动新训练，完成后核验具体迁移差异。

## v1.13.75（2026-09-15）

- S4完成77步及固定双评测，原始评分复核：SC190/226、Logistika1188/1446，较基线+0.44/+0.55个百分点，仍未达标。[结果](docs/cpt_s4_result_20260915.safe.json)。
- 新训练题14→28/32，另一组来源诊断仍5/9，学习与迁移收益存在差距；无新训练运行，未晋级模型。

## v1.13.74（2026-09-15）

- S4协调器已启动：固定32条件任务各3次加135保持任务，231记录/39887输入token、77步、LR5e-7；真实数据审核通过，加载门禁后自动训练及双评测。[注册](docs/cpt_s4_registration_20260915.safe.json)。
- 增加受限S4数据与输出配置，容量检查使用实际数据盘；11项相关测试通过。尚无新训练后收益结论。

## v1.13.73（2026-09-15）

- 核对4项来源探针错误及冻结开发主题，生成32个条件/计算训练候选，保留来源与数值验算；仍待语义近似及训练格式审核。[准备摘要](docs/cpt_s4_training_preparation_20260915.safe.json)。
- 核实项目实际数据盘有559G可用，区分仅196G的容器overlay，为下一最小实验准备独立存储；尚未启动训练。

## v1.13.72（2026-09-15）

- S4第二批9道来源条件/计算题完成原模型探针：5/9，检出净收益、平方根分配和单位空间补货频率等4项错误，原始评分复核通过。[安全结果](docs/cpt_s4b_probe_20260915.safe.json)。
- 尚非训练就绪数据；继续核实冻结开发主题重叠与来源条件，不将单轮小测错误当作官方收益或直接回流训练。

## v1.13.71（2026-09-15）

- 五号机 Qwen3.5-9B 单机 FSDP2 8 + vLLM TP4/DP2 8 完成三步真实 GRPO 更新，退出码 0；修复 IPC 物理/逻辑编号、同步桶与奖励合同，并关闭实测触发主机锁页分配失败的训练侧 CPU offload。
- 交付 [跑通记录与启动方法](技术报告/single_machine_9b_grpo_success_20260915.md)、隔离准备/启动脚本及13项通过测试；8 rank 检查点均可读取，760份/每rank优化器状态 step 均为3，三步梯度非零，NPU已释放。此为工程烟测，不宣称数学泛化收益。

## v1.13.70（2026-09-15）

- S4首批来源探针完成：原Step120为12/12，逐题输入与原始解析复核通过；该批未检出知识缺口，不直接投入新训练。
- 继续寻找更有区分度的来源条件任务，旧探针已结束；双评测3个百分点目标仍未达成。

## v1.13.69（2026-09-15）

- S4首批12道来源情境题完成条件、唯一答案及算术审阅，启动原Step120难度探针；私有题文保留在授权工作区与原训练服务器。[安全摘要](docs/cpt_s4_probe_20260915.safe.json)。
- 尚未启动新训练；字面近似检查不能替代语义污染审核，待原始探针结果后决定下一配方。

## v1.13.68（2026-09-15）

- S4首批操作知识来源审查：完整阅读19段，提取14个有来源与范围限制的知识单元；2个涉及冻结开发主题暂缓，其余12个等待任务及难度审核。[安全摘要](docs/cpt_s4_source_audit_20260915.safe.json)。
- 私有来源不入Git；尚未训练就绪或启动新训练，保留上下文、题目近似检查及来源探针门禁。

## v1.13.67（2026-09-15）

- 按原脚本在五号机实际复现 9B 单机 8+8 GRPO 同步停滞：实时堆栈和仅修正 IPC 地址的同次运行干预，确认接收端物理 chip 8–15 与发送端逻辑 rank 0–7 错配。
- 干预后进一步复现 FP32 embedding 3880 MiB 超过 2560 MiB 同步桶的断言，交付 [复现报告](技术报告/single_machine_9b_grpo_reproduction_20260915.md)；临时别名已清理、NPU 已释放，未修改共享训练源码，未宣称跑通训练。

## v1.13.66（2026-09-15）

- LoRA-CPT来源探针完成并复核：157→159/212，改对3、改错1；固定双评测仍未达3个百分点目标。[探针结果](docs/cpt_lora_source_probe_20260915.safe.json)。
- 明确旧训练与探针均已结束，下一步恢复S4操作知识来源审核及难度诊断，尚未启动新训练。

## v1.13.65（2026-09-14）

- LoRA-CPT完整614步及固定双评测完成并复核：SC189→190/226、Logistika1180→1184/1446，仍未达标；16rank基础权重不变、适配器更新，1872924真实token和HF完整性检查通过。[结果](docs/cpt_lora_result_20260914.safe.json)。
- 同语料全参数CPT为189/1185，LoRA未显示明显优势；已启动212题来源探针，区分材料知识学习与评测迁移，暂不盲目追加遍数。

## v1.13.64（2026-09-14）

- 修复LoRA流水线推理8卡可见性泄漏到16卡训练，按阶段隔离设备环境，并在训练启动器清除可见性限制；新增切换回归测试。
- 原正式运行在初始化退出、零训练步/无检查点；复用通过的两步/导出/推理门禁，在新快照与目录恢复614步训练，保留失败记录，尚无正式评测分数。

## v1.13.63（2026-09-14）

- LoRA两步模型CPU合并导出验证通过：160个语言层适配模块，FP32增量合并后转BF16，1199张量/15分片完整，无缺失或形状类型差异。[门禁报告](docs/cpt_lora_gate_20260914.safe.json)。
- 协调器已进入实际推理烟测，通过后自动接续正式614步单遍CPT及双评测；两步工程门禁模型不用于宣称正式收益。

## v1.13.62（2026-09-14）

- LoRA-CPT两步实机训练及保存通过：16rank基础权重哈希不变、适配器均更新，实际5808输入token；关闭不兼容冻结参数的融合梯度累积路径，保留早期失败快照。
- 启动自动接续协调器，正在验证适配器CPU合并导出；导出与推理门禁通过后，从全新适配器执行同语料614步一遍训练和固定双评测，尚无正式LoRA效果结论。

## v1.13.61（2026-09-14）

- 按用户授权开始 LoRA-CPT 对照，冻结原 Step120 与此前同一份 4912 条正文语料；新增两步实机门禁、进程内 Bridge 兼容与基础权重/适配器哈希审计。
- 数据真实分词检查通过；rank64/alpha128 的分布式兼容门禁仍在验证，CPU适配器合并导出脚本尚待实测，正式614步训练尚未启动。保留全部失败快照，自动跟进优先转向LoRA。

## v1.13.60（2026-09-14）

- 收窄工程报告单机 9B 结论：现有失败脚本与日志仅属于 9 月 13 日复测，不能据此否定负责人确认的此前通过记录；成功运行的原始归档尚待定位。
- 扩查 5 号机 veRL 配置快照、旧 slime 工作区与容器变更；已找到的早期 9B 1,000 步成功记录属于 SFT，未混写为 GRPO 证据。

## v1.13.59（2026-09-14）

- S3完成133步及固定双评测，原始输出独立复核：SC189→191/226、Logistika1180→1179/1446，仍未达标；来源探针157→204/212，同模板未见条件2→10/11，来源组开发21→21/22。[结果](docs/cpt_sft_s3_result_20260914.safe.json)。
- 开始S4来源覆盖审查：从既有教材抽取操作主题候选段落，尚非训练就绪数据，下一步核对来源范围、题库污染、开发组隔离及任务难度；不重复启动S3。

## v1.13.58（2026-09-14）

- 将工程复现报告第四章 27B 部分改为“原始模型 → Step100 → Step120”的完整主线，补齐前 100 步的启动、参数、数据、检查点与验证。
- 只读核验 5 号机第一阶段日志和产物，区分 train237/236、基础奖励/dense30 及两阶段保存策略，保留早期日志步数口径差异。

## v1.13.57（2026-09-14）

- 按第三章的端到端结构补齐[工程复现报告第四章](技术报告/sources/engineering_report_20260913_工程复现报告.md)：单机 9B 与双机 27B 分别覆盖环境、数据、奖励、配置、启动、验证和故障定位，补充 27B 恢复与 HF 导出步骤。
- 再次只读核验 5 号机实际脚本、Step120 日志、数据契约、检查点及 HF 清单；保留 9B 尚未完成闭环的真实状态，明确 27B 续训覆盖参数与优化器重置，不启动训练。

## v1.13.56（2026-09-14）

- S3 条件辨析 SFT 已启动：135 条审核任务加 44 条条件变体各 6 次曝光，共 399 条、133 步，原 Step120 起点、LR5e-7；真实分词/监督预算及训练器加载检查通过，训练前 212 题探针完成。[冻结配方](docs/cpt_s3_registration_20260914.safe.json)。
- 自动接续模型导出、训练后探针及固定双评测；目前进入训练初始化，尚无新双评测结果。此轮同时改变数据、曝光与学习率，不能作单因素归因。

## v1.13.55（2026-09-14）

- 来源题难度探针完成：原Step120为144/157（训练123/135、开发21/22），逐条输入、答案解析及评分复核通过；13道错误再次对照来源确认，下一步聚焦分类例外与条件组合。[探针结果](docs/cpt_source_probe_20260914.safe.json)。
- 多数新增定义题已经掌握，尚未启动S3，不将题量或同模型审核通过率当作提升证据；目标仍未达成。

## v1.13.54（2026-09-14）

- 来源制题/同模型盲审完成：196候选、194筛选通过；逐题来源复核后暂缓37条答案歧义、范围错配或同条件重复，保留157条做学习价值探针，尚未训练。[审核摘要](docs/cpt_source_task_review_20260914.safe.json)。
- 训练/开发拟分135/22，保持来源主题组互斥并重新排列选项；已启动原Step120闭卷探针，先验证是否存在值得学习的知识缺口，不将同模型自评当作事实认证。

## v1.13.53（2026-09-14）

- S1低LR对照完成并独立复核：训练363→373/402、开发65→65/74，SC189→190/226、Logistika1180→1169/1446；两档LR均未达标，实际加载顺序报告及逐步token量一致。[S1结果](docs/cpt_sft_s1_result_20260914.safe.json)。
- 启动205份来源知识的本地条件辨析制题与盲审，按共享来源/主题合并29组，生成与审核均保留记录；筛选结果仍需语义及事实复核，不自动开训、不调用付费API。

## v1.13.52（2026-09-14）

- S2单遍SFT结果已复核：训练客观题363→393/402、开发65→68/74，但SC189→186/226、Logistika1180→1175/1446，未达标、不晋级。[安全结果](docs/cpt_sft_s2_result_20260914.safe.json)。
- 独立重解析基线与候选共10,032条输出，协议一致、零无效/截断；启动同数据同起点同197步的S1（LR2e-7）对照，独立目录及快照保留全部失败证据。

## v1.13.51（2026-09-14）

- 再次只读核验五号机周末 9B 单机 GRPO 失败，补充三次运行一致的 AllGather 等待 rank 0、EI0002 / 1836 秒超时及随后 507001 的证据；设备 plog 确认训练组 0–7、权重接收端 8–15，且脚本超时值未在所查 worker 生效。
- 更新 [单机核验报告](技术报告/single_machine_9b_grpo_report_20260914.md)，区分已证通信超时与尚未定位的 rank 0 阻塞起因；现场 NPU 全部健康且无计算进程，CPU 合成调用再次复现奖励接口错误，未启动训练或修改服务器环境。

## v1.13.50（2026-09-14）

- 按现有技术报告格式交付 [9B 单机多卡核验报告](技术报告/single_machine_9b_grpo_report_20260914.md)与 [27B 多机多卡核验报告](技术报告/multi_machine_27b_grpo_report_20260914.md)，分别覆盖单机 8+8 与 5/6 号机各 16 chip 的角色分离方案。
- 5 号机只读核实 9B 权重同步失败、数据/奖励接口不匹配（合成答案复现 TypeError），确认 27B 历史 Step120 日志、参数同步和检查点记录；区分配置建议、工程完成与能力收益，未启动训练。

## v1.13.49（2026-09-14）

- S2已完成197步及模型导出，实际88,819训练token完整；导出1199张量、15分片检查通过。
- 修复协调器自身未加载快照包根目录的导入错误，新增仅恢复训练后审计/评测入口，保留旧失败记录；真实容器导入与训练预算复核通过，恢复进程已启动，未重复训练，暂无新双评测结论。

## v1.13.48（2026-09-14）

- 按用户“最快方案”要求先执行S2：复用591条审核任务，从原Step120直接单遍SFT，LR1e-6、batch3、197步；S1保留作后续对照。
- 服务器连接已恢复，真实分词/监督范围及安装训练器完整加载门禁通过；独立协调器已启动训练前476题探针，自动接续SFT、导出及固定双评测，尚无新训练结论。[执行记录](docs/cpt_sft_goal_20260912.md)。

## v1.13.47（2026-09-12）

- 增加S1/S2只读数据预检：固定文件身份、实际模型重新分词、答案/EOS监督范围、完整批次和token预算核对；9项定向单元测试通过。
- 预检工具尚未在训练容器运行，不替代分布式采样与并行兼容性验证。复查训练服务器SSH仍被重置，新训练未启动。

## v1.13.46（2026-09-12）

- 建立CPT/SFT自主假设检验目标：固定Step120与评分协议，SC至少196/226、Logistika至少1224/1446；注册直接Step120的同数据学习率对照。[执行账本](docs/cpt_sft_goal_20260912.md)。
- 整理205份带来源制题请求、115个证据组，5项达标规则测试通过；请求未冒充审核QA。三台服务器SSH握手重置，尚未启动新训练，目标未达成。

## v1.13.45（2026-09-11）

- 新知识语料单遍CPT与双评测全部完成：SC189→189/226，Logistika1180→1185/1446；合计改对14、改错9，净增5题。[最终结果](docs/cpt_knowledge_complete_result_20260911.safe.json)。
- 独立核实614步、完整输入token预算、检查点与HF导出，并重新解析全部5,016条新评测输出；零无效零截断。llin模型保留，不追加训练，完成后暂停自动跟进。

## v1.13.44（2026-09-11）

- 按用户要求启动新知识语料单遍CPT：全部13份材料加205条核对知识，4,912条，batch8、614步；自动接续两个评测集，模型以llin命名。[本轮执行记录](docs/cpt_knowledge_complete_one_epoch_20260911.md)。
- 真实训练容器全量数据门禁、5项定向测试和独立快照检查通过；每30分钟自动跟进已切换至本轮。尚无本轮训练完成或评测结果，私有材料不入库。

## v1.13.43（2026-09-11）

- 将核对后的补充知识从110条扩至205条，合并全部13份原材料，交付4,912条、1,868,012正文token的本地CPT包。[交付范围与验证](docs/cpt_complete_package_20260911.md)。
- 新增可复现合并构建与CPU读取检查，10项定向测试通过；保留300题处置索引与82题问题登记，不宣称全部错题知识已解决。未启动训练，私有材料不入库。

## v1.13.42（2026-09-11）

- 整理110条已选定核对的CPT核心补充知识，9,290正文token，保留术语、排除条件与公式；51个主题部分覆盖、11个主题仍有缺口。[核心补充说明](docs/cpt_reviewed_core_20260911.md)。
- 新增审核清单驱动构建和5项定向测试，交付本地JSONL/Parquet与候选处置表；未修改旧语料、未启动训练，私有材料不入库。

## v1.13.41（2026-09-11）

- 整理300道错题为62个知识主题、81条关系与条件，汇集13份教材402个候选页面/成员、127个不同百科版本及23条权威来源说明。[知识材料交付说明](docs/cpt_knowledge_expansion_20260911.md)。
- 交付本地知识文档与逐题追溯表，登记82题的冲突/缺图/语境事项；候选未宣称训练就绪，未改旧语料或启动训练。私有题目和教材内容不入库。

## v1.13.40（2026-09-11）

- 完成CPT教材保留审计：300道仍错题生成来源候选，9例对照原书与实际训练正文，确认7例存在标题、短定义或公式步骤删除，2例支持文本保留仍错；样本不代表总体比例。[核对报告](docs/cpt_textbook_coverage_20260911.md)。
- 核实31,637个保留块均进入训练正文，交付只读检索工具和安全统计；私有题目/教材摘录不入库，本次未改语料或启动训练。

## v1.13.39（2026-09-11）

- 全部13份材料单遍CPT与双评测完成：SC189→189/226，Logistika1180→1183/1446，合计改对10、改错7，净+3题。该结果为包含评测来源教材的学习实验。[完整结果](docs/cpt_logists_full_result_20260911.safe.json)。
- 独立复核523步、全部语料预算、完整检查点/导出及5,016次新评测请求，零无效零截断；llin模型保留，自动跟进已暂停。

## v1.13.38（2026-09-11）

- 按用户纠正纳入全部13份教材，取消评测同源/文字重合排除及开发集留出，交付4,707条、1,844,881正文token；正常质量清洗与去重保留。117条评测指纹匹配仅记录、不排除。[完整材料执行账本](docs/cpt_logists_full_one_epoch_20260911.md)。
- 已启动Step120起点、batch9、523步单遍CPT，自动接续两个评测集；11项测试、全量语料检查及实际训练加载门禁通过，自动跟进已更新。尚无本轮完成或评测结果。

## v1.13.37（2026-09-10）

- 物流教材单遍CPT与双评测全部完成；独立复核5,016次请求，零无效零截断。SC189→189/226，Logistika1180→1180/1446，合计改对9、改错9，无净准确率收益。[完整结果](docs/cpt_logists_result_20260910.safe.json)。
- 完整377步训练、检查点及1,199张量HF导出再次核验通过；llin模型保留，不替换Step120，自动跟进已暂停。

## v1.13.36（2026-09-10）

- 物流教材单遍377步训练及1,183,197输入token审核完成，完整检查点门禁通过；尚无双评测结果。
- 修复导出子进程误用系统版Megatron Bridge的问题，独立快照仅接续导出与评测，保留旧文件、不重复训练；4项测试及实际模型Bridge识别检查通过。[最新账本](docs/cpt_logists_one_epoch_20260910.md)。

## v1.13.35（2026-09-10）

- 按用户“一遍一测”要求启动新物流教材 CPT：Step120 起点、3,016 条、batch8、377 步一遍，随后自动导出 llin 命名模型并运行两套评测各3轮。[本轮执行账本](docs/cpt_logists_one_epoch_20260910.md)。
- 3项预算测试及远端语法/编译检查通过；真实3,016条分词门禁通过，训练已初始化，自动跟进已接续。尚无新训练完成或评测结果。

## v1.13.34（2026-09-10）

- 清洗用户补充的物流教材，7 本共 2,189 页使用 MinerU 解析；交付 3,016 条训练样本、1,180,181 正文 token 和 161 条开发验证样本，隔离 4 份评测同源资料。[语料交付与接入说明](docs/cpt_logists_release_20260910.md)。
- 9 项测试、全量来源/分词/去重/评测隔离检查、逐字节复现及真实训练容器 CPU 加载验证通过；已授权上传服务器，尚未启动新 CPT。正文与原书不提交 Git。

## v1.13.33（2026-09-10）

- 新ERP单遍CPT及双评测全部完成；5,016请求独立复核、零无效零截断。SC189→187/226，Logistika1180→1178/1446，合计净−4题，未观察到收益。[完整结果](docs/cpt_erp_result_20260910.safe.json)。
- llin命名模型已导出保留，不替换Step120；自动跟进已暂停，旧实验剩余验证保持取消。

## v1.13.32（2026-09-10）

- 新ERP语料单遍47步完成，32,515输入token及完整检查点核验通过；修复训练后检查快照缺失依赖造成的中断。
- 新增仅接续导出/评测入口并以独立code_v2恢复，不重复训练；3项测试通过，双评测尚未出结果。

## v1.13.31（2026-09-10）

- 按用户最新要求停止旧实验剩余验证，取消旧等待队列并立即启动新语料单遍CPT；模型保存名固定为llin-step120-erp-cpt-1epoch-20260910。
- 新进程已进入training并通过47条真实分词门禁；自动跟进仅保留新CPT及双评测。3项定向测试及远端编译/语法检查通过，尚无新评测成绩。

## v1.13.30（2026-09-10）

- 按用户要求部署新ERP语料单遍CPT及双评测流水线：Step120起点，47篇/batch1/47步，随后SC-bench知识与LogistikaBench各3轮；[执行账本](docs/cpt_erp_one_epoch_20260910.md)记录冻结配置和资源队列。
- 3项预算测试及远端编译/语法检查通过；独立协调进程已排队等待旧实验确认流程释放资源，自动跟进已接续。实际训练与评测尚未开始，不报告新成绩。

## v1.13.29（2026-09-10）

- 将470份候选材料分层清洗，交付可追溯的ERP流程CPT种子语料：训练47篇32,468 token、验证3篇3,515 token；其余材料保留许可/版面/评测同源复核状态。[语料说明与接入方式](docs/cpt_corpus_release_20260910.md)。
- 新增可重复构建、固定文档族划分、增量比较和独立验证；8项测试、完整文件复现和实际训练容器CPU加载检查通过。正文留在本地及训练容器，未启动新训练或变更现有两臂实验。

## v1.13.28（2026-09-10）

- 按用户要求调查评测知识结构，重新核算303道错题的类别与质量筛查，归档470个文件（113份PDF、5,357页，另含网页与目录）；[调查报告与索引入口](docs/cpt_material_survey_20260910.md)记录实际下载、PDF可读性、来源及许可限制。
- 新增可复用收集和PDF文本检查程序；原始正文留在本地CPT_resources，Git仅保存安全元数据。标记评测同源材料与MIT非商业AI训练限制，未将候选材料直接加入训练，原两臂实验继续按冻结协议运行。

## v1.13.27（2026-09-10）

- 完成0–8遍主曲线及45,144请求审核；合计最高点为第1遍，比Step120净多3题，尚无稳定收益结论。[完整安全结果](docs/cpt_main_curve_20260910.safe.json)保留所有曲线点。
- 第二阶段原分块臂116步完成，每步token均匹配冻结计划；已排队单步完整恢复重放、第0/1/2遍新进程复测及四模型通用回归，5项恢复相关测试通过。Agent回归及最终验证仍未完成。

## v1.13.26（2026-09-10）

- 用真实检查点复现并修正整遍边界的数据加载恢复问题：只在内存标记该遍结束，下一遍29批及顺序与新建采样器一致，原文件保持不变；3项测试通过。新增单步完整恢复探针，NPU数值重放尚待执行。
- 完成隔离Agent 20题×3次及通用314题回归预检，尚未启动回归推理。第一阶段正在评测第7遍，第二阶段继续等待完整曲线；[执行账本](docs/cpt_two_experiments_20260910.md)记录剩余验证。

## v1.13.25（2026-09-10）

- 冻结第二阶段两臂的长度配对采样，116次更新的监督token差均≤0.551%、每遍完整覆盖；实际NPU因果注意力验证段内可见、跨样本隔离，11项相关测试通过。
- 已将原分块/关联分块各4遍训练及全量评测排队，等待第一阶段主曲线完成后自动启动；新增独立存储与[冻结协议](docs/cpt_paired_stage2_protocol_20260910.safe.json)。逐遍评测、峰值复测和最终回归仍未完成，不宣称实验收益或交付通过。

## v1.13.24（2026-09-10）

- 核验8遍CPT全部232步、预算及完整检查点，修复评测推理库路径并恢复逐遍评测，基线5016请求已完成。
- 完成跨章关联语料v2：4685段落完整覆盖、116条记录、监督token差0.51%，并通过实际TP4/CP2的CPU序列打包契约检查；10项相关测试通过。[账本与限制](docs/cpt_two_experiments_20260910.md)明确逐遍评测、每步预算、NPU注意力与第二阶段训练仍待完成。

## v1.13.23（2026-09-10）

- 根据首个383GB完整检查点的真实清单，修正随机数/调度器状态门禁，补齐测试；新增仅接续现有训练的衔接入口，保留运行代码副本及训练进程。

## v1.13.22（2026-09-10）

- 启动原始Step120的固定8遍CPT曲线，逐遍保留模型/优化器/调度器/RNG与数据加载状态，接续0–8遍两套全量基准三轮评测；新增专用存储、输入输出对应与完整状态门禁，3项定向测试通过。
- 冻结[两阶段实验协议](docs/cpt_two_experiments_20260910.md)，第二阶段采用共同Step120起点的原分块/跨章节关联分块等训练量对照。第一阶段运行中，第二阶段和最终回归尚未完成。

## v1.13.21（2026-09-10）

- 完成全量提示对照复核：总计+16题，但SC知识−1题，输出token达72.39倍，不晋级为交付方案。
- 新增只读逐条评分/退步类型核验，74道退步中61道为有效错误多数答案；保存[完整结果与限制](docs/supply_chain_full_prompt_result_20260910.md)，不将截断修复或额外训练视为已证实解法。

## v1.13.20（2026-09-09）

- 冻结并启动全量1672题原协议/核查方案同期三轮对照；两组TP8并行、总并发64，保留逐题得失、截断与输出成本，不训练、不以部分分数替代交付。
- 新增3项分片与评分测试；[全量验证协议](docs/supply_chain_full_prompt_comparison_20260909.md)明确候选晋级和原能力回归边界。

## v1.13.19（2026-09-09）

- 重算新SFT逐题多数票与稳定配对：相对纯CPT改对12、改错14；确认25道匹配训练题原本全对，停止追加训练和扩量。
- 复用既有96题开展无训练的指引×输出预算四条件诊断，拆分旧实验混杂；[整体复审与协议](docs/supply_chain_rethink_20260909.md)保留Agent回归未完成及原因未识别的边界。

## v1.13.18（2026-09-09）

- 完成教材引用恢复及任务型QA独立审核，最终冻结591训练/105开发题；补齐隔离Agent环境与314题通用回归集，11项数据准备测试通过。
- 已完成Step120/CPT的新任务预答，正式启动单遍73步CPT后SFT，并接上自动领域/任务/通用验证。见[执行账本](docs/supply_chain_global_rebuild_20260909.md)，训练及完整收益验证尚未完成，不宣称目标达成。

## v1.13.17（2026-09-09）

- 完成44章知识单元提取/独立审核，88次百炼调用，通过127单元；识别引用跨窗口和审核缺项问题，保留原始失败证据，未启动训练。
- 新增8项引用结构测试；核验Agent训练/验证/测试指令哈希隔离，定位回归sandbox缺schema文档。恢复动作和预算边界见[执行账本](docs/supply_chain_global_rebuild_20260909.md)，后台继续推进。

## v1.13.16（2026-09-09）

- 开始用户授权的整体能力重构，完成115教材块、436候选与实际355题的全量覆盖盘点及1672题匿名错误分层；137次百炼调用，6项分类结构测试通过。
- 保存[持续执行账本](docs/supply_chain_global_rebuild_20260909.md)、安全证据与核算notebook，安排完成后通知；新训练与完整回归尚未启动，不将覆盖标签或旧同配置净增1题当作收益证明。

## v1.13.15（2026-09-09）

- 完成能力型SFT的训练/导出结果核查：历史对照1371→1373，7题改对、5题改错；三轮稳定子集仅1题稳定改对，尚无明确增益证据。
- 核实历史评测输出长度元数据不一致，启动同配置双模型三轮补测；新增重复结果核查和可选汇总参数溯源。纠正Step120原生检查点未定位的旧记录；Agent/通用回归与直接SFT对照仍未完成。见[结果与限制](docs/book_capability_sft_20260909.md)。

## v1.13.14（2026-09-09）

- 批量整理365道能力型训练候选，保留355道最小答案QA、隔离10道，题面与71道开发候选不改；完成答案/EOS监督制数及9项相关测试。
- 提交纯教材CPT起点的2e-7、单遍44步SFT试验，结束后自动导出与领域三轮评测；保存[试验记录](docs/book_capability_sft_20260909.md)。Step120对照臂和Agent/通用回归未完成，不宣称训练收益或交付通过。

## v1.13.13（2026-09-09）

- 完成冻结40题的Step120/纯教材CPT共160次预答及80次匿名评分，35题分数一致、5题未解决；原始汇总相同，但正文复核发现评分偏教材细节和一处真实算术错误。
- 保存[能力型QA预答与扩量决策](docs/book_capability_probe_20260909.md)、安全结果和核算notebook；建议先批量整理数百道候选并小规模SFT验证，再决定扩到2000–3000道，本轮未训练、不更改旧候选池。

## v1.13.12（2026-09-08）

- 完成全书107来源的能力型QA批量构造、题面与来源审核、重复检测及分层抽检；最终组织436道候选（训练365、开发71），覆盖44章，不再围绕少量旧题反复修订。
- 保存[批量数据组织记录](docs/book_capability_batch_20260908.md)、安全证据和核对notebook，冻结40道后续预答样本；本轮未预答目标模型、未扩到2000–3000道、未启动SFT，明确候选不等于已证实有训练价值。

## v1.13.11（2026-09-08）

- 完成6道教材修订候选的Step120/纯CPT共20次预答及12次匿名评分；资料题不测闭卷，对应输入一致、全部回答正常结束。
- 保存[测试及评分有效性复核](docs/book_repair_value_20260908.md)、安全结果和复核notebook：两模型无明确差异，发现教材细节偏好及部分评分理由不可靠，不将原始扣分直接解释为知识缺失，未训练或晋级题组。

## v1.13.10（2026-09-08）

- 使用5号机百炼修订17道教材自建资料依赖题，新增仅看题面审查、条件与引用审核；API通过12道，经当前任务内容复核继续隔离6道，保留4道闭卷与2道带资料修订候选。
- 保存[修订与复核记录](docs/book_question_repair_20260908.md)、安全证据和身份校验；旧数据池及分组不变，不继承旧预答分数，未启动训练。

## v1.13.09（2026-09-08）

- 完成教材204题的Step120/纯CPT双起点预答（816次）与双轮匿名评分，补评引用格式失败项并保留旧记录；发现17道闭卷题仍依赖缺失资料，新增拦截并整组隔离。
- 保存[学习价值筛选与质量修正](docs/book_task_aligned_value_20260908.md)、安全证据和复核notebook；最终为学习候选8组32题、开发8组32题、隔离35组140题，未启动训练，不把开卷改善等同于知识缺失证明。

## v1.13.08（2026-09-08）

- 新增[CPT 语料分块、知识关联与重复曝光技术报告](docs/cpt_chunking_knowledge_structure_report_20260908.md)，复核固定无重叠切块、next-token 监督及四遍曝光，解释上下文缺失、共享参数知识组合与过拟合的区别。
- 给出变化边界、重叠前缀和关联文本的分阶段对照方案，明确 token 预算、分层评测及证据边界；本次仅新增文档，未修改训练程序或启动训练。

## v1.13.07（2026-09-08）

- 完成教材四任务数据首批构造：百炼qwen3.8-max生成、隐藏答案独立求答、来源审核及语义去重，保留51组204题（训练候选172、开发32），未预答目标模型、未训练。
- 保存[构造与审核记录](docs/book_task_aligned_20260908.md)、安全汇总、报告和复核notebook；记录初次失败与两项契约修正，复用12组预检避免重复生成，不将教材或QA正文提交仓库。

## v1.13.06（2026-09-08）

- 完成新顺序的无训练诊断：96题、Step120与纯教材CPT、三个条件各三次重复，另对3道来源支持题配对复测，共1,764次本地生成；未调用百炼、未训练。
- 保存[结果与复现说明](docs/logistics_strategy_diagnostic_20260908.md)、安全汇总、复核notebook及报告。发现候选顺序敏感、核查提示有收益也有代价，旧教材检索不足以支持知识/推理二分；下一轮优先独立来源的概念辨析与证据使用SFT，并比较两个训练起点。

## v1.13.05（2026-09-08）

- 完成[领域适配路线重审与下一实验协议](docs/logistics_strategy_reset_20260908.md)：核对官方评测范围、269项候选表、主题分配及诊断证据缺口，不再把最小答案修正作为默认主线。
- 增加安全汇总复查notebook，明确Step120与纯教材CPT均须保留为对照；下一步优先做不更新权重的任务形式/证据干预诊断。本轮未生成训练数据、未调用百炼、未训练。

## v1.13.04（2026-09-08）

- 实施下一轮启动前门禁：单答案冻结标准重复评分51/56一致（91.1%），无效项计入分母；80题成对构造44题通过来源审核，但监督token差42.6%，未过5%预算匹配，不启动训练。
- 核对旧开源80题源文件及答案一致性（76直接一致、4待复核），Agent16条中12条有历史警告，回放未就绪。保存[门禁实测](docs/sft_next_preflight_20260908.md)、安全汇总、代码和notebook，明确最小修改尚未确认，不改写失败标准。

## v1.13.03（2026-09-08）

- 完成119题三模型固定参考答案NLL诊断，内容损失与EOS分列，输入token哈希一致；56次位置互换教材审核全部格式有效，28对中7对语义判断不稳定。不新增训练。
- 保存[下一轮证据论证与有条件协议](docs/next_sft_evidence_and_protocol_20260908.md)：推荐先稳定语义评价，再做最小语义修正答案对照和回放保护；明确门禁、成败与不能保证达标的边界，附来源论文、安全汇总及复查notebook。

## v1.13.02（2026-09-08）

- 复核7道教材下降案例，区分明确矛盾、对比遗漏及完整性/判分争议；80条真实训练记录逐token重建通过，10步日志与15,491总序列tokens、7,183监督tokens对齐，同批闭卷回答明显缩短。
- 补测64道真实训练候选，两个检查点四条件推理后自动接百炼匿名评分，训练内机会题单列，不混为开发或维护。保存[后续审计](docs/targeted_book_pilot_followup_20260908.md)、安全证据和notebook，不新增训练或覆盖原始分数。

## v1.13.01（2026-09-08）

- 补齐纯CPT与定向SFT第5/10步教材答案同轮匿名评分：55题50有效，5题引用ID无效隔离；共同34开发题闭卷双问法通过7/7/3，16训练内维护13/13/10。没有新增训练或宣称收益。
- 保存[结果与局限](docs/targeted_book_pilot_result_20260908.md)、安全汇总、协议、评分器及复查notebook；校验题面/来源/回答哈希和共同分母，私有正文留5号机。

## v1.13.00（2026-09-08）

- 冻结并准备定向教材SFT短程试验：64候选+16维护，另3候选保留，39开发不训练；7,183监督tokens，一遍10步，学习率5e-7，保留5/10步。新增来源/切分/哈希检查、训练启动器和显式模型身份预答。
- 保存[试验协议](docs/targeted_book_pilot_20260908.md)与安全准备汇总；教材生成评测覆盖完整39开发题，官方集仅检查固定最终步。推理完成不等于评分完成，不宣称收益或独立Agent/通用回归安全。

## v1.12.99（2026-09-08）

- 修复教材概念组的主题循环/开发切分耦合，新增19/21章开发来源并让同章旧候选退出训练侧；完成96组补充构造、105问420次CPT预答与盲评分，截断及无效评分按规则隔离。
- 两批72概念组完成全局语义筛查，保留59组；合并与整组质量门禁后组织67问候选机会、16问维护、39问开发、55问隔离，开发四主题已覆盖，尚未训练。保存[补充与合并记录](docs/targeted_book_supplement_20260908.md)、安全汇总及检查代码，保留失败响应，不提交私有正文。

## v1.12.98（2026-09-08）

- 完成需求驱动教材QA首批120概念组尝试，保留37组111问；纯教材CPT完成两个问法的闭卷/带证据共444次预答，全部自然结束，百炼盲评分100问有效。
- 按质量与目标模型表现组织63问候选学习机会、14问维护、10问开发、24问隔离池；仍缺物料搬运及开发覆盖、语义去重和训练预算验收，未启动训练。修复百炼JSON提示约束并加入单请求HTTP预检，保留失败批次；新增脚本、测试和[安全检查记录](docs/targeted_book_groups_20260908.md)，不提交私有教材或QA。

## v1.12.97（2026-09-07）

- 按纯教材CPT的301道错题重新建立需求视角，完成旧363题的CPT三轮复测，覆盖其中298道错题；闭卷8/298、带教材50/298，基础探针57/63→62/63。旧审核缺口保持不变，不将Step120诊断直接套给CPT，也不把基础题高分当知识齐备证明。
- 完成旧2000条QA题型/开发评分审计，新增[需求驱动SFT组织协议](docs/targeted_sft_rebuild_20260907.md)与保守学习机会筛选器，区分来源质量、目标模型需求和训练后实际收益。当前新版数据与训练未完成，15项相关测试通过；私有诊断题不得进入训练或Git。

## v1.12.96（2026-09-07）

- 完成教材 QA SFT 回退诊断：同一 186 道开发题双模型闭卷推理及百炼匿名配对复核，183 对有效，自动评分满分 41→78；正式评测净少答对 26 道（21 改对、47 改错），区分教材收益与评测迁移。
- 补齐参考答案损失、截断控制、分类分解及训练/导出结构检查，新增安全汇总、可复现脚本、核验测试与[诊断来源记录](docs/book_sft_diagnosis_20260907.notes.md)。本轮未新增训练，不提交教材、私有问答或凭据，不把自动裁判当作人工金标。

## v1.12.95（2026-09-07）

- 精简Step120客户报告：将CPT整体准确率、书籍CPT比较及SFT起点纳入正文，删除逐题变化、训练损失等内部细节，附录仅保留来源索引。
- 新增正文整合版Word并同步可检索正文，保留五段优化主线、三模型对照及未出结果空栏，完成8页视觉检查与格式核验。

## v1.12.94（2026-09-07）

- 修订Step120报告的证据与实验承接：明确条件对照的支持范围和未决问题，补齐教材辅助到CPT、CPT到问答SFT的验证逻辑，区分诊断CPT与书籍训练起点。
- 保留原12张对比表，新增独立题集及能力回归空白结果表，同步Word与可检索正文，完成12页视觉检查及格式核验。

## v1.12.93（2026-09-07）

- 按客户要求重组物流知识报告主线：背景、根因探测、评测语料CPT、书籍问答SFT、未做SFT模型对照；补充实验及训练细节移至附录，保留全部12张对比表。
- 另存主线修订版Word，同步可检索正文；明确SFT增量与总收益的比较对象，保留知识缺失判断边界和结果空栏，完成12页排版检查。

## v1.12.92（2026-09-07）

- 将Step120客户报告中的训练前后、候选模型与诊断条件对比统一表格化，删除内部假设的3个百分点目标。
- 按用户指定的SFT完成后报告口径调整叙述，新增三模型成绩表与结论空栏，未填造SFT结果；同步Word与可检索正文，保留既定字体行距，完成12页视觉检查。

## v1.12.91（2026-09-07）

- 按客户报告口径精炼Step120物流知识实验材料，开篇明确验证目标与优化路径，压缩重复解释和内部讨论措辞，保留实验数据、证据边界及SFT三模型验证设计。
- 同步Word与可检索正文，保留指定字体、字号、缩进、黑色文字和1.5倍行距，完成9页视觉检查与表格数据核验。

## v1.12.90（2026-09-07）

- 调整Step120阶段报告排版：正文首行缩进2字符、标题不缩进，全部文字统一黑色，表头改为浅灰底；保留宋体/Times New Roman小三正文及1.5倍行距。
- 整理附录长文件名和段间距，完成10页视觉检查，核验正文及表格内容未改变。

## v1.12.89（2026-09-07）

- 将Step120阶段报告重组为问题发现、CPT尝试、无训练根因诊断及书籍SFT验证的递进路线，明确阶段性证据与待验证效果。
- 按用户决定，将书籍CPT四遍第116步候选（1371/1672，81.998%）确定为后续SFT起点，设计原始Step120、最佳书籍CPT、CPT后SFT三模型对照；分别解释增量收益、总收益及无法证明CPT必要性的边界，结果继续待填，未启动训练。
- 同步Word与可检索正文，核验候选身份、历史分数与指定字体行距，完成10页视觉检查。

## v1.12.88（2026-09-07）

- 更正Step120阶段报告中实验三的含义：改为原始Step120无训练错题诊断，集中呈现303错题与60正确对照、四条件结果、八题复核、四题补证和12问闭卷探针，删除重复基线表。
- 第二部分改为探测方法与优化思路；同题答案SFT移至方案一补充诊断，方案二继续预留。同步Word与可检索正文，保留宋体/Times New Roman小三及1.5倍行距，章节与格式检查及9页视觉检查通过。

## v1.12.87（2026-09-07）

- 按用户纠正后的生产目标启动百炼qwen3.8-max批量教材QA：每批500、并发64，审核有问题即丢弃，跨批去重并在服务器内筛查评测词面重合，目标2,000条，不再修题或反复裁决。
- 接入纯教材CPT第116步后的答案监督SFT与两评测集三轮评测；按4个教材章节留开发集，训练1遍、学习率1e-6。原容器根盘可用约2.2TB，输出放专用/opt目录，不占满/data3、不删除旧模型。
- 已完成2,500候选并保留2,000（训练1,814、开发186），达到目标后停止API生产。修复制数的BatchEncoding兼容问题并保留原失败记录，恢复任务进入16设备训练初始化；14项相关测试通过，不宣称训练或评测完成。见[批量生产与训练记录](docs/book_sft_production_2000_20260907.md)。

## v1.12.86（2026-09-07）

- 按指定规范调整Step120实验Word报告：中文正文宋体、英文数字Times New Roman，正文及表格小三15磅，标题黑体分24/20/16磅，统一1.5倍行距。
- 重排为8页，修正孤立表头、长文件名及引用换行；核验全部正文和表格内容一致，字体与行距属性检查及8页视觉检查通过，方案二预留不变。

## v1.12.85（2026-09-07）

- 完成修订版40条中四类各2条的原文对照代理抽查：3条保留、2条适用条件修订、2条措辞澄清、1条题型调整；不是5条事实错误，也不是人类专家验收。
- 抽样可复现，决定绑定原候选/教材/逐条QA哈希，不新增API调用或生成QA，不改历史40/40自动初筛结果；其余32条尚未抽查，未扩大或训练。见[八条来源抽查](docs/book_sft_revision40_source_review8_20260907.md)。

## v1.12.84（2026-09-07）

- 优化Step120物流知识实验Word材料排版：微软雅黑标题、宋体11磅正文与17磅固定行距，统一标题层级、段落留白、页边距及附录格式。
- 重设4张表格列宽、内边距、深蓝表头和交替底纹，突出汇总行；保留方案二空白。已逐项校验正文及表格文本未变，完成5页Word渲染视觉检查。

## v1.12.83（2026-09-07）

- 用5号机百炼qwen3.8-max完成40条修订版教材QA，40章、四类各10条；明确闭卷题面和短答案，改用来源编号由程序回填证据，生成与逐断言审核使用独立请求。
- 40/40通过本轮自动审核，结构/编号错误和词面评测重合命中均0；同模型自审、新样本和新规则不能证明质量完美或相对旧版的因果提升，仍不扩大、不训练。10项相关测试通过，见[修订小样记录](docs/book_sft_revision40_20260907.md)。

## v1.12.82（2026-09-07）

- 对8条教材QA复核争议各做一次百炼逐疑点裁决，编号证据由程序回填：7条格式/证据编号有效、1条无效；有效项中2条涉及实质疑点、2条仅措辞、3条疑似误报，均非独立事实结论，不自动恢复训练资格。
- 发现裁决器可能混淆带资料问答与闭卷SFT，补充题面外部材料指代检查；既有151条初筛通过项中15条命中，保留原数据及原状态，不扩大、不训练。8项相关测试通过，见[争议裁决记录](docs/book_sft_api_adjudication_20260907.md)。

## v1.12.81（2026-09-07）

- 新增5页Step120物流知识能力诊断与优化实验Word阶段报告及可检索正文，核对8月25日开源续训Step120、LogistikaBench和SC-bench知识题口径，区分早期同名模型与其他沙箱实验。
- 汇总原生/Step120基线、四条件推理探测、评测题直接/改写CPT和原始模型对照；书籍问答SFT章节预留，已完成的同题答案SFT仅列为辅助诊断，保留知识缺口与知识利用混合解释及同题训练边界。
- 已核对来源、关键计数和百分比，并通过Word导出后的5页视觉检查。见[Word材料](汇报材料/Step120物流知识能力诊断与优化实验阶段报告_20260907.docx)和[可检索正文](docs/step120_logistics_experiment_brief_20260907.md)。

## v1.12.80（2026-09-07）

- 完成教材QA二次盲于初审结论的百炼自动复核：原通过项分题型各抽5条（共20），另抽结构拒绝/内容拒绝各4条，样本编号及来源哈希在请求前冻结。
- 原通过项12/20再次通过、8/20出现疑点，不能将40%质疑率直接称为事实错误率；拒绝项未被直接恢复。32条引用失败中4条仅空白差异、27条内容不匹配、1条缺失/过短。
- 保留原始151条初筛结果，不扩大、不训练，不将同模型二次复核标成人工验收；新增复核脚本、6项相关单测及安全统计，见[复核记录](docs/book_sft_api_review_20260907.md)。

## v1.12.79（2026-09-07）

- 使用5号机百炼直连 `qwen3.8-max` 完成教材SFT小样试制：200条候选，44章覆盖，四类题型各50条，并发上限64；不加载本地模型，不训练、不扩大。
- 原文逐字引用检查和独立请求同模型审核后151条进入待抽查队列，32条结构/引用拒绝、17条内容审核拒绝；服务器内对1,672条评测题做词面重合筛查，命中0条，不宣称语义零泄露或人工审核完成。
- 书籍、QA、完整API响应和凭据仅留服务器；仓库包含生成/审核脚本、单测、复核笔记本和安全统计。见[试制记录](docs/book_sft_api_pilot_20260907.md)。

## v1.12.78（2026-09-07）

- 完成同4个补证知识点的闭卷定义/辨析/应用诊断：12问×3轮，模型输入不含参考资料、选项或评分标准，仍使用原始Step120，不训练；协议与3项语义评分要点在模型加载前冻结。
- 审阅26份不同回答并按哈希绑定决定：核心通过6/12（定义2/4、辨析3/4、应用1/4），仅1个知识点三问核心全过；保留知识范围、术语对应、场景映射混合问题的解释，不强行二分缺知识/缺推理。
- 单独记录2次回答的附加事实错误；若对已发现的此类错误也否决，事后敏感性结果为5/12，不替换原主评分。24项相关测试通过，逐回答独立复算和哈希校验通过；见[闭卷知识诊断](docs/step120_closed_knowledge_20260907.md)。

## v1.12.77（2026-09-07）

- 重新核算Step120历史303道错题及60道对照的三轮诊断，并归档安全结果；拆分自动证据门禁，发现363题中259题审核缺失/无效，纠正将“未验证”直接当作资料不足的解释风险。
- 经本轮最多8题新授权，冻结3道候选加5道系统抽样错题并完成先盲审后对照的代理复核；记录基础探针过浅、缺图、重复选项及题型约束问题，不改原题、金标签或正式评分。
- 完成四题独立来源补证与八题题型提示只推理复测：同4题闭卷/原教材0/4，补证4/4，三轮一致，含2道原候选；题型提示单独0/8。审核预算512/1536均8/8解析，未复现历史空审核，不能声称长度问题已定位。20项相关测试通过，全文留私有目录，不训练；见[八题复核记录](docs/step120_evidence_review_20260907.md)。

## v1.12.76（2026-09-07）

- 按最新领域问答能力目标启动只推理诊断：固定Step120历史303道错题，加60道同数据集/题型/类别的原答对匹配对照；不训练、不使用评测题CPT/SFT候选，也不改写原正式成绩。
- 从用户授权的独立物流教材检索有出处的片段，检索与探针生成不接收金答案；比较原题闭卷/加资料、基础知识题闭卷/加资料四条件，每条件三轮。资料充分性和基础题均做自动检查，失败项单独保留。
- 明确同模型作者/审核器偏差、单书覆盖限制、历史错题选择偏差和诊断提示变化；自动结果只作初步机制线索，不直接判定缺知识或缺推理。样本已冻结、5号机诊断已启动，50项相关测试通过，说明见[诊断协议](docs/step120_qa_diagnostic_20260907.md)。

## v1.12.75（2026-09-07）

- 归档已完成的全量同题答案SFT正式评测：Step120、1遍/418步、2遍/836步三轮逐题多数票分别为`1369/1672（81.88%）`、`1558/1672（93.18%）`、`1579/1672（94.44%）`；两遍相对基线净增210题、12.56个百分点。
- 分数据集保留权衡：LogistikaBench由`1180/1446`提升到`1335/1446`、`1359/1446`；SC-bench知识部分由`189/226`提升到`223/226`、`220/226`。按总答对数选择836步模型，但SC-bench单项418步更好；两组SFT多数票结果均无解析失败。
- 补齐[最终结果报告](docs/logistics_mcq_fit_results_20260907.md)与[无正文汇总](docs/logistics_mcq_fit_final_20260907.safe.json)，核验分母、分组加总及配对净增；成绩明确属于训练过评测题后的同题结果，不是独立测试。模型权重、题面、金答案和逐题预测不上传GitHub。
