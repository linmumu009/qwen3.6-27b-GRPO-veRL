...
def test_beta_binomial_tail_and_rebased_capacity():
    native = {
        ...
        "additional_frozen_overlap_outside_eval22": 4,
        "native_error_states_outside_all_frozen_sets": 7,
        "outside_all_frozen_first_error_category_counts": {
            "executable_wrong_or_insufficient_evidence": 6,
            "schema_syntax_or_execution_error": 1,
        },
    }
    native_margin = {
        "contract": NATIVE_MARGIN_CONTRACT,
        "training_allowed": False,
        "scope": {"pairs": 7},
        "decision": {"retain_as_candidate_training_source_stratum": True},
    }
    evaluation = {
    ...
    result = build_strategy(
        native_supply=native, native_margin=native_margin, eval_summary=evaluation
    )
    assert result["corrected_capacity_logic"]["eval22_pairs_reusable_for_training"] == 0
    assert result["existing_zero_npu_opportunity"]["remaining_pairs_if_all_native_states_pass_pair_audit"] == 41
    assert 0.49 < result["corrected_capacity_logic"]["probability_queue_alone_reaches_48_if_every_task_is_approved"] < 0.51
    assert 0.76 < result["rebased_capacity"]["probability_review_queue_supplies_remaining_pairs_if_all_review_tasks_are_approved"] < 0.77
    assert result["rebased_capacity"]["approved_candidate_tasks_for_90pct_predictive_probability"] == 158
    assert result["rebased_capacity"]["approved_candidate_tasks_for_95pct_predictive_probability"] == 172
    ...
    assert result["decision"]["retained_native_pairs"] == 7
    assert result["decision"]["remaining_pair_gap"] == 41
