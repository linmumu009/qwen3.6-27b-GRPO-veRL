...
2026-09-11，全部13份材料的单遍523步训练、完整HF导出和双评测各3轮均已完成，独立复核通过。[最终安全结果](cpt_logists_full_result_20260911.safe.json)。

| 评测集 | Step120 | 完整材料CPT后 | 改对 | 改错 |
|---|---:|---:|---:|---:|
| SC-bench知识 | 189/226 | 189/226 | 2 | 2 |
| LogistikaBench | 1180/1446 | 1183/1446 | 8 | 5 |
| 合计 | 1369/1672 | 1372/1672 | 10 | 7 |

本轮LogistikaBench净增加3题，约+0.21个百分点；SC持平。整体改对10题、改错7题，属于小幅净提升，尚不能由这一次实验认定稳定收益。该实验包含评测来源教材，不描述为独立泛化。相比前一轮部分材料总分持平，本轮净多3题；因语料范围、训练token和批量也发生变化，不能把差异全部归因于新增四本教材。

独立核验了训练文件哈希、4,707条和13来源、523步与1,849,588输入token、有限loss/梯度/LR及完整检查点。重新核对HF导出1,199个张量和15个分片，无缺失、额外张量、形状或dtype差异。新旧两组各5,016次请求的题目×repeat完整，重新解析答案、计算多数票及改对改错，零无效、零截断，案例/提示/生成参数一致。

模型已保存为`llin-step120-logists-full-cpt-1epoch-20260911`，完整路径见安全结果。原Step120与前一轮模型继续保留，没有自动替换模型或追加遍数。自动跟进`cpt`已暂停。下文保留启动时的历史快照。
