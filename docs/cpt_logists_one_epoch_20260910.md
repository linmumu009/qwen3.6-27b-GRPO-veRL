...
2026-09-10，单遍377步训练、完整HF导出及两个评测集各3轮全部完成，独立复核通过。新模型共5,016次请求、1,672题×3轮完整，零无效、零截断；与Step120案例和提示/生成协议一致。[完整安全结果](cpt_logists_result_20260910.safe.json)。

| 评测集 | Step120 | 本轮CPT后 | 改对 | 改错 |
|---|---:|---:|---:|---:|
| SC-bench知识 | 189/226 | 189/226 | 2 | 2 |
| LogistikaBench | 1180/1446 | 1180/1446 | 7 | 7 |
| 合计 | 1369/1672 | 1369/1672 | 9 | 9 |

两套评测净正确数均持平，本配置未观察到净准确率收益；并非每道题表现不变，也不能据此单独判定语料选取有误或所有CPT无效。选材参考过评测知识范围，这仍是开发评测，不宣称独立泛化。

再次核验了训练377步、1,183,197输入token和完整检查点；重新核对HF导出1,199个张量、15个分片、语言层0–63完整，无缺失/额外张量、形状或dtype差异。模型以`llin-step120-logists-cpt-1epoch-20260910`保存，保留检查点、原失败快照与日志，不替换Step120。没有追加训练遍数。

自动跟进`cpt`已暂停。下文保留启动和导出修复历史，不代表当前仍在运行。
