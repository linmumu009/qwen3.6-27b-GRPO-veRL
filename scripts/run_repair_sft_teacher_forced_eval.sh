...
DATA_FILE="${DATA_FILE:-${DATA_DIR}/repair_sft_train.parquet}"
...
for path in "${MODEL_PATH}/config.json" "${SOURCE_MODEL_DIST_CKPT}/.metadata" "${DATA_FILE}"; do
  if [[ ! -e "${path}" ]]; then
    printf 'required teacher-forced input missing: %s\n' "${path}" >&2
    exit 2
  fi
done
...
mkdir -p "${OUTPUT_DIR}"
python3 "${PROJECT_ROOT}/scripts/check_teacher_forced_component_masks.py" \
  --data-file "${DATA_FILE}" \
  --model-path "${MODEL_PATH}" \
  --max-length "${MAX_LENGTH}" \
  --output "${OUTPUT_DIR}/component_mask_gate.json"

cat > "${OUTPUT_DIR}/experiment_contract.txt" <<EOF
purpose=repair_sft_teacher_forced_component_diagnostic
...
torchrun --standalone --nnodes=1 --nproc_per_node="${NPROC}" \
  ...
  "data.custom_cls.path=${PROJECT_ROOT}/scripts/qwen36_teacher_forced_diagnostic_dataset.py" \
  data.custom_cls.name=Qwen36TeacherForcedDiagnosticDataset \
  model=hf_model \
  "model.path=${MODEL_PATH}" \
  ...
  trainer.project_name=llin-repair-sft-diagnostic \
