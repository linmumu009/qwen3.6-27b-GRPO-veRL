...
# The six scenes are the audience-facing causal story.  The thirteen real
# engineering stages remain available as evidence, but no longer compete with
# the explanation of how a world is built.
SCENES = (
    StoryScene(
        "brief",
        "01",
        "从业务描述中，找出这个世界有什么",
        "先回答：这个物流世界里，有谁、有什么、要解决什么问题？",
        "物流业务说明",
        "识别角色 · 对象 · 目标",
        "世界骨架",
        ("intake", "prd"),
    ...
    StoryScene(
        ...
        "04",
        "给同一个现实世界加上运行规则",
        "事实只说明发生了什么；制度、SLA 与流程才说明应该怎样运行。",
        "事实世界",
        "叠加制度 · SLA · 流程",
        "规则世界",
        ("catalog", "documents"),
    ...
    StoryScene(
        ...
        "05",
        "从同一个世界，提出三种完全不同的问题",
        "数仓看现实，知识看规则，混合任务把现实和规则放在一起判断。",
        "事实世界 + 规则世界",
        "选择不同的观察方式",
        "三类评测任务",
        ("dwh_tasks", "kb_tasks", "hybrid"),
        6200,
    ),
    StoryScene(
        "seal",
        "06",
        "冻结世界，并把答案留在评测者一侧",
        "最后固定世界版本和可见边界，让智能体只能进入世界、不能偷看答案。",
        "完整世界 + 三类任务",
        "冻结版本 · 隔离秘密 · 挂载环境",
        "可进入的物流沙箱",
        ("freeze", "runner"),
...
EXAMPLE_CASES = (
    ...
    },
    {
        ...
        "nodes": (
            {
                "label": "一个采样单元",
                "title": "“运输中的运单”成为一种可抽取场景",
                "code": "waybill · in_transit",
    ...
    },
    {
        ...
        "nodes": (
            {
                "label": "真实问题",
                "title": "“生成冷链运单最新一期的数据概览报告。”",
                "code": "task_000285",
                "items": ("对象：冷链运单", "任务：生成概览报告", "只使用已经生成的运单事实"),
                "detail": "这条问题来自 5 号机已生成任务包。",
                "source": "dwh_tasks.jsonl · task_000285",
                "arrow": "绑定实际证据结构",
            },
            {
                "label": "证据计划",
                "title": "先确认运单表能够回答",
                "code": "filter + aggregate + format",
                "items": ("表：fact_waybill（运单事实表）", "字段：product_type（产品类型）", "字段：status（运单状态）", "字段：temp_sensor_id（温度传感器）"),
    ...
    },
    {
        ...
        "nodes": (
            {
                "label": "真实问题",
                "title": "“本季度华南区运单重货专线产品合规检查，依据《各线路时效承诺标准（SLA）》检查合规性？”",
                "code": "HT-0051 · data_then_policy",
                "items": ("时间：本季度", "区域：华南区", "产品：重货专线", "判断：实际表现是否符合 SLA"),
                "detail": "问题本身明确要求事实与规则两条证据链。",
                "source": "hybrid_tasks.jsonl · HT-0051",
                "arrow": "拆成两条证据计划",
            },
            {
                "label": "事实证据",
                ...
                "items": ("expected_tables：fact_delivery", "时间范围：本季度", "验证结果：979 条派送记录", "在 SQLite 中确认记录存在"),
            ...
            {
                "label": "规则证据",
                "title": "从 doc_002 读取时效口径和预警规则",
                "code": "requires_doc_search = true",
                "items": ("source_documents：doc_002", "section：timeliness", "按产品类型和线路确定标准时效", "达到节点阈值 80% 触发预警"),
                "detail": "这条链回答“制度要求什么”。",
...
EXPLAINER_HTML_TEMPLATE = r'''<!doctype html>
<html lang="zh-CN">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>一票运单如何长成物流沙箱</title>
<style>
:root{color-scheme:dark;--bg:#050a12;--surface:#0a1322;--surface2:#0d1a2c;--line:#213550;--text:#eff7ff;--muted:#91a6bd;--fact:#42b5ff;--fact2:#92dcff;--rule:#a77cff;--task:#ffc75a;--ok:#52dda0;--danger:#ff657a}
*{box-sizing:border-box}html{background:var(--bg)}body{margin:0;min-width:1120px;color:var(--text);font-family:Inter,"PingFang SC","Microsoft YaHei",system-ui,sans-serif;background:radial-gradient(circle at 50% -10%,rgba(66,181,255,.12),transparent 34%),radial-gradient(circle at 86% 12%,rgba(167,124,255,.09),transparent 26%),var(--bg)}button,select{font:inherit}.sr-only{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}.app{width:100%;padding:18px 28px 16px}.top{display:flex;justify-content:space-between;align-items:flex-start;gap:28px}.eyebrow{font-size:11px;letter-spacing:.18em;color:var(--fact2);text-transform:uppercase;margin-bottom:5px}.top h1{margin:0;font-size:27px;font-weight:600;letter-spacing:-.03em}.top p{margin:6px 0 0;color:var(--muted);font-size:13px}.legend{display:flex;gap:15px;padding-top:9px;color:#afc0d3;font-size:11px;white-space:nowrap}.legend i{display:inline-block;width:8px;height:8px;border-radius:50%;margin-right:5px}.legend .f{background:var(--fact);box-shadow:0 0 12px var(--fact)}.legend .r{background:var(--rule);box-shadow:0 0 12px var(--rule)}.legend .t{background:var(--task);box-shadow:0 0 12px var(--task)}
.toolbar{height:42px;margin-top:14px;display:flex;align-items:center;gap:7px}.btn,.speed{height:32px;border:1px solid #2a4160;border-radius:7px;background:#0b1728;color:#d9e8f7;padding:0 12px;cursor:pointer}.btn.primary{background:#d9f2ff;color:#06101c;border-color:#d9f2ff;font-weight:600}.btn:hover:not(:disabled){border-color:#65baff}.btn:disabled{opacity:.38;cursor:not-allowed}.speed{padding:0 6px}.status{margin-left:10px;flex:1}.status-line{display:flex;justify-content:space-between;margin-bottom:5px;color:#89a0b8;font-size:10px}.status-line strong{color:#dbeafb;font-weight:500}.track{height:3px;background:#14243a;overflow:hidden}.fill{height:100%;width:0;background:linear-gradient(90deg,var(--fact),var(--rule),var(--task),var(--ok));box-shadow:0 0 12px rgba(82,221,160,.45);transition:width .45s ease}
.causal-nav{display:grid;grid-template-columns:repeat(6,1fr);gap:18px;margin:8px 0 12px}.cause{position:relative;min-height:64px;padding:9px 11px;border:1px solid #1b2c42;border-radius:8px;background:rgba(8,16,29,.68);color:#738ba4;text-align:left;cursor:pointer;transition:.35s}.cause:not(:last-child)::after{content:"→";position:absolute;right:-16px;top:21px;color:#38516e;font-size:19px}.cause small{display:block;font-size:9px;letter-spacing:.12em;margin-bottom:4px}.cause b{display:block;font-size:12px;font-weight:500;color:#98aec4}.cause span{display:block;margin-top:3px;font-size:10px}.cause.done{border-color:rgba(82,221,160,.28);color:#75b89b}.cause.done b{color:#b4d7c8}.cause.current{border-color:var(--fact);background:linear-gradient(135deg,rgba(66,181,255,.16),rgba(11,24,41,.95));box-shadow:0 0 0 1px rgba(66,181,255,.12),0 0 30px rgba(66,181,255,.08);transform:translateY(-2px);color:#8ed7ff}.cause.current b{color:#fff}.cause.current::before{content:"";position:absolute;left:10px;right:10px;bottom:-1px;height:2px;background:var(--fact);box-shadow:0 0 14px var(--fact)}
.theatre{position:relative;border:1px solid #20344e;background:linear-gradient(180deg,rgba(10,20,35,.98),rgba(5,12,22,.99));box-shadow:0 24px 80px rgba(0,0,0,.34);overflow:hidden}.theatre::before{content:"";position:absolute;inset:0;background-image:linear-gradient(rgba(88,139,191,.035) 1px,transparent 1px),linear-gradient(90deg,rgba(88,139,191,.035) 1px,transparent 1px);background-size:42px 42px;mask-image:linear-gradient(to bottom,#000,transparent 80%);pointer-events:none}.scene-head{position:relative;z-index:3;min-height:82px;padding:16px 22px 12px;display:grid;grid-template-columns:1fr auto;gap:30px;border-bottom:1px solid #172940}.scene-kicker{color:var(--fact2);font-size:10px;letter-spacing:.14em}.scene-title{display:block;margin-top:5px;font-size:23px;font-weight:600;letter-spacing:-.025em}.scene-question{align-self:center;max-width:500px;color:#b5c6d8;font-size:13px;line-height:1.55;text-align:right}.scene-question::before{content:"这一步解决什么问题";display:block;color:#657d96;font-size:9px;letter-spacing:.12em;margin-bottom:3px}.canvas{position:relative;height:392px;overflow:hidden}.canvas::after{content:"";position:absolute;left:0;right:0;bottom:0;height:90px;background:linear-gradient(transparent,rgba(5,12,22,.86));pointer-events:none}.visual-label{position:absolute;z-index:5;left:22px;top:18px;color:#6e88a2;font-size:10px;letter-spacing:.12em}.center-note{position:absolute;z-index:8;left:50%;bottom:20px;transform:translateX(-50%);min-width:420px;padding:9px 17px;border:1px solid rgba(121,166,207,.26);background:rgba(5,13,24,.88);backdrop-filter:blur(10px);text-align:center;color:#c9d9e8;font-size:12px}.center-note strong{color:#fff;font-weight:600}.chip{display:inline-flex;align-items:center;justify-content:center;border:1px solid #29435f;background:rgba(11,26,45,.94);color:#d9eaff;border-radius:7px;padding:8px 12px;font-size:12px}.chip.fact{border-color:rgba(66,181,255,.52);box-shadow:0 0 22px rgba(66,181,255,.08)}.chip.rule{border-color:rgba(167,124,255,.52);box-shadow:0 0 22px rgba(167,124,255,.08)}.chip.task{border-color:rgba(255,199,90,.56)}
.causal-strip{position:relative;z-index:4;display:grid;grid-template-columns:1fr 54px 1.18fr 54px 1fr;align-items:stretch;border-top:1px solid #1a2c44;background:#07111e}.causal-box{padding:11px 15px}.causal-box small{display:block;color:#6e849b;font-size:9px;letter-spacing:.11em;margin-bottom:5px}.causal-box strong{font-size:13px;font-weight:500}.causal-box.input strong{color:var(--fact2)}.causal-box.change{background:rgba(167,124,255,.06)}.causal-box.change strong{color:#cfb8ff}.causal-box.result strong{color:#80e4b4}.flow-arrow{display:grid;place-items:center;color:#507193;font-size:22px;background:linear-gradient(90deg,transparent,rgba(66,181,255,.04),transparent)}
.brief-card{position:absolute;left:8%;top:52px;width:310px;height:226px;padding:25px 27px;background:linear-gradient(145deg,#102038,#0a1525);border:1px solid #2a4766;box-shadow:0 20px 55px rgba(0,0,0,.34);transform:rotateY(7deg) rotateZ(-1deg)}.brief-card b{font-size:17px}.brief-card p{color:#91a9bf;font-size:11px;line-height:1.7}.brief-lines i{display:block;height:5px;margin:9px 0;background:#28415c;border-radius:3px}.brief-lines i:nth-child(2){width:82%}.brief-lines i:nth-child(3){width:62%}.scan{position:absolute;left:7.5%;top:48px;width:320px;height:3px;background:linear-gradient(90deg,transparent,var(--fact2),transparent);box-shadow:0 0 18px var(--fact);animation:scan 3.2s ease-in-out infinite}.extract{position:absolute;left:46%;right:6%;top:48px;bottom:72px}.extract::before{content:"";position:absolute;left:-70px;top:110px;width:70px;border-top:1px solid var(--fact);box-shadow:0 0 12px var(--fact)}.extract-group{position:absolute;display:flex;gap:10px;align-items:center}.extract-group strong{width:70px;color:#7893ad;font-size:10px;letter-spacing:.12em}.extract-group.roles{top:12px}.extract-group.objects{top:92px}.extract-group.goals{top:172px}.extract .chip{opacity:0;transform:translateX(-18px);animation:materialize .45s forwards}.extract-group:nth-child(1) .chip{animation-delay:.5s}.extract-group:nth-child(2) .chip{animation-delay:1.35s}.extract-group:nth-child(3) .chip{animation-delay:2.2s}.extract .count{position:absolute;right:2px;top:8px;color:#72cfff;font-size:11px}
.world-grid{position:absolute;inset:38px 8% 58px}.world-grid svg{position:absolute;inset:0;width:100%;height:100%;overflow:visible}.world-grid path{fill:none;stroke:#1e87c7;stroke-width:1.4;filter:drop-shadow(0 0 6px rgba(66,181,255,.45));stroke-dasharray:6 8;animation:dash 5s linear infinite}.place{position:absolute;width:92px;height:52px;display:grid;place-items:center;border:1px solid #2d6b96;background:#0a2135;color:#aee4ff;font-size:11px}.place.warehouse{left:3%;top:12%}.place.hub{left:45%;top:43%}.place.outlet{right:3%;top:13%}.place.customer{right:8%;bottom:2%}.waybill{position:absolute;z-index:6;left:16%;top:57%;width:128px;padding:8px 10px;border:1px solid var(--task);background:#281e0d;color:#ffe4a4;box-shadow:0 0 25px rgba(255,199,90,.18);font-size:10px;animation:parcelMove 4.7s cubic-bezier(.5,0,.35,1) infinite}.waybill b{display:block;font-size:12px;color:#fff3ce;margin-bottom:3px}.state-lane{position:absolute;left:26%;right:22%;bottom:16px;display:flex;align-items:center;justify-content:center}.state{position:relative;padding:7px 12px;border:1px solid #28445e;background:#091827;color:#6f89a1;font-size:10px}.state:not(:last-child)::after{content:"→";position:absolute;right:-21px;top:5px;color:#45617d}.state+.state{margin-left:30px}.state:nth-child(1){animation:stateGlow 4.7s infinite}.state:nth-child(2){animation:stateGlow 4.7s 1.55s infinite}.state:nth-child(3){animation:stateGlow 4.7s 3.1s infinite}.repair-line{position:absolute;right:7%;top:46%;width:292px;padding:9px 12px;border-left:2px solid var(--danger);background:rgba(42,10,20,.72);color:#ff9cab;font-size:10px;animation:repairSwap 4.7s infinite}.repair-line b{display:block;color:#fff;font-size:11px;margin-bottom:3px}.mechanic-counts{position:absolute;left:7%;top:44%;display:grid;gap:7px}.mechanic-counts span{color:#86a0b9;font-size:10px}.mechanic-counts strong{display:inline-block;width:45px;color:#e9f7ff;font-size:17px}
.projection{position:absolute;left:5%;top:68px;width:28%;height:205px;transform:perspective(500px) rotateY(14deg)}.ontology-node{position:absolute;border:1px solid #3076a7;background:#0a2136;color:#a9e3ff;padding:7px 10px;font-size:10px}.ontology-node:nth-child(1){left:10px;top:16px}.ontology-node:nth-child(2){right:5px;top:77px}.ontology-node:nth-child(3){left:25px;bottom:5px}.projection svg{position:absolute;width:100%;height:100%}.projection line{stroke:#3189c4;stroke-width:1;stroke-dasharray:4 5;animation:dash 4s linear infinite}.morph-arrow{position:absolute;left:34%;top:151px;width:12%;height:1px;background:linear-gradient(90deg,var(--fact),var(--rule));box-shadow:0 0 12px var(--fact)}.morph-arrow::after{content:"世界结构 → 数据结构";position:absolute;left:5px;top:-23px;color:#829bb3;font-size:10px}.schema-stack{position:absolute;left:48%;top:50px;width:19%;height:246px}.schema-sheet{position:absolute;width:160px;height:105px;border:1px solid rgba(167,124,255,.5);background:rgba(27,17,52,.82);transform:skewY(-4deg);box-shadow:-9px 9px 0 rgba(73,45,120,.15)}.schema-sheet:nth-child(1){left:0;top:0}.schema-sheet:nth-child(2){left:15px;top:55px}.schema-sheet:nth-child(3){left:30px;top:110px}.schema-sheet span{display:block;margin:12px;color:#d9c9ff;font-size:10px}.schema-count{position:absolute;left:38px;bottom:0;color:#d5bcff;font-size:12px}.reality-route{position:absolute;right:5%;top:62px;width:27%;height:220px}.reality-route .mini-route{position:absolute;left:12px;right:12px;top:88px;height:2px;background:#214a66}.reality-route .mini-route::before,.reality-route .mini-route::after{content:"";position:absolute;top:-8px;width:17px;height:17px;border:2px solid var(--fact);border-radius:50%;background:#091827}.reality-route .mini-route::before{left:0}.reality-route .mini-route::after{right:0}.parcel-dot{position:absolute;z-index:4;left:18px;top:82px;width:18px;height:18px;background:var(--task);box-shadow:0 0 16px var(--task);animation:dotRoute 3.7s ease-in-out infinite}.fact-rain{position:absolute;left:10%;right:10%;top:124px;display:flex;gap:5px;flex-wrap:wrap;justify-content:center}.fact-rain i{width:4px;height:4px;background:var(--fact);opacity:.2;animation:factPulse 1.8s infinite}.fact-rain i:nth-child(3n){animation-delay:.5s}.fact-rain i:nth-child(4n){animation-delay:1s}.fact-total{position:absolute;left:0;right:0;bottom:0;text-align:center;color:#83d4ff;font-size:11px}.fact-total strong{display:block;color:#fff;font-size:25px}
.same-world{position:absolute;left:6%;right:6%;top:42px;bottom:58px}.world-disc{position:absolute;left:7%;top:10px;width:41%;height:230px;border:1px solid rgba(66,181,255,.45);border-radius:50%;background:radial-gradient(circle,rgba(66,181,255,.13),rgba(5,15,27,.86) 66%);box-shadow:inset 0 0 50px rgba(66,181,255,.08)}.world-disc::after{content:"同一个事实世界";position:absolute;left:0;right:0;bottom:25px;text-align:center;color:#9edfff;font-size:11px}.world-disc .route-cross{position:absolute;left:18%;right:18%;top:48%;height:1px;background:#2b739e;transform:rotate(-12deg)}.world-disc .route-cross::after{content:"W-DEMO-001 · 已签收";position:absolute;left:28%;top:-29px;border:1px solid #2d749f;background:#0a2135;padding:6px 9px;color:#b8e9ff;font-size:10px}.rule-overlay{position:absolute;right:5%;top:12px;width:44%;height:225px}.rule-page{position:absolute;left:24px;right:0;height:55px;padding:10px 12px;border:1px solid rgba(167,124,255,.5);background:linear-gradient(90deg,rgba(45,27,75,.92),rgba(17,15,35,.86));color:#cfbaff;font-size:11px;opacity:0;transform:translateX(45px);animation:ruleIn .5s forwards}.rule-page:nth-child(1){top:8px;animation-delay:.35s}.rule-page:nth-child(2){top:76px;animation-delay:1.05s}.rule-page:nth-child(3){top:144px;animation-delay:1.75s}.rule-page b{display:block;color:#fff;font-size:12px;margin-bottom:2px}.rule-beam{position:absolute;left:42%;top:117px;width:16%;height:2px;background:linear-gradient(90deg,var(--fact),var(--rule));box-shadow:0 0 20px var(--rule)}.rules-stats{position:absolute;right:4%;bottom:2px;color:#9e87d1;font-size:10px}.rules-stats strong{color:#e9deff;font-size:16px;margin:0 3px}
.task-world{position:absolute;left:50%;top:46px;transform:translateX(-50%);width:198px;height:92px;border:1px solid #47667f;border-radius:50%;background:radial-gradient(circle,rgba(66,181,255,.12),rgba(167,124,255,.08),transparent 74%);display:grid;place-items:center;text-align:center;color:#dcecff;font-size:11px;box-shadow:0 0 48px rgba(81,151,214,.13)}.task-world strong{display:block;font-size:13px}.task-beams{position:absolute;left:13%;right:13%;top:137px;height:68px}.task-beams i{position:absolute;left:50%;top:0;width:1px;height:100%;transform-origin:top;background:linear-gradient(var(--fact),var(--task));box-shadow:0 0 10px rgba(255,199,90,.4)}.task-beams i:nth-child(1){transform:rotate(55deg)}.task-beams i:nth-child(2){transform:rotate(0)}.task-beams i:nth-child(3){transform:rotate(-55deg)}.task-cards{position:absolute;left:4%;right:4%;top:192px;display:grid;grid-template-columns:repeat(3,1fr);gap:14px}.task-card{min-height:126px;padding:13px 15px;border:1px solid #294158;background:rgba(8,18,31,.94);text-align:center}.task-card.fact-task{border-top:2px solid var(--fact)}.task-card.rule-task{border-top:2px solid var(--rule)}.task-card.hybrid-task{border-top:2px solid var(--task)}.task-card small{display:block;color:#6f879f;font-size:9px;letter-spacing:.1em}.task-card b{display:block;margin:5px 0 6px;font-size:14px}.task-card p{margin:0;color:#b4c6d7;font-size:11px;line-height:1.5}.task-card strong{display:inline-block;margin-top:7px;font-size:20px}.fact-task strong{color:var(--fact2)}.rule-task strong{color:#cfbaff}.hybrid-task strong{color:#ffe0a0}.compare{position:absolute;z-index:9;left:50%;top:141px;transform:translateX(-50%);padding:5px 10px;border:1px solid rgba(255,199,90,.45);background:#211b0d;color:#ffe0a0;font-size:10px;white-space:nowrap}.compare b{color:#fff}
.seal-world{position:absolute;left:5%;right:5%;top:38px;bottom:52px;display:grid;grid-template-columns:1fr 92px 1fr;align-items:center}.vault,.runner-zone{height:232px;padding:21px;border:1px solid #263e58;background:rgba(7,17,30,.88)}.vault{border-color:rgba(255,199,90,.4)}.runner-zone{border-color:rgba(82,221,160,.42)}.zone-title{font-size:12px;color:#fff}.zone-title small{display:block;color:#71869d;font-size:9px;letter-spacing:.11em;margin-bottom:4px}.vault-items,.runner-items{display:flex;flex-wrap:wrap;gap:8px;margin-top:22px}.locked{border-color:rgba(255,199,90,.45);color:#ffe0a0}.visible-item{border-color:rgba(82,221,160,.42);color:#a7f0cc}.lock-note{margin-top:18px;color:#a9905c;font-size:10px}.agent-orb{margin:20px auto 0;width:70px;height:70px;border:1px solid var(--ok);border-radius:50%;display:grid;place-items:center;color:#aff0d0;font-size:10px;box-shadow:0 0 30px rgba(82,221,160,.13);animation:agentPulse 2s ease-in-out infinite}.firewall{height:248px;display:grid;place-items:center;color:var(--danger);font-size:10px;text-align:center}.firewall::before{content:"";position:absolute;width:2px;height:220px;background:linear-gradient(transparent,var(--danger),transparent);box-shadow:0 0 18px rgba(255,101,122,.45)}.firewall span{position:relative;z-index:2;padding:7px;background:var(--bg)}.freeze-ring{position:absolute;left:calc(50% + 78px);top:32px;width:34%;height:245px;border:1px solid rgba(82,221,160,.34);border-radius:50%;animation:sealRing 2.4s ease-out both}
.ledger{margin-top:9px;border-top:1px solid #17283c;color:#8da3b9}.ledger summary{cursor:pointer;padding:10px 3px;font-size:11px;color:#8fa7be;list-style:none}.ledger summary::before{content:"＋";display:inline-block;width:20px;color:#5c83a7}.ledger[open] summary::before{content:"－"}.ledger-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:1px;background:#16283d;border:1px solid #16283d}.ledger-row{min-height:48px;padding:8px 11px;background:#081321;display:grid;grid-template-columns:74px 1fr auto;gap:10px;align-items:center}.ledger-row small{color:#67819a;font-size:9px}.ledger-row b{font-size:10px;font-weight:500;color:#c4d5e5}.ledger-row span{color:#71869b;font-size:9px;text-align:right}.footer{display:flex;justify-content:space-between;margin-top:9px;color:#536c84;font-size:9px}.footer a{color:#6f91b0;text-decoration:none}
.transition{position:absolute;z-index:40;inset:82px 0 0;display:grid;place-items:center;background:radial-gradient(circle,rgba(24,58,89,.88),rgba(4,9,17,.98) 62%);opacity:0;visibility:hidden;transition:.35s}.transition.show{opacity:1;visibility:visible}.transition-inner{text-align:center}.transition-inner small{display:block;color:#72cfff;letter-spacing:.18em;font-size:10px}.transition-inner strong{display:block;margin:10px 0 7px;font-size:27px}.transition-inner p{color:#9db2c6;font-size:12px}.end{position:absolute;z-index:60;inset:0;display:grid;place-items:center;background:radial-gradient(circle,rgba(24,95,70,.36),rgba(3,8,14,.98) 58%);opacity:0;visibility:hidden;transition:.6s}.end.show{opacity:1;visibility:visible}.end-card{text-align:center;width:540px;padding:30px;border:1px solid rgba(82,221,160,.55);background:linear-gradient(145deg,rgba(8,44,34,.96),rgba(5,15,24,.98));box-shadow:0 0 90px rgba(82,221,160,.15)}.end-check{width:76px;height:76px;margin:auto;border:2px solid var(--ok);border-radius:50%;display:grid;place-items:center;color:var(--ok);font-size:37px;box-shadow:0 0 0 11px rgba(82,221,160,.05),0 0 35px rgba(82,221,160,.25)}.end-card small{display:block;margin-top:18px;color:#78d8ac;letter-spacing:.15em}.end-card h2{margin:7px 0 4px;font-size:28px}.end-card p{margin:0 0 18px;color:#9abbb0;font-size:12px}.end-metrics{display:grid;grid-template-columns:repeat(3,1fr);margin:17px 0;border-top:1px solid rgba(82,221,160,.2);border-bottom:1px solid rgba(82,221,160,.2)}.end-metrics div{padding:10px;color:#7f9f95;font-size:9px}.end-metrics strong{display:block;color:#e4fff5;font-size:16px}
body.finished .scan,body.finished .world-grid path,body.finished .waybill,body.finished .state,body.finished .repair-line,body.finished .projection line,body.finished .parcel-dot,body.finished .fact-rain i,body.finished .rule-page,body.finished .agent-orb,body.finished .freeze-ring{animation-play-state:paused!important}
@keyframes scan{0%,100%{transform:translateY(0);opacity:.4}50%{transform:translateY(216px);opacity:1}}@keyframes materialize{to{opacity:1;transform:none}}@keyframes dash{to{stroke-dashoffset:-80}}@keyframes parcelMove{0%,16%{left:16%;top:57%}42%,55%{left:47%;top:42%}82%,100%{left:76%;top:23%}}@keyframes stateGlow{0%,28%,100%{color:#6f89a1;border-color:#28445e;background:#091827}8%,20%{color:#fff;border-color:var(--fact);background:#0b3451;box-shadow:0 0 18px rgba(66,181,255,.25)}}@keyframes repairSwap{0%,47%{border-color:var(--danger);color:#ff9cab}58%,100%{border-color:var(--ok);color:#8de7b9;background:rgba(7,42,29,.72)}}@keyframes dotRoute{0%,12%{left:18px}88%,100%{left:calc(100% - 30px)}}@keyframes factPulse{0%,100%{opacity:.12;transform:scale(.7)}50%{opacity:.75;transform:scale(1.25)}}@keyframes ruleIn{to{opacity:1;transform:none}}@keyframes agentPulse{50%{box-shadow:0 0 46px rgba(82,221,160,.35);transform:scale(1.04)}}@keyframes sealRing{from{opacity:0;transform:scale(1.3)}to{opacity:1;transform:scale(1)}}
@media(prefers-reduced-motion:reduce){*,*::before,*::after{animation-duration:.01ms!important;animation-iteration-count:1!important;transition-duration:.01ms!important}}
</style>
</head><body><main class="app">
<header class="top"><div><div class="eyebrow">ONE WAYBILL · ONE CAUSAL STORY</div><h1>一票运单，如何一步步长成一个物流沙箱？</h1><p>始终沿着同一条逻辑线看：先建世界，再让它运行，再从中产生任务，最后冻结边界。</p></div><div class="legend"><span><i class="f"></i>现实 / 状态</span><span><i class="r"></i>规则 / 制度</span><span><i class="t"></i>任务 / 判断</span></div></header>
<section class="toolbar" aria-label="播放控制"><button id="prev" class="btn" type="button">← 上一步</button><button id="play" class="btn primary" type="button">暂停</button><button id="next" class="btn" type="button">下一步 →</button><button id="restart" class="btn" type="button">重新播放</button><label class="sr-only" for="speed">播放速度</label><select id="speed" class="speed"><option value=".75">0.75×</option><option value="1" selected>1×</option><option value="1.5">1.5×</option><option value="2">2×</option></select><div class="status"><div class="status-line"><strong id="progressText"></strong><span id="live" aria-live="polite"></span></div><div class="track"><div id="fill" class="fill"></div></div></div></section>
<nav id="causalNav" class="causal-nav" aria-label="沙箱生成的六步因果链"></nav>
<section class="theatre"><header class="scene-head"><div><span id="sceneKicker" class="scene-kicker"></span><strong id="sceneTitle" class="scene-title"></strong></div><div id="sceneQuestion" class="scene-question"></div></header><div id="canvas" class="canvas" role="img" aria-live="polite"></div><div class="causal-strip"><div class="causal-box input"><small>输入：已经有什么</small><strong id="inputLabel"></strong></div><div class="flow-arrow">→</div><div class="causal-box change"><small>变化：这一部做什么</small><strong id="changeLabel"></strong></div><div class="flow-arrow">→</div><div class="causal-box result"><small>结果：世界因此获得</small><strong id="resultLabel"></strong></div></div>
<div id="transition" class="transition"><div class="transition-inner"><small id="transitionStep"></small><strong id="transitionTitle"></strong><p id="transitionText"></p></div></div>
<div id="end" class="end"><div class="end-card"><div class="end-check">✓</div><small>WORLD MODEL SEALED</small><h2>沙箱生成完成</h2><p>世界版本已冻结；智能体只能观察和行动，任务与答案仍留在评测者一侧。</p><div class="end-metrics"><div><strong>36,101</strong>事实状态</div><div><strong>65</strong>制度文档</div><div><strong>1,520</strong>三类任务</div></div><button id="endRestart" class="btn" type="button">重新观看</button></div></div></section>
<details class="ledger"><summary>查看 13 个工程阶段与落盘凭证（主线的可审计证据）</summary><div id="ledgerGrid" class="ledger-grid"></div></details>
<footer class="footer"><span>真实生成汇总：运营分析-8767b626 · 80分31秒</span><span><a href="https://github.com/renjunxiang/sf_my_sandbox/tree/758917009d0ebb0fb36561197171f6abdd279d96/.runtime_state/v20/scenes/运营分析-8767b626">v20 执行证据</a> · 不展示任务正文、hidden gold、SQL 或数据库行</span></footer>
</main>
<script id="stages" type="application/json">__STAGE_DATA__</script><script id="sceneData" type="application/json">__SCENE_DATA__</script>
<script>(()=>{'use strict';const stages=JSON.parse(document.getElementById('stages').textContent),scenes=JSON.parse(document.getElementById('sceneData').textContent);const navLabels=['业务描述','世界结构','现实实例','世界规则','观察任务','冻结沙箱'];const $=id=>document.getElementById(id),el={nav:$('causalNav'),kicker:$('sceneKicker'),title:$('sceneTitle'),question:$('sceneQuestion'),canvas:$('canvas'),input:$('inputLabel'),change:$('changeLabel'),result:$('resultLabel'),progress:$('progressText'),live:$('live'),fill:$('fill'),prev:$('prev'),play:$('play'),next:$('next'),restart:$('restart'),speed:$('speed'),transition:$('transition'),transitionStep:$('transitionStep'),transitionTitle:$('transitionTitle'),transitionText:$('transitionText'),end:$('end'),endRestart:$('endRestart'),ledger:$('ledgerGrid')};const state={scene:0,playing:true,finished:false,transitioning:false,speed:1,timer:null};const reduced=matchMedia('(prefers-reduced-motion:reduce)').matches;const esc=v=>String(v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]));
const visuals={brief:`<span class="visual-label">业务散文不是世界模型；先把隐含结构显式化</span><div class="brief-card"><b>德运物流 · 运营分析</b><p>全国零担快运网络<br>五条产品线 · 多级运营角色<br>查询 / 对比 / 趋势 / 归因 / 诊断</p><div class="brief-lines"><i></i><i></i><i></i></div></div><i class="scan"></i><div class="extract"><span class="count">识别到 5 类运营角色</span><div class="extract-group roles"><strong>谁在世界里</strong><span class="chip">仓库经理</span><span class="chip">网点主管</span><span class="chip">区域运营</span></div><div class="extract-group objects"><strong>世界里有什么</strong><span class="chip fact">运单</span><span class="chip fact">仓库</span><span class="chip fact">线路</span><span class="chip fact">承运商</span></div><div class="extract-group goals"><strong>世界要回答</strong><span class="chip task">查询</span><span class="chip task">对比</span><span class="chip task">归因</span><span class="chip task">诊断</span></div></div><div class="center-note"><strong>得到的不是文件清单，</strong>而是后续所有建模共享的“世界骨架”。</div>`,
mechanics:`<span class="visual-label">用一票示意运单贯穿状态、动作与约束</span><div class="world-grid"><svg viewBox="0 0 1000 290" preserveAspectRatio="none"><path d="M90 55 C310 10 370 180 510 155 S780 0 915 58"/><path d="M90 55 C280 160 330 210 510 155 S760 245 915 232"/></svg><div class="place warehouse">仓库</div><div class="place hub">分拨中心</div><div class="place outlet">直营网点</div><div class="place customer">收件客户</div><div class="waybill"><b>W-DEMO-001</b>示意运单（非真实数据库行）</div><div class="state-lane"><span class="state">待揽收</span><span class="state">运输中</span><span class="state">已签收</span></div></div><div class="mechanic-counts"><span><strong>57</strong>实体</span><span><strong>76</strong>状态</span><span><strong>80</strong>动作</span></div><div class="repair-line"><b>校验并修复世界规则</b>incident_closed 不存在 → 改为真实状态 incident_closed_without_compensation · 9/9 通过</div><div class="center-note"><strong>世界建模 = 实体 + 状态 + 动作 + 约束。</strong>运单现在能合法地从一个状态走向下一个状态。</div>`,
reality:`<span class="visual-label">从“世界应该长什么样”到“这个世界此刻发生了什么”</span><div class="projection"><svg viewBox="0 0 250 205"><line x1="45" y1="35" x2="190" y2="95"/><line x1="190" y1="95" x2="62" y2="175"/><line x1="45" y1="35" x2="62" y2="175"/></svg><span class="ontology-node">ENTITY · 运单</span><span class="ontology-node">STATE · 运输中</span><span class="ontology-node">ACTION · 到达网点</span></div><div class="morph-arrow"></div><div class="schema-stack"><div class="schema-sheet"><span>waybill_id · status</span></div><div class="schema-sheet"><span>route · warehouse</span></div><div class="schema-sheet"><span>event · timestamp</span></div><div class="schema-count">64 张关联表</div></div><div class="reality-route"><div class="mini-route"></div><i class="parcel-dot"></i><div class="fact-rain">${'<i></i>'.repeat(48)}</div><div class="fact-total"><strong>36,101</strong>条状态实例</div></div><div class="center-note"><strong>Schema 是世界结构的投影，数据是世界状态的实例。</strong>现在可以查询“实际发生了什么”。</div>`,
rules:`<span class="visual-label">事实层和规则层描述的是同一个物流世界，但回答不同问题</span><div class="same-world"><div class="world-disc"><div class="route-cross"></div></div><div class="rule-beam"></div><div class="rule-overlay"><div class="rule-page"><b>线路 SLA</b>规定时效阈值与例外条件</div><div class="rule-page"><b>理赔规范</b>规定异常关闭和赔付条件</div><div class="rule-page"><b>运营流程</b>规定角色、动作与审批边界</div><div class="rules-stats"><strong>65</strong>份文档 · <strong>1,587</strong>chunks · <strong>176,139</strong>词</div></div></div><div class="center-note"><span style="color:var(--fact2)">事实：实际发生了什么</span>　≠　<span style="color:#cfbaff">规则：应该怎样运行</span></div>`,
questions:`<span class="visual-label">三类任务不是三套文件加工；它们是对同一个世界的三种观察</span><div class="task-world"><div><strong>同一个物流世界 W</strong>事实状态 + 制度规则</div></div><div class="task-beams"><i></i><i></i><i></i></div><div class="compare">示意判断：<span style="color:var(--fact2)">实际时效 T_actual</span> &gt; <span style="color:#cfbaff">SLA 阈值 T_sla</span> → <b>不合规</b></div><div class="task-cards"><div class="task-card fact-task"><small>只观察事实数据库</small><b>Step 5.1 数仓任务</b><p>问：<strong style="font-size:11px">实际发生了什么？</strong><br>查询真实状态、数量、趋势与关系</p><strong>555</strong></div><div class="task-card rule-task"><small>只观察制度文档</small><b>Step 5.2 知识任务</b><p>问：<strong style="font-size:11px">规则要求什么？</strong><br>从政策、SLA 与流程中找到依据</p><strong>465</strong></div><div class="task-card hybrid-task"><small>同时观察事实 + 规则</small><b>Step 5.3 混合任务</b><p>问：<strong style="font-size:11px">现实是否符合规则？</strong><br>把实际状态与制度阈值进行比较</p><strong>500</strong></div></div>`,
seal:`<span class="visual-label">世界可见，答案不可见：这是评测沙箱成立的边界</span><div class="seal-world"><div class="vault"><div class="zone-title"><small>EVALUATOR SIDE · 不给模型看</small>评测者保险库</div><div class="vault-items"><span class="chip locked">Tasks</span><span class="chip locked">Hidden Gold</span><span class="chip locked">校验和</span></div><div class="lock-note">🔒 任务与答案只用于评测，不进入 Runner</div></div><div class="firewall"><span>不可穿越<br>可见性边界</span></div><div class="runner-zone"><div class="zone-title"><small>RUNNER SIDE · 模型可观察</small>冻结的物流世界</div><div class="runner-items"><span class="chip visible-item">DB 事实</span><span class="chip visible-item">Docs 规则</span><span class="chip visible-item">Schema</span></div><div class="agent-orb">AGENT<br>观察 / 行动</div></div><div class="freeze-ring"></div></div><div class="center-note"><strong>冻结的是世界版本、身份和可见边界。</strong>智能体进入同一个可复现世界，但永远看不到预设答案。</div>`};
function renderNav(){el.nav.innerHTML=scenes.map((s,i)=>`<button type="button" class="cause ${i<state.scene?'done':i===state.scene?'current':''}" data-scene="${i}"><small>STEP ${s.number}</small><b>${navLabels[i]}</b><span>${esc(s.result_label)}</span></button>`).join('');el.nav.querySelectorAll('button').forEach(b=>b.addEventListener('click',()=>jump(Number(b.dataset.scene))))}
function renderLedger(){el.ledger.innerHTML=stages.map((s,i)=>{const files=s.outputs.map(x=>x.name).slice(0,2).join(' · ');return `<div class="ledger-row"><small>${String(i+1).padStart(2,'0')} · ${esc(s.step)}</small><b>${esc(s.title)}</b><span>${esc(s.duration)}<br>${esc(files)}</span></div>`}).join('')}
function render(){const s=scenes[state.scene];el.kicker.textContent=`STEP ${s.number} · ${navLabels[state.scene]}`;el.title.textContent=s.title;el.question.textContent=s.question;el.input.textContent=s.input_label;el.change.textContent=s.change_label;el.result.textContent=s.result_label;el.canvas.innerHTML=visuals[s.key];el.canvas.setAttribute('aria-label',`${s.title}。${s.question}`);el.progress.textContent=state.finished?'完成 · 6/6':`${state.scene+1}/6 · ${navLabels[state.scene]}`;el.live.textContent=state.finished?'✓ 沙箱生成完成':`正在展示：${s.change_label}`;el.fill.style.width=`${state.finished?100:((state.scene+1)/scenes.length)*100}%`;el.prev.disabled=state.finished||state.scene===0||state.transitioning;el.next.disabled=state.finished||state.transitioning;el.play.disabled=state.finished;el.play.textContent=state.finished?'已结束':state.playing?'暂停':'继续播放';el.next.textContent=state.scene===scenes.length-1?'完成生成':'下一步 →';renderNav()}
function clearTimer(){if(state.timer!==null){clearTimeout(state.timer);state.timer=null}}
function schedule(){clearTimer();if(!state.playing||state.finished||state.transitioning)return;state.timer=setTimeout(()=>{if(state.scene===scenes.length-1)finish();else transitionTo(state.scene+1)},scenes[state.scene].replay_ms/state.speed)}
function transitionTo(index){if(state.transitioning||state.finished)return;clearTimer();state.transitioning=true;const next=scenes[index];el.transitionStep.textContent=`STEP ${next.number} · ${navLabels[index]}`;el.transitionTitle.textContent=next.title;el.transitionText.textContent=`${scenes[state.scene].result_label}，成为下一步的输入。`;el.transition.classList.add('show');setTimeout(()=>{state.scene=index;state.transitioning=false;el.transition.classList.remove('show');render();schedule()},reduced?50:650/state.speed)}
function jump(index){clearTimer();state.scene=Math.max(0,Math.min(scenes.length-1,index));state.finished=false;state.transitioning=false;document.body.classList.remove('finished');el.end.classList.remove('show');el.transition.classList.remove('show');render();schedule()}
function finish(){clearTimer();state.finished=true;state.playing=false;state.transitioning=false;document.body.classList.add('finished');el.end.classList.add('show');render()}
function restart(){state.playing=true;jump(0)}
el.prev.addEventListener('click',()=>jump(state.scene-1));el.next.addEventListener('click',()=>{if(state.scene===scenes.length-1)finish();else transitionTo(state.scene+1)});el.play.addEventListener('click',()=>{if(state.finished)return;state.playing=!state.playing;render();schedule()});el.restart.addEventListener('click',restart);el.endRestart.addEventListener('click',restart);el.speed.addEventListener('change',()=>{state.speed=Number(el.speed.value);schedule()});document.addEventListener('keydown',e=>{if(e.key==='ArrowRight')el.next.click();if(e.key==='ArrowLeft')el.prev.click();if(e.key===' '&&!state.finished){e.preventDefault();el.play.click()}});renderLedger();render();requestAnimationFrame(schedule)})();</script>
</body></html>
'''
...
SPHERE_HTML_TEMPLATE = r'''<!doctype html>
<html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>物流运营沙箱 · 粒子世界生成实录</title>
<style>
:root{color-scheme:dark;--bg:#050b14;--panel:#081421;--panel2:#0b1a2a;--line:#1d3850;--text:#edf7ff;--muted:#88a1b8;--cyan:#61ddff;--blue:#42a5ff;--purple:#aa7cff;--gold:#ffc857;--green:#50e39c;--red:#ff7084}*{box-sizing:border-box}html{background:var(--bg)}body{margin:0;min-width:1180px;color:var(--text);background:radial-gradient(circle at 68% 30%,rgba(40,111,164,.12),transparent 38%),radial-gradient(circle at 86% 64%,rgba(126,75,203,.1),transparent 32%),var(--bg);font-family:Inter,"PingFang SC","Microsoft YaHei",system-ui,sans-serif}.app{min-height:100vh;padding:16px 24px 18px}.top{display:flex;justify-content:space-between;gap:30px;align-items:flex-start}.eyebrow{color:var(--cyan);font-size:10px;letter-spacing:.16em;margin-bottom:5px}.top h1{margin:0;font-size:26px;font-weight:600;letter-spacing:-.03em}.top p{margin:6px 0 0;color:var(--muted);font-size:12px}.run{padding-top:3px;text-align:right;color:#8ea7bd;font-size:10px}.run strong{display:block;color:#d8e9f6;font-size:12px;margin-bottom:3px}.controls{height:40px;margin-top:10px;display:flex;gap:7px;align-items:center}.btn,.speed{height:31px;padding:0 11px;border:1px solid #29445d;border-radius:6px;background:#091725;color:#d8e8f5;font:inherit;font-size:11px;cursor:pointer}.btn.primary{background:#dff5ff;color:#05101a;border-color:#dff5ff;font-weight:600}.btn:disabled{opacity:.35;cursor:not-allowed}.progress{margin-left:8px;flex:1}.progress-copy{display:flex;justify-content:space-between;color:#7891aa;font-size:9px;margin-bottom:4px}.progress-copy strong{color:#d5e5f2;font-weight:500}.track{height:3px;background:#13263a}.fill{height:100%;width:0;background:linear-gradient(90deg,var(--cyan),var(--blue),var(--purple),var(--gold),var(--green));transition:width .35s}
.architecture{height:82px;margin-top:4px;display:grid;grid-template-columns:1fr 34px 1fr 44px 1.42fr 44px 1fr 34px 1fr;align-items:center}.macro-node{min-height:52px;padding:7px 10px;border-top:1px solid #1a344b;border-bottom:1px solid #1a344b;background:rgba(7,18,30,.58);color:#637d94;transition:.3s}.macro-node small{display:block;font-size:8px;letter-spacing:.12em;margin-bottom:4px}.macro-node b{display:block;color:#8fa7bb;font-size:11px;font-weight:500}.macro-node span{display:block;margin-top:3px;font-size:8px}.macro-node.active{border-color:var(--cyan);background:linear-gradient(90deg,rgba(40,134,185,.15),rgba(11,27,44,.7));color:#8adfff}.macro-node.active b{color:#fff}.macro-node.done{border-color:rgba(80,227,156,.34);color:#72a992}.macro-node.done b{color:#b4d7c8}.macro-arrow{height:1px;background:#29465e;position:relative}.macro-arrow::after{content:"›";position:absolute;right:-2px;top:-11px;color:#527794;font-size:19px}.branch{display:grid;grid-template-columns:1fr;gap:4px}.branch .macro-node{min-height:26px;padding:4px 8px}.branch .macro-node small{display:inline;margin-right:6px}.branch .macro-node b{display:inline}.split{height:55px;border-top:1px solid #29465e;border-bottom:1px solid #29465e;border-left:1px solid #29465e}.merge{height:55px;border-top:1px solid #29465e;border-bottom:1px solid #29465e;border-right:1px solid #29465e}
.rail{display:grid;grid-template-columns:repeat(12,1fr);gap:4px;padding:4px 0 9px;border-top:1px solid #132538}.rail-step{min-width:0;padding:5px 4px;border:0;border-bottom:2px solid #183047;background:transparent;color:#577087;text-align:left;cursor:pointer}.rail-step small{display:block;font-size:8px}.rail-step b{display:block;margin-top:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;color:#7890a5;font-size:9px;font-weight:500}.rail-step.done{border-color:rgba(80,227,156,.42)}.rail-step.current{border-color:var(--cyan);background:linear-gradient(180deg,transparent,rgba(62,180,231,.08));color:#79ddff}.rail-step.current b{color:#fff}
.workspace{border-top:1px solid var(--line);border-bottom:1px solid var(--line);background:linear-gradient(180deg,rgba(8,20,33,.96),rgba(4,12,21,.98))}.step-head{min-height:68px;padding:11px 15px;display:grid;grid-template-columns:1fr minmax(360px,620px);gap:28px;border-bottom:1px solid #162d42}.step-token{color:var(--cyan);font-size:9px;letter-spacing:.12em}.step-title{display:block;margin-top:3px;font-size:19px;font-weight:600}.purpose{align-self:center;text-align:right;color:#aec1d1;font-size:11px;line-height:1.55}.purpose::before{content:"为什么做这一步";display:block;margin-bottom:2px;color:#607b94;font-size:8px;letter-spacing:.12em}.workspace-body{display:grid;grid-template-columns:310px minmax(0,1fr);min-height:476px}.actions{padding:13px 15px 11px;border-right:1px solid #162d42;background:rgba(6,16,27,.7)}.section-kicker{color:#627d95;font-size:8px;letter-spacing:.13em}.actions h2{margin:4px 0 11px;font-size:14px;font-weight:500}.op{position:relative;margin-bottom:6px;padding:7px 8px 7px 31px;color:#637c92;border-left:1px solid #233e56;transition:.25s}.op-mark{position:absolute;left:6px;top:7px;width:15px;height:15px;border:1px solid #31516b;border-radius:50%;display:grid;place-items:center;font-size:8px}.op b{display:block;color:#829aad;font-size:10px;font-weight:500}.op span:last-child{display:block;margin-top:2px;font-size:9px;line-height:1.4}.op.done{color:#709985;border-color:#2f7054}.op.done b{color:#add0bf}.op.done .op-mark{border-color:var(--green);color:var(--green)}.op.current{color:#c5d8e7;border-color:var(--cyan);background:linear-gradient(90deg,rgba(38,141,190,.14),transparent)}.op.current b{color:#fff}.op.current .op-mark{border-color:var(--cyan);color:var(--cyan);box-shadow:0 0 12px rgba(97,221,255,.35)}.op.error{border-color:var(--red)}.op.repair{border-color:var(--gold)}.active-detail{margin-top:10px;padding:9px 10px;background:rgba(13,31,49,.82);color:#b9cddd;font-size:10px;line-height:1.5}.active-detail strong{color:var(--cyan);font-weight:500}.handoff{margin-top:10px;color:#8ca4b8;font-size:9px;line-height:1.5}.handoff b{color:#c4d7e6;font-weight:500}.evidence{margin-top:12px;border-top:1px solid #172d41;padding-top:8px}.evidence summary{color:#6f899f;font-size:9px;cursor:pointer}.file-row{display:flex;flex-wrap:wrap;gap:5px;margin-top:7px}.file{padding:3px 5px;background:#0a1928;color:#7892a9;font:8px ui-monospace,SFMono-Regular,Consolas,monospace}
.world{position:relative;min-width:0;display:grid;grid-template-rows:1fr auto;background:radial-gradient(circle at 50% 48%,rgba(59,164,222,.065),transparent 33%)}.canvas-wrap{position:relative;min-height:430px;overflow:hidden}.world-title{position:absolute;left:16px;top:13px;z-index:2;color:#7390a8;font-size:9px;letter-spacing:.13em;pointer-events:none}.world-title b{display:block;margin-top:4px;color:#dceaf5;font-size:12px;font-weight:500;letter-spacing:0}.legend{position:absolute;right:15px;top:12px;z-index:2;display:flex;gap:10px;flex-wrap:wrap;justify-content:flex-end;max-width:470px;color:#8199ad;font-size:8px;pointer-events:none}.legend span{white-space:nowrap}.legend i{display:inline-block;width:6px;height:6px;border-radius:50%;margin-right:4px;vertical-align:1px}.legend .shared{background:var(--cyan)}.legend .fact{background:var(--blue)}.legend .rule{background:var(--purple)}.legend .task{background:var(--gold)}.legend .seal{background:var(--green)}#worldCanvas{display:block;width:100%;height:430px}.sphere-note{min-height:45px;padding:10px 16px;border-top:1px solid #162d42;display:flex;align-items:center;justify-content:space-between;gap:20px;color:#b8cad8;font-size:11px}.sphere-note strong{color:var(--cyan);font-weight:500}.world-count{color:#7791a8;font-size:9px;white-space:nowrap}.seal-badge{position:absolute;left:50%;bottom:12px;z-index:2;transform:translateX(-50%) translateY(8px);padding:6px 10px;background:rgba(6,21,19,.84);border:1px solid rgba(80,227,156,.48);color:#9df0c5;font-size:9px;letter-spacing:.1em;opacity:0;transition:.35s;pointer-events:none}.seal-badge.show{opacity:1;transform:translateX(-50%) translateY(0)}.end{display:none;position:absolute;left:16px;right:16px;bottom:12px;z-index:3;align-items:center;justify-content:space-between;gap:18px;padding:8px 11px;background:rgba(5,24,20,.92);border-left:2px solid var(--green);color:#a9dbc4;font-size:9px}.end.show{display:flex}.end strong{color:#eafff4;font-size:11px;font-weight:500}.end .btn{height:26px;color:#bff4d6;border-color:#327454;background:#081c18}.source{margin-top:8px;color:#566f85;font-size:8px;text-align:right}body.finished *{animation-play-state:paused!important}body.finished .fill{transition:none}
@media(max-width:1220px){.app{padding-left:16px;padding-right:16px}.workspace-body{grid-template-columns:292px minmax(0,1fr)}.architecture{grid-template-columns:1fr 26px 1fr 34px 1.38fr 34px 1fr 26px 1fr}.purpose{max-width:500px}.legend{max-width:400px}}
</style></head><body><main class="app"><header class="top"><div><div class="eyebrow">LOGISTICS WORLD CONSTRUCTION · VERIFIED V20 REPLAY</div><h1>一份物流 PRD，怎样生长成一个成熟世界？</h1><p>看同一个世界从一个种子持续分裂：结构变成事实，规则成为记忆，任务始终在世界之外观察它。</p></div><div class="run"><strong>真实场景 · 运营分析-8767b626</strong>13:15:20—14:35:51 · 生成耗时 80分31秒</div></header>
<div class="controls"><button id="restart" class="btn" type="button">重新播放</button><button id="prev" class="btn" type="button">← 上一阶段</button><button id="play" class="btn primary" type="button">暂停</button><button id="next" class="btn" type="button">下一动作 →</button><select id="speed" class="speed" aria-label="播放速度"><option value="1">1×</option><option value="1.5" selected>1.5×</option><option value="2.5">2.5×</option></select><div class="progress"><div class="progress-copy"><strong id="progressLabel">1/12</strong><span id="live" aria-live="polite">正在建立世界边界</span></div><div class="track" role="progressbar" aria-label="生成进度" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"><div id="progressFill" class="fill"></div></div></div></div>
<section class="architecture" aria-label="生成过程总结构"><div id="macro-understand" class="macro-node"><small>01</small><b>业务理解</b><span>PRD → 角色与目标</span></div><div class="macro-arrow"></div><div id="macro-model" class="macro-node"><small>02</small><b>世界建模</b><span>实体 + 状态 + 动作</span></div><div class="split"></div><div class="branch"><div id="macro-facts" class="macro-node"><small>03A</small><b>事实分支</b></div><div id="macro-rules" class="macro-node"><small>03B</small><b>规则分支</b></div></div><div class="merge"></div><div id="macro-tasks" class="macro-node"><small>04</small><b>事实 × 规则</b><span>三类外部观察任务</span></div><div class="macro-arrow"></div><div id="macro-seal" class="macro-node"><small>05</small><b>冻结交付</b><span>Runner / Raw 隔离</span></div></section>
<nav id="rail" class="rail" aria-label="十二个实际执行阶段"></nav>
<section class="workspace"><header class="step-head"><div><span id="stepToken" class="step-token"></span><strong id="stepTitle" class="step-title"></strong></div><div id="purpose" class="purpose"></div></header><div class="workspace-body"><aside class="actions"><span class="section-kicker">STAGE ACTIONS</span><h2>本阶段动作</h2><div id="operations"></div><div id="activeDetail" class="active-detail"></div><div id="handoff" class="handoff"></div><details class="evidence"><summary>查看真实落盘凭证（不参与主叙事）</summary><div id="files" class="file-row"></div></details></aside><section class="world" aria-label="沙箱世界生长画面"><div class="canvas-wrap"><div class="world-title">WORLD STATE<b>粒子世界 · 累积生长</b></div><div class="legend" aria-label="图例"><span><i class="shared"></i>共享世界模型</span><span><i class="fact"></i>事实世界</span><span><i class="rule"></i>规则世界</span><span><i class="task"></i>外部观察任务</span><span><i class="seal"></i>冻结边界</span></div><canvas id="worldCanvas" role="img" aria-label="物流沙箱从一个 PRD 粒子逐步生长为空间球体">浏览器需要支持 Canvas 才能播放粒子世界。</canvas><div id="sealBadge" class="seal-badge">RUNNER WORLD · VERSION SEALED</div><div id="end" class="end"><strong>✓ 物流运营沙箱生成完成 · 世界已冻结</strong><span>Runner 内：DB + Docs + Schema　｜　Raw 外：1,520 Tasks + Hidden Gold</span><button id="endRestart" class="btn" type="button">重新观看</button></div></div><div class="sphere-note"><span><strong>球体变化：</strong><span id="worldChange"></span></span><span id="worldStats" class="world-count" aria-live="polite"></span></div></section></div></section><div class="source">数量来自固定提交 7589170 的 v20 实际运行摘要；57 是实体类型，36,101 是实例化记录，口径不混用。</div></main>
<script id="stages" type="application/json">__STAGE_DATA__</script><script id="processSteps" type="application/json">__PROCESS_DATA__</script>
<script>(()=>{'use strict';const stages=JSON.parse(document.getElementById('stages').textContent),steps=JSON.parse(document.getElementById('processSteps').textContent),stageMap=Object.fromEntries(stages.map(s=>[s.key,s])),reduced=matchMedia('(prefers-reduced-motion: reduce)').matches;const state={step:0,op:-1,playing:true,finished:false,transitioning:false,speed:1.5,timer:null};const el={rail:document.getElementById('rail'),token:document.getElementById('stepToken'),title:document.getElementById('stepTitle'),purpose:document.getElementById('purpose'),ops:document.getElementById('operations'),active:document.getElementById('activeDetail'),handoff:document.getElementById('handoff'),files:document.getElementById('files'),restart:document.getElementById('restart'),prev:document.getElementById('prev'),play:document.getElementById('play'),next:document.getElementById('next'),speed:document.getElementById('speed'),progress:document.getElementById('progressLabel'),live:document.getElementById('live'),fill:document.getElementById('progressFill'),track:document.querySelector('.track'),canvas:document.getElementById('worldCanvas'),change:document.getElementById('worldChange'),stats:document.getElementById('worldStats'),seal:document.getElementById('sealBadge'),end:document.getElementById('end')};const esc=s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
const CHANGES={intake:'一颗 PRD 种子被放入独立场景；它是后续全部世界内容的共同来源。',prd:'种子分裂成 5 类运营角色与业务目标，世界第一次知道“谁在观察、为何观察”。',factor:'角色需求继续分裂为 57 类实体、76 个状态与 80 个动作，形成可运行的世界骨架。',taxonomy:'骨架展开 15 条情境轨道、115 个采样单元与 27 条组合约束，世界能变化但不会失真。',schema:'蓝色事实半球长出 64 个表锚点，每个世界对象与关系开始拥有稳定存储位置。',data:'64 个表锚点释放 36,101 个实例粒子，并以 300 条显式关系把事实世界连接起来。',dwh_tasks:'555 个金色探针离开事实半球，成为只观察“实际发生了什么”的数仓任务。',catalog:'同一世界骨架向右分裂出 65 个规则锚点，事实与规则保持同源。',documents:'65 个规则锚点释放 1,587 个证据粒子，形成可检索、可追溯的制度记忆。',kb_tasks:'465 个金色探针离开规则半球，成为只观察“应该怎样运行”的知识任务。',hybrid:'500 个双源探针跨越两个半球：171 个事实→规则、156 个规则→事实、173 个合规判断。',freeze:'绿色冻结壳只封装 Runner 可见世界；1,520 个任务与 Hidden Gold 永久留在壳外。'};
const PHASES={understand:['understand'],model:['model'],facts:['facts'],rules:['rules'],tasks:['tasks'],seal:['seal']};function renderMacros(){for(const id of ['understand','model','facts','rules','tasks','seal']){const n=document.getElementById(`macro-${id}`),active=PHASES[id].includes(steps[state.step].phase),done=(id==='understand'&&state.step>1)||(id==='model'&&state.step>3)||(id==='facts'&&state.step>6)||(id==='rules'&&state.step>9)||(id==='tasks'&&state.step>10)||(id==='seal'&&state.finished);n.className=`macro-node ${active?'active':done?'done':''}`}}
function renderRail(){el.rail.innerHTML=steps.map((s,i)=>`<button type="button" class="rail-step ${i<state.step?'done':i===state.step?'current':''}" data-step="${i}"><small>${s.number}</small><b>${esc(s.label)}</b></button>`).join('');el.rail.querySelectorAll('button').forEach(b=>b.addEventListener('click',()=>jump(Number(b.dataset.step),true)))}
const TAU=Math.PI*2,particles=[],categories=new Map(),birthTimes=new Map();let seedValue=7531;function rnd(){seedValue=(seedValue*1664525+1013904223)>>>0;return seedValue/4294967296}function fib(i,n,r=1,hemi=0){const y=1-2*(i+.5)/n,a=i*2.399963229728653,x=Math.cos(a)*Math.sqrt(1-y*y),z=Math.sin(a)*Math.sqrt(1-y*y);return{x:hemi<0?-Math.abs(x)*r:hemi>0?Math.abs(x)*r:x*r,y:y*r,z:z*r}}function add(kind,count,born,unlock,target,origin,size=1){const start=particles.length;for(let i=0;i<count;i++){const t=target(i,count),o=origin(i,count,t);particles.push({kind,born,unlock,x:t.x,y:t.y,z:t.z,ox:o.x,oy:o.y,oz:o.z,size:size*(.82+rnd()*.36)})}categories.set(kind,{start,count,born,unlock})}
const center=()=>({x:0,y:0,z:0});add('seed',1,0,-1,()=>center(),()=>center(),8);add('roles',5,1,0,(i,n)=>{const a=TAU*i/n;return{x:Math.cos(a)*.22,y:Math.sin(a)*.22,z:(i-2)*.035}},()=>center(),4.4);add('entities',57,2,0,(i,n)=>fib(i,n,.48,0),(i)=>particles[categories.get('roles').start+i%5],2.15);add('states',76,2,1,(i,n)=>{const p=fib(i,n,.58);return{x:p.x,y:p.y,z:p.z}},(i)=>particles[categories.get('entities').start+i%57],1.7);add('actions',80,2,1,(i,n)=>{const p=fib(i,n,.65);return{x:p.x,y:p.y,z:p.z}},(i)=>particles[categories.get('entities').start+(i*7)%57],1.65);add('taxonomy',115,3,1,(i,n)=>{const p=fib(i,n,.75);return{x:p.x,y:p.y,z:p.z}},(i)=>particles[categories.get(i%2?'states':'actions').start+i%(i%2?76:80)],1.25);add('tables',64,4,1,(i,n)=>{const p=fib(i,n,.78,-1);return{x:p.x-.055,y:p.y,z:p.z}},(i)=>particles[categories.get('entities').start+i%57],2.45);add('facts',36101,5,1,(i,n)=>{const p=fib(i,n,.76+rnd()*.2,-1);return{x:p.x-.055,y:p.y,z:p.z}},(i)=>particles[categories.get('tables').start+i%64],.72);add('dwh',555,6,3,(i,n)=>{const a=TAU*i/n*11,b=TAU*i/n,y=Math.sin(a)*.84,z=Math.cos(b)*.64;return{x:-Math.sqrt(1.22*1.22-y*y-z*z),y,z}},(i)=>particles[categories.get('facts').start+(i*61)%36101],1.22);add('docanchors',65,7,1,(i,n)=>{const p=fib(i,n,.78,1);return{x:p.x+.055,y:p.y,z:p.z}},(i)=>particles[categories.get('entities').start+i%57],2.45);add('chunks',1587,8,1,(i,n)=>{const p=fib(i,n,.76+rnd()*.2,1);return{x:p.x+.055,y:p.y,z:p.z}},(i)=>particles[categories.get('docanchors').start+i%65],.95);add('kb',465,9,1,(i,n)=>{const a=TAU*i/n*9,b=TAU*i/n,y=Math.sin(a)*.84,z=Math.sin(b)*.64;return{x:Math.sqrt(1.22*1.22-y*y-z*z),y,z}},(i)=>particles[categories.get('chunks').start+(i*37)%1587],1.25);add('hybridA',171,10,0,(i,n)=>{const a=TAU*i/n;return{x:Math.cos(a)*1.18,y:Math.sin(a*3)*.34,z:Math.sin(a)*1.18}},()=>center(),1.45);add('hybridB',156,10,1,(i,n)=>{const a=TAU*(i+.31)/n;return{x:Math.cos(a)*1.27,y:Math.sin(a*2)*.42,z:Math.sin(a)*1.27}},()=>center(),1.45);add('hybridC',173,10,2,(i,n)=>{const a=TAU*(i+.67)/n;return{x:Math.cos(a)*1.36,y:Math.sin(a*5)*.29,z:Math.sin(a)*1.36}},()=>center(),1.45);
const colors={seed:'#edf7ff',roles:'#78e5ff',entities:'#69dcff',states:'#c3f4ff',actions:'#53c8e8',taxonomy:'#80d8ed',tables:'#55b8ff',facts:'#389cff',dwh:'#ffd067',docanchors:'#b68cff',chunks:'#9b70f7',kb:'#ffd067',hybridA:'#ffe39a',hybridB:'#ffc857',hybridC:'#ffb53e'};let ctx=el.canvas.getContext('2d'),cw=0,ch=0,dpr=1,worldRaf=0,lastFrame=0,worldAngle=-.22,frozenAngle=-.22;function resize(){const rect=el.canvas.getBoundingClientRect();dpr=Math.min(devicePixelRatio||1,2);cw=rect.width;ch=rect.height;el.canvas.width=Math.round(cw*dpr);el.canvas.height=Math.round(ch*dpr);ctx.setTransform(dpr,0,0,dpr,0,0);drawWorld(performance.now(),true)}new ResizeObserver(resize).observe(el.canvas);
function visible(p){return p.born<state.step||(p.born===state.step&&state.op>=p.unlock)||state.finished}function eased(p,now){if(reduced||p.born<state.step||state.finished)return 1;const key=`${p.kind}:${p.born}:${p.unlock}`,born=birthTimes.get(key);if(born===undefined)return 1;const x=Math.min(1,Math.max(0,(now-born)/1050));return 1-Math.pow(1-x,3)}function markBirths(){for(const c of categories.values()){if(c.born===state.step&&state.op>=c.unlock){const key=`${particles[c.start].kind}:${c.born}:${c.unlock}`;if(!birthTimes.has(key))birthTimes.set(key,performance.now())}}}
function project(x,y,z,angle,scale,cx,cy){const ca=Math.cos(angle),sa=Math.sin(angle),x1=x*ca-z*sa,z1=x*sa+z*ca,tilt=-.16,ct=Math.cos(tilt),st=Math.sin(tilt),y1=y*ct-z1*st,z2=y*st+z1*ct,p=1/(1.9-z2*.24);return{x:cx+x1*scale*p,y:cy+y1*scale*p,z:z2,p}}
function line3(a,b,angle,scale,cx,cy,color,alpha=.22,width=1){const p=project(a.x,a.y,a.z,angle,scale,cx,cy),q=project(b.x,b.y,b.z,angle,scale,cx,cy);ctx.strokeStyle=color;ctx.globalAlpha=alpha;ctx.lineWidth=width;ctx.beginPath();ctx.moveTo(p.x,p.y);ctx.lineTo(q.x,q.y);ctx.stroke()}
function ring3(axis,r,angle,scale,cx,cy,color,alpha=.13){ctx.strokeStyle=color;ctx.globalAlpha=alpha;ctx.lineWidth=.7;ctx.beginPath();for(let i=0;i<=72;i++){const a=TAU*i/72;let p=axis===0?{x:0,y:Math.cos(a)*r,z:Math.sin(a)*r}:axis===1?{x:Math.cos(a)*r,y:0,z:Math.sin(a)*r}:{x:Math.cos(a)*r,y:Math.sin(a)*r,z:0},q=project(p.x,p.y,p.z,angle,scale,cx,cy);i?ctx.lineTo(q.x,q.y):ctx.moveTo(q.x,q.y)}ctx.stroke()}
function tag(text,x,y,accent,align='left'){ctx.font='500 11px Inter,"Microsoft YaHei",sans-serif';const w=ctx.measureText(text).width+18,h=23,left=align==='right'?x-w:x;ctx.globalAlpha=.92;ctx.fillStyle='rgba(5,14,24,.82)';ctx.fillRect(left,y-h/2,w,h);ctx.fillStyle=accent;ctx.fillRect(align==='right'?x-3:left,y-h/2,3,h);ctx.fillStyle='#e5f2fb';ctx.textAlign=align;ctx.textBaseline='middle';ctx.fillText(text,align==='right'?x-9:left+9,y);ctx.globalAlpha=1}
function labels(){const key=steps[state.step].key,op=state.op;if(state.step===0)tag('PRD · 1 个世界种子',cw*.5,ch*.5-46,'#edf7ff','left');if(state.step>=1)tag('共享模型 · 5 类运营角色',cw*.5,ch*.16,'#61ddff','left');if(state.step>=2){tag('57 类实体',cw*.5-105,ch*.34,'#61ddff','right');tag('76 状态 · 80 动作',cw*.5+112,ch*.39,'#61ddff','left')}if(state.step>=3){tag('15 条情境轨道 · 115 采样单元',cw*.5,ch*.82,'#80d8ed','left');if(state.step>3||op>=2)tag('27 条组合约束',cw*.5,ch*.25,'#50e39c','left')}if(state.step>=4)tag(state.step>=5?'事实半球 · 64 表 · 36,101 实例':'事实半球 · 64 个表锚点',cw*.08,ch*.36,'#42a5ff','left');if(state.step>=6)tag('555 数仓任务 · 观察事实',cw*.06,ch*.68,'#ffc857','left');if(state.step>=7)tag(state.step>=8?'规则半球 · 65 文档 · 1,587 证据块':'规则半球 · 65 个规则锚点',cw*.94,ch*.36,'#aa7cff','right');if(state.step>=9)tag('465 知识任务 · 观察规则',cw*.94,ch*.68,'#ffc857','right');if(state.step>=10)tag('500 混合任务 · 比较事实与规则',cw*.5,ch*.91,'#ffc857','left');if(state.step===11&&(op>=1||state.finished)){tag('壳内 RUNNER · DB + Docs + Schema',cw*.5,ch*.56,'#50e39c','left');tag('壳外 RAW · 1,520 Tasks + Hidden Gold',cw*.5,ch*.08,'#ffc857','left')}}
function drawWorld(now,force=false){if(!ctx||!cw)return;const scale=Math.min(cw*.31,ch*.40),cx=cw*.54,cy=ch*.51;if(state.playing&&!state.finished&&!reduced&&lastFrame)worldAngle+=(now-lastFrame)*.000025;lastFrame=now;if(state.finished)worldAngle=frozenAngle;ctx.clearRect(0,0,cw,ch);ctx.globalAlpha=1;const gradient=ctx.createRadialGradient(cx,cy,0,cx,cy,scale*1.2);gradient.addColorStop(0,'rgba(42,118,163,.09)');gradient.addColorStop(.72,'rgba(39,78,112,.025)');gradient.addColorStop(1,'rgba(0,0,0,0)');ctx.fillStyle=gradient;ctx.fillRect(0,0,cw,ch);if(state.step>=3){for(let i=0;i<15;i++)ring3(i%3,.70+(i%5)*.045,worldAngle+(i%4)*.08,scale,cx,cy,'#69d7ef',.035+(i%3)*.018)}if(state.step>3||(state.step===3&&state.op>=2)){const e=categories.get('entities'),a=categories.get('actions');for(let i=0;i<27;i++)line3(particles[e.start+(i*11)%57],particles[a.start+(i*17)%80],worldAngle,scale,cx,cy,'#50e39c',.13,.8)}if(state.step>5||(state.step===5&&state.op>=2)){const f=categories.get('facts');for(let i=0;i<300;i++)line3(particles[f.start+(i*97)%36101],particles[f.start+(i*173+113)%36101],worldAngle,scale,cx,cy,'#63c4ff',.025,.45)}if(state.step>9||(state.step===9&&state.op>=1)){const d=categories.get('chunks');for(let i=0;i<374;i++)line3(particles[d.start+(i*19)%1587],particles[d.start+(i*43+67)%1587],worldAngle,scale,cx,cy,'#b68cff',.045,.5)}if(state.step===11&&(state.op>=1||state.finished)){ctx.strokeStyle='#50e39c';ctx.globalAlpha=.7;ctx.lineWidth=1.4;ctx.beginPath();ctx.ellipse(cx,cy,scale*.62,scale*.58,0,0,TAU);ctx.stroke();ctx.globalAlpha=.1;ctx.fillStyle='#50e39c';ctx.beginPath();ctx.ellipse(cx,cy,scale*.62,scale*.58,0,0,TAU);ctx.fill()}const batches=new Map;let visibleCount=0;for(const p of particles){if(!visible(p))continue;visibleCount++;const t=eased(p,now),x=p.ox+(p.x-p.ox)*t,y=p.oy+(p.y-p.oy)*t,z=p.oz+(p.z-p.oz)*t,q=project(x,y,z,worldAngle,scale,cx,cy);if(!batches.has(p.kind))batches.set(p.kind,[]);batches.get(p.kind).push([q.x,q.y,q.z,q.p,p.size])}for(const [kind,pts] of batches){ctx.fillStyle=colors[kind];const micro=kind==='facts'||kind==='chunks';ctx.globalAlpha=micro?(kind==='facts'?.43:.55):(kind==='dwh'||kind==='kb'||kind.startsWith('hybrid')?.88:.8);if(micro){for(const p of pts){const s=Math.max(.55,p[4]*p[3]);ctx.fillRect(p[0],p[1],s,s)}}else{for(const p of pts){const s=Math.max(.8,p[4]*p[3]);ctx.beginPath();ctx.arc(p[0],p[1],s,0,TAU);ctx.fill()}}}ctx.globalAlpha=1;labels();el.canvas.dataset.visibleParticles=String(visibleCount);el.canvas.setAttribute('aria-label',`物流沙箱粒子世界，当前可见 ${visibleCount.toLocaleString('zh-CN')} 个粒子`);if(force||!state.finished){} }
function worldLoop(now){drawWorld(now);if(!state.finished&&!reduced)worldRaf=requestAnimationFrame(worldLoop)}function startWorldLoop(){if(reduced){drawWorld(performance.now(),true);return}if(!worldRaf)worldRaf=requestAnimationFrame(worldLoop)}function stopWorldLoop(){if(worldRaf){cancelAnimationFrame(worldRaf);worldRaf=0}frozenAngle=worldAngle;drawWorld(performance.now(),true)}
function currentCounts(){let n=0;for(const p of particles)if(visible(p))n++;return n}function render(){const view=steps[state.step],stage=stageMap[view.key],complete=state.op>=stage.operations.length-1;markBirths();el.token.textContent=`实际执行 ${state.step+1}/12 · ${stage.step} · ${stage.branch} · ${stage.duration}`;el.title.textContent=view.label;el.purpose.textContent=view.purpose;el.ops.innerHTML=stage.operations.map((op,i)=>`<div class="op ${i<state.op?'done':i===state.op?'current':''} ${op.status==='error'?'error':op.status==='repair'?'repair':''}"><span class="op-mark">${i<state.op?'✓':i+1}</span><b>${esc(op.label)}</b><span>${esc(op.outcome)}</span></div>`).join('');const activeOp=state.op>=0?stage.operations[state.op]:null;el.active.innerHTML=activeOp?`<strong>正在发生：</strong>${esc(activeOp.detail)}`:'<strong>准备开始：</strong>上一阶段形成的粒子保持原位，本阶段只增加新的世界内容。';const next=steps[state.step+1];el.handoff.innerHTML=complete?`<b>本阶段闭合：</b>${esc(stage.handoff)}${next?`<br>接下来，球体继续生长为「${esc(next.label)}」。`:''}`:'<b>因果规则：</b>只有当前动作完成，新粒子才会成为下一动作可使用的世界内容。';el.files.innerHTML=[...stage.inputs.map(x=>`来源 · ${x.name}`),...stage.outputs.map(x=>`落盘 · ${x.name}`)].map(x=>`<span class="file">${esc(x)}</span>`).join('');el.change.textContent=CHANGES[view.key];const count=currentCounts();el.stats.textContent=`当前可见 ${count.toLocaleString('zh-CN')} 个世界粒子`;const frac=(state.op+1)/stage.operations.length,pct=((state.step+Math.max(0,frac))/steps.length)*100;el.progress.textContent=state.finished?'完成 · 12/12':`${state.step+1}/12 · ${view.label}`;el.live.textContent=state.finished?'✓ 世界冻结，生成结束':activeOp?`正在执行：${activeOp.label}`:'等待本阶段第一个动作';el.fill.style.width=`${state.finished?100:Math.max(1,pct)}%`;el.track.setAttribute('aria-valuenow',String(Math.round(state.finished?100:pct)));el.prev.disabled=state.finished||state.step===0||state.transitioning;el.next.disabled=state.finished||state.transitioning;el.play.disabled=state.finished;el.play.textContent=state.finished?'已结束':state.playing?'暂停':'继续播放';el.next.textContent=complete?(state.step===steps.length-1?'完成生成':'进入下一阶段 →'):'下一动作 →';el.seal.classList.toggle('show',state.step===11&&(state.op>=1||state.finished));renderMacros();renderRail();drawWorld(performance.now(),true);startWorldLoop()}
function clearTimer(){if(state.timer!==null){clearTimeout(state.timer);state.timer=null}}function schedule(){clearTimer();if(!state.playing||state.finished||state.transitioning)return;const stage=stageMap[steps[state.step].key],delay=state.op<0?560:steps[state.step].replay_ms;state.timer=setTimeout(tick,delay/state.speed)}function tick(){const stage=stageMap[steps[state.step].key];if(state.op<stage.operations.length-1){state.op++;render();schedule()}else if(state.step<steps.length-1){advanceStage()}else finish()}function advanceStage(){clearTimer();state.transitioning=true;setTimeout(()=>{state.step++;state.op=-1;state.transitioning=false;render();schedule()},reduced?20:310/state.speed)}function jump(index,pause=false){clearTimer();birthTimes.clear();state.step=Math.max(0,Math.min(steps.length-1,index));state.op=-1;state.finished=false;state.transitioning=false;if(pause)state.playing=false;document.body.classList.remove('finished');el.end.classList.remove('show');el.seal.classList.remove('show');render();schedule()}function finish(){clearTimer();state.finished=true;state.playing=false;document.body.classList.add('finished');el.end.classList.add('show');el.seal.classList.add('show');render();stopWorldLoop()}function restart(){birthTimes.clear();state.playing=true;jump(0)}el.prev.addEventListener('click',()=>jump(state.step-1,true));el.next.addEventListener('click',()=>{const stage=stageMap[steps[state.step].key];if(state.op<stage.operations.length-1){state.op++;render()}else if(state.step<steps.length-1){advanceStage()}else finish();schedule()});el.play.addEventListener('click',()=>{if(state.finished)return;state.playing=!state.playing;render();schedule()});el.restart.addEventListener('click',restart);document.getElementById('endRestart').addEventListener('click',restart);el.speed.addEventListener('change',()=>{state.speed=Number(el.speed.value);schedule()});document.addEventListener('keydown',e=>{if(e.key==='ArrowRight')el.next.click();if(e.key==='ArrowLeft')el.prev.click();if(e.key===' '&&!state.finished){e.preventDefault();el.play.click()}});render();requestAnimationFrame(()=>resize());schedule()})();</script></body></html>
'''
...
PPT_DECK_HTML_TEMPLATE = r'''<!doctype html>
<html lang="zh-CN">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>物流运营沙箱生成过程 · 分页讲解</title>
<style>
:root{color-scheme:light;--red:#e92331;--red-soft:#fff0f1;--ink:#17191c;--muted:#687078;--line:#d9dde0;--soft:#f6f7f7;--green:#228558;--green-soft:#edf7f1;--amber:#8d611d;--amber-soft:#fbf4e6}
*{box-sizing:border-box}html,body{width:100%;height:100%;margin:0;overflow:hidden;background:transparent;color:var(--ink);font-family:Inter,"PingFang SC","Microsoft YaHei",system-ui,sans-serif}button,select{font:inherit}button:focus-visible,select:focus-visible{outline:3px solid rgba(233,35,49,.28);outline-offset:2px}
body{display:grid;place-items:center}.deck{container-type:size;width:min(100vw,calc(100vh * 4 / 3));height:min(100vh,calc(100vw * 3 / 4));aspect-ratio:4/3;overflow:hidden;background:#fff;padding:2.3cqw 2.5cqw 1.7cqw;display:grid;grid-template-rows:auto auto auto minmax(0,1fr) auto auto auto;gap:1.05cqw}
.head{display:grid;grid-template-columns:minmax(0,1fr) auto;gap:2cqw;align-items:start}.kicker{color:var(--red);font-size:1.3cqw;font-weight:700;letter-spacing:.08em}.title{margin:.35cqw 0 0;font-size:2.65cqw;line-height:1.18;font-weight:700;letter-spacing:-.025em}.page-no{text-align:right;font-size:1.15cqw;color:var(--muted)}.page-no b{display:block;color:var(--ink);font-size:2.5cqw;line-height:1}.intro{margin:0;color:var(--muted);font-size:1.35cqw;line-height:1.45;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.rail{display:grid;grid-template-columns:repeat(12,1fr);gap:.45cqw}.rail button{min-width:0;padding:.5cqw .2cqw;border:0;border-top:.22cqw solid var(--line);background:transparent;color:#9aa0a5;font-size:.86cqw;text-align:left;cursor:pointer;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.rail button.current{border-color:var(--red);background:var(--red-soft);color:var(--ink);font-weight:700}.rail button.done{border-color:var(--green);color:#527263}
.flow{min-height:0;display:flex;align-items:stretch;gap:0;padding:.35cqw 0;overflow:hidden}.node{flex:1 1 0;min-width:0;display:flex;flex-direction:column;opacity:.13;transition:opacity .24s ease,transform .24s ease;transform:translateY(.4cqw)}.node.current{opacity:1;transform:none}.node.done{opacity:.72;transform:none}.node-label{min-height:2.2cqw;color:var(--muted);font-size:1.12cqw;font-weight:700;letter-spacing:.05em}.node-card{height:100%;min-height:0;overflow:hidden;border-top:.42cqw solid #aeb4b8;background:var(--soft);padding:1.05cqw 1.05cqw .85cqw;display:flex;flex-direction:column}.node.current .node-card{border-color:var(--red);background:var(--red-soft)}.node.done .node-card{border-color:var(--green);background:var(--green-soft)}.node-card h2{margin:0;font-size:1.7cqw;line-height:1.28}.code{margin-top:.35cqw;color:var(--red);font:1.12cqw "Cascadia Mono",Consolas,monospace;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.items{display:grid;margin-top:.65cqw}.item{padding:.55cqw 0;border-top:1px solid var(--line);font-size:1.25cqw;line-height:1.28}.detail{margin:.65cqw 0 0;color:#4c5359;font-size:1.12cqw;line-height:1.36}.source{margin-top:auto;padding-top:.55cqw;color:#8a9196;font:.92cqw "Cascadia Mono",Consolas,monospace;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.arrow{flex:0 0 5.5cqw;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:.3cqw;text-align:center;opacity:.13;transition:opacity .2s}.arrow.active{opacity:1}.arrow span{color:var(--red);font-size:1.03cqw;line-height:1.2}.arrow b{color:#9da3a7;font-size:2.2cqw;line-height:1}
.scale{display:grid;grid-template-columns:13cqw minmax(0,1fr);gap:1cqw;padding:.8cqw 1cqw;border-top:.25cqw solid var(--ink);font-size:1.22cqw;line-height:1.38}.scale b{color:var(--red)}.why{padding:.72cqw 1cqw;background:var(--amber-soft);font-size:1.12cqw;line-height:1.35;color:#5c4a2a;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.why b{color:var(--amber)}
.controls{display:flex;align-items:center;gap:.6cqw}.control{height:2.8cqw;min-height:24px;padding:0 1cqw;border:1px solid #aeb4b8;border-radius:.35cqw;background:#fff;color:var(--ink);font-size:1.08cqw;cursor:pointer}.control.primary{border-color:var(--red);background:var(--red);color:#fff}.control:disabled{opacity:.35;cursor:default}.progress{flex:1;margin-left:.45cqw}.progress-copy{display:flex;justify-content:space-between;margin-bottom:.25cqw;color:var(--muted);font-size:.9cqw}.track{height:.32cqw;min-height:3px;background:#e5e7e8;overflow:hidden}.fill{height:100%;width:0;background:var(--red);transition:width .25s}.complete{display:none;margin-left:auto;color:var(--green);font-size:1.15cqw;font-weight:700}.complete.show{display:block}.footnote{color:#9aa0a5;font-size:.82cqw;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
@media(prefers-reduced-motion:reduce){*{transition:none!important}}
</style>
</head>
<body>
<main class="deck" id="deck">
  <header class="head"><div><div class="kicker" id="kicker"></div><h1 class="title" id="title"></h1></div><div class="page-no"><b id="pageNo"></b><span>真实生成步骤</span></div></header>
  <p class="intro" id="intro"></p>
  <nav class="rail" id="rail" aria-label="沙箱生成分页"></nav>
  <section class="flow" id="flow" aria-live="polite"></section>
  <div class="scale"><b>同样规则批量展开</b><span id="scale"></span></div>
  <div class="why"><b>→ 下一页为什么能继续：</b><span id="why"></span></div>
  <div class="controls"><button class="control" id="restart" type="button">重新播放</button><button class="control" id="prev" type="button">← 上一页</button><button class="control primary" id="play" type="button">暂停</button><button class="control" id="next" type="button">下一项 →</button><label><span style="position:absolute;clip:rect(0,0,0,0)">播放速度</span><select class="control" id="speed" aria-label="播放速度"><option value="0.75">0.75×</option><option value="1" selected>1×</option><option value="1.5">1.5×</option><option value="2">2×</option></select></label><div class="progress"><div class="progress-copy"><span id="progressText"></span><span id="live" aria-live="polite"></span></div><div class="track" id="track" role="progressbar" aria-label="讲解进度" aria-valuemin="0" aria-valuemax="100" aria-valuenow="0"><div class="fill" id="fill"></div></div></div><span class="complete" id="complete">✓ 沙箱生成完成</span></div>
  <div class="footnote">固定 v20 场景 运营分析-8767b626 · 真实字段、记录、制度与任务样本 · 总耗时 80:31</div>
</main>
<script id="stages" type="application/json">__STAGE_DATA__</script><script id="processSteps" type="application/json">__PROCESS_DATA__</script><script id="exampleCases" type="application/json">__EXAMPLE_DATA__</script>
<script>(()=>{'use strict';
const stages=JSON.parse(document.getElementById('stages').textContent),steps=JSON.parse(document.getElementById('processSteps').textContent),cases=JSON.parse(document.getElementById('exampleCases').textContent),stageMap=Object.fromEntries(stages.map(x=>[x.key,x])),caseMap=Object.fromEntries(cases.map(x=>[x.key,x])),shortNames=['输入','业务理解','世界模型','情境空间','表结构','真实数据','数仓任务','知识目录','制度文档','知识任务','混合任务','冻结交付'],reduced=matchMedia('(prefers-reduced-motion:reduce)').matches;
const state={page:0,beat:0,playing:true,finished:false,transitioning:false,speed:1,timer:null};const $=id=>document.getElementById(id),el={kicker:$('kicker'),title:$('title'),pageNo:$('pageNo'),intro:$('intro'),rail:$('rail'),flow:$('flow'),scale:$('scale'),why:$('why'),restart:$('restart'),prev:$('prev'),play:$('play'),next:$('next'),speed:$('speed'),progress:$('progressText'),live:$('live'),track:$('track'),fill:$('fill'),complete:$('complete')};const esc=s=>String(s).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
function renderRail(){el.rail.innerHTML=steps.map((s,i)=>`<button type="button" class="${i<state.page?'done':i===state.page?'current':''}" data-page="${i}">${String(i+1).padStart(2,'0')} ${shortNames[i]}</button>`).join('');el.rail.querySelectorAll('button').forEach(b=>b.addEventListener('click',()=>jump(Number(b.dataset.page),true)))}
function renderFlow(scene){el.flow.innerHTML=scene.nodes.map((node,i)=>{const status=i<state.beat?'done':i===state.beat?'current':'',items=node.items.slice(0,4).map(x=>`<span class="item">${esc(x)}</span>`).join(''),arrow=node.arrow?`<div class="arrow ${i<=state.beat?'active':''}"><span>${esc(node.arrow)}</span><b>→</b></div>`:'';return `<article class="node ${status}"><div class="node-label">${esc(node.label)}</div><div class="node-card"><h2>${esc(node.title)}</h2><div class="code">${esc(node.code)}</div><div class="items">${items}</div><p class="detail">${esc(node.detail)}</p><div class="source">证据 · ${esc(node.source)}</div></div></article>${arrow}`}).join('')}
function render(){const view=steps[state.page],stage=stageMap[view.key],scene=caseMap[view.key],total=cases.reduce((n,x)=>n+x.nodes.length,0),done=cases.slice(0,state.page).reduce((n,x)=>n+x.nodes.length,0)+state.beat+1,pct=state.finished?100:Math.round(done/total*100);el.kicker.textContent=`${stage.step} · ${stage.branch} · 一个真实例子`;el.title.textContent=scene.title;el.pageNo.textContent=`${String(state.page+1).padStart(2,'0')} / 12`;el.intro.textContent=scene.intro;renderFlow(scene);el.scale.textContent=scene.scale;el.why.textContent=scene.why;el.progress.textContent=`${shortNames[state.page]} · ${state.beat+1}/${scene.nodes.length}`;el.live.textContent=state.finished?'播放已停止':`当前：${scene.nodes[state.beat].label}`;el.fill.style.width=`${pct}%`;el.track.setAttribute('aria-valuenow',String(pct));el.prev.disabled=state.finished||state.page===0||state.transitioning;el.next.disabled=state.finished||state.transitioning;el.play.disabled=state.finished;el.play.textContent=state.finished?'已结束':state.playing?'暂停':'继续';el.next.textContent=state.beat<scene.nodes.length-1?'下一项 →':state.page<steps.length-1?'下一页 →':'完成';el.complete.classList.toggle('show',state.finished);renderRail()}
function clearTimer(){if(state.timer!==null){clearTimeout(state.timer);state.timer=null}}function schedule(){clearTimer();if(!state.playing||state.finished||state.transitioning)return;const scene=caseMap[steps[state.page].key],delay=(reduced?120:4200)/state.speed;state.timer=setTimeout(()=>{if(state.beat<scene.nodes.length-1){state.beat++;render();schedule()}else if(state.page<steps.length-1){advance()}else finish()},delay)}function advance(){clearTimer();state.transitioning=true;setTimeout(()=>{state.page++;state.beat=0;state.transitioning=false;render();schedule()},reduced?20:260/state.speed)}function jump(index,pause=false){clearTimer();state.page=Math.max(0,Math.min(steps.length-1,index));state.beat=0;state.finished=false;state.transitioning=false;if(pause)state.playing=false;render();schedule()}function finish(){clearTimer();state.page=steps.length-1;state.beat=caseMap.freeze.nodes.length-1;state.finished=true;state.playing=false;render()}function restart(){state.playing=true;jump(0)}
el.restart.addEventListener('click',restart);el.prev.addEventListener('click',()=>jump(state.page-1,true));el.next.addEventListener('click',()=>{if(state.finished)return;const scene=caseMap[steps[state.page].key];if(state.beat<scene.nodes.length-1){state.beat++;render()}else if(state.page<steps.length-1){advance()}else finish();schedule()});el.play.addEventListener('click',()=>{if(state.finished)return;state.playing=!state.playing;render();schedule()});el.speed.addEventListener('change',()=>{state.speed=Number(el.speed.value);schedule()});document.addEventListener('keydown',e=>{if(e.key==='ArrowRight')el.next.click();if(e.key==='ArrowLeft')el.prev.click();if(e.key===' '&&!state.finished){e.preventDefault();el.play.click()}});render();schedule()})();</script>
</body></html>
'''
...
def render_html() -> str:
    return (
        PPT_DECK_HTML_TEMPLATE.replace("__STAGE_DATA__", _safe_json([asdict(stage) for stage in STAGES]))
