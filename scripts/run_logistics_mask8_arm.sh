...
ARM="${ARM:?ARM must be all or answer}"
...
RUN_NAME="${RUN_NAME:-logistics-mask8-${ARM}-20260905}"
...
RECORDS_PER_EXPOSURE="${RECORDS_PER_EXPOSURE:-8}"
EXPECTED_SEQUENCE_TOKENS="${EXPECTED_SEQUENCE_TOKENS:-$((EXPECTED_CONTENT_TOKENS + RECORDS_PER_EXPOSURE))}"
STEPS_PER_EXPOSURE="${STEPS_PER_EXPOSURE:-2}"
TOTAL_EXPOSURES="${TOTAL_EXPOSURES:-32}"
...
if [[ "${TRAIN_BATCH_SIZE}" -ne 4 || "${RECORDS_PER_EXPOSURE}" -ne 8 || \
      "${STEPS_PER_EXPOSURE}" -ne 2 || "${TOTAL_EXPOSURES}" -ne 32 || "${TOTAL_STEPS}" -ne 64 ]]; then
  printf 'reviewed8 requires 8 records, batch 4, 2 steps/exposure, and 32 exposures/64 steps\n' >&2
  exit 2
fi
available_bytes="$(df -B1 --output=avail "${PROJECT_ROOT}/runs" | tail -n 1 | tr -d ' ')"
if [[ "${available_bytes}" -lt 450000000000 ]]; then
  printf 'reviewed8 final optimizer and HF export require at least 450GB free\n' >&2
  exit 2
fi
...
ANSWER_ONLY=false
if [[ "${ARM}" == answer ]]; then ANSWER_ONLY=true; fi
...
purpose=reviewed8_same_text_${ARM}_supervision_positive_control
promotion_allowed=false
delivery_metric_valid=false
interpretation=training_item_fit_only_not_generalization
...
objective=same_text_${ARM}_targets_including_eos
...
save_contents=model_optimizer_extra
...
torchrun --standalone --nnodes=1 --nproc_per_node="${NPROC}" \
  ...
  data.num_workers=0 \
  "data.custom_cls.path=${PROJECT_ROOT}/scripts/qwen36_exam_mask_dataset.py" \
  data.custom_cls.name=Qwen36ExamMaskDataset \
  model=hf_model \
  "model.path=${MODEL_PATH}" \
...
python3 "${PROJECT_ROOT}/scripts/summarize_logistics_cpt_exposure_curve.py" \
  --run-dir "${OUTPUT_DIR}" \
  --experiment "logistics_reviewed8_mask_${ARM}" \
