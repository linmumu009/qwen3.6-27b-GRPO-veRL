"""One preregistered P2 arm using audited data and both existing baselines.
...
def main():
    ...
    for k in ('packet','data','out'):p.add_argument('--'+k,required=True,type=Path)
    a=p.parse_args();out=a.out.resolve();code=Path(__file__).resolve().parent
    assert out.parent==ROOT/'runs' and out.name.startswith('llin-transfer-p2-run-')
    ...
    try:
        with (ROOT/'runs/.logistics-exam-cpt.lock').open('a') as lock:
            ...
            assert budget['budgets']['train']['sequence_tokens']==78602 and budget['budgets']['train']['loss_tokens']==8770
            assert budget['budgets']['train']['sha256']=='f0f7e1a7460b3d4a151b9a198464ff98dcc284beee53c3b6f9e53141d361f54d'
            ...
            train_env=dict(env,LLIN_COORDINATOR_LOCKED='1',SFT_OUTPUT=str(out/'training'),SFT_DATA=str(data),SFT_ARM='P2',
                SFT_LR='5e-07',SFT_STEPS='105',SFT_MODEL_NAME='llin-step120-p2-hf',
                SFT_TRAIN_SHA=budget['budgets']['train']['sha256'],SFT_DEV_SHA=budget['budgets']['dev']['sha256'])
            run('training_export',['bash',str(code/'run_cpt_transfer_p2.sh')],train_env)
            ...
            probe('post',out/'training/llin-step120-p2-hf')
            ...
            passed=all(all(v['prospective_signal_gates'].values()) for v in (result,repeat_result))
            result=dict(versus_main=result,versus_repeat=repeat_result,retention_floor=52,
                next_action='broader_data_and_confirmation_review' if passed else 'stop_mixture_route_review',
