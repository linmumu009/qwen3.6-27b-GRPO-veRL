"""Reparse P2 raw outputs against the frozen packet; optionally rebuild tokens."""
import argparse
...
def verify(packet,run,tokenizer=None):
    ...
    for stage in ('baseline','baseline_repeat','post'):
        ...
        expected_model='/workspace/llin-verl-grpo/runs/llin-step120-opensource-20260825-02/hf_export_step120_opensource' if stage!='post' else '/workspace/llin-verl-grpo/runs/llin-transfer-p2-run-20260917-01/training/llin-step120-p2-hf'
    ...
    if 'post' in result:
        ...
        for stage,key in [('baseline','versus_main'),('baseline_repeat','versus_repeat')]:
            ...
            view['next_action']='broader_data_and_confirmation_review' if all(view['prospective_signal_gates'].values()) else 'stop_mixture_route_review'
        ...
        passed=all(all(stored[k]['prospective_signal_gates'].values()) for k in ('versus_main','versus_repeat'))
        assert stored['next_action']==('broader_data_and_confirmation_review' if passed else 'stop_mixture_route_review')
        ...
        assert training['steps']==105 and training['sequence_tokens']==budget['budgets']['train']['sequence_tokens']==78602
