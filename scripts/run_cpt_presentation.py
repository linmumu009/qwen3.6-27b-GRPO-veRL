"""P1-only 300-call diagnostic. Preserve completed batches; never retry reservations."""
import argparse
import hashlib
import json
import os
...
OUT=ROOT/'runs/llin-presentation-20260923-01'
...
def prepared(package):
    ...
    assert reg['max_calls']==300 and reg['training_allowed'] is False and packet['training_allowed'] is False
    assert sha(Path(__file__))==reg['runner_sha256'] and sha(package/'packet.private.json')==reg['packet_sha256']
    assert len(packet['requests'])==300 and len({r['id'] for r in packet['requests']})==300 and len(packet['reuse'])==100
...
def main():
    ...
    if a.worker:
        ...
        freeze=dict(packet_sha256=reg['packet_sha256'],prompt_token_sha256=digest(prompts),calls=300)
    ...
    with (ROOT/'runs/.logistics-exam-cpt.lock').open('a') as lock:
        ...
        save(OUT/'preflight.safe.json',dict(checked_at=time.time(),free_bytes=shutil.disk_usage(OUT).free,memory_mb=used,
            model_matches=True,code_matches=True,historical_token_prompts_match=100,max_prompt_tokens=max(len(p['prompt_token_ids']) for p in prompts)))
        ...
        try:
            ...
            assert read(OUT/'p1/completed.safe.json')['calls']==300
            save(status,dict(state='complete_pending_local_verification',calls=300,training_runs=0))
