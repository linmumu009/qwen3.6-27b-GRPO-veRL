...
export CPT_ARM="${CPT_ARM:?CPT_ARM required}"
export CPT_BATCH_PLAN="${CPT_BATCH_PLAN:?CPT_BATCH_PLAN required}"
...
RUN_NAME="${RUN_NAME:-logistics-cpt-paired-${CPT_ARM}-4x-20260910-01}"
...
CHECKPOINT_STEPS="${CHECKPOINT_STEPS:-116}"
LEARNING_RATE="${LEARNING_RATE:-5e-7}"
MIN_LEARNING_RATE="${MIN_LEARNING_RATE:-1e-7}"
# Two warmup steps over the fixed 116-step schedule.
WARMUP_RATIO="${WARMUP_RATIO:-0.017241379310344827}"
...
if [[ "${TRAIN_BATCH_SIZE}" -ne 4 || "${STEPS_PER_EXPOSURE}" -ne 29 || \
      "${TOTAL_EXPOSURES}" -ne 4 || "${TOTAL_STEPS}" -ne 116 || "${CHECKPOINT_STEPS}" -ne 116 ]]; then
  printf 'paired CPT requires batch=4, steps_per_exposure=29, exposures=4, total_steps=116, checkpoint_steps=116\n' >&2
  exit 2
fi
...
sampler=frozen_length_matched_both_arms
...
torchrun --standalone --nnodes=1 --nproc_per_node="${NPROC}" \
  ...
  "${SCRIPT_DIR}/run_cpt_paired_trainer.py" \
