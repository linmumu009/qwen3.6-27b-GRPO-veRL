...
RUN_NAME="${RUN_NAME}" \
OUTPUT_DIR="${RUN_DIR}" \
bash "${PROJECT_ROOT}/scripts/run_pi_formal_100step_12groups.sh" \
  > "${RUN_DIR}/driver.log" 2>&1
exit_code=$?
set -e

# Successful completion must have exactly the final step-100 checkpoint. Verify
# it before publishing exit code 0 so an incomplete PP checkpoint cannot pass.
...
if [[ "${exit_code}" == "0" ]]; then
  if [[ ! -f "${latest_iteration_file}" ]]; then
    ...
    if [[ "${latest_iteration}" != "100" ]]; then
      printf 'expected_global_step_100_got_%s\n' "${latest_iteration}" \
        > "${RUN_DIR}/CHECKPOINT_INVALID"
      exit_code=8
    else
