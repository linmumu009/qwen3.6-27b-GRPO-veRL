...
OUT=ROOT/'runs/cpt-stage2-storage-20260910/llin/llin-cpt-knowledge-complete-one-epoch-20260911'
CORPUS=ROOT/'runs/llin-knowledge-complete-20260911'
...
TRAIN_SHA='6ffa16684e617f657293918bf1d89ae8930ec24e0e08da346f175c4710c1f1c8'
...
def audit_training(metrics, lengths):
    assert len(lengths)==4912 and sum(lengths)==1872924
    assert sorted(metrics)==list(range(1,615)), 'Missing or extra training steps'
    ...
    return dict(steps=614,epochs=1,records=4912,batch_size=8,sequence_tokens=1872924,
        exact_divisible_epoch=True,mean_step_loss=sum(m['train/loss'] for m in metrics.values())/614)
...
def main():
    ...
    parser.add_argument('--after-training',action='store_true',help='Continue from completed step614 without repeating training')
        ...
        try:
            ...
            with (ROOT/'runs/.logistics-exam-cpt.lock').open('a') as resource:
                ...
                env=dict(pinned_environment(os.environ,code),TRAIN_FILE=str(CORPUS/'train.parquet'),EXPECTED_TRAIN_SHA=TRAIN_SHA,
                    EXPECTED_CONTENT_TOKENS='1868012',OUTPUT_DIR=str(OUT/'llin-training'),RUN_NAME='llin-cpt-knowledge-complete-one-epoch-20260911')
                ...
                if args.after_training:
                    checkpoint_gate(OUT/'llin-training/checkpoints/global_step_614')
                else:
                    run('training',['bash',str(code/'run_cpt_knowledge_complete_one_epoch.sh')],env)
                ...
                checkpoint=OUT/'llin-training/checkpoints/global_step_614'
                ...
                run('export_retry' if args.retry_export else 'export',[sys.executable,str(code/'export_megatron_dist_to_hf.py'),
                    '--actor-checkpoint',str(checkpoint),'--base-model',str(BASE),'--output-dir',str(OUT/'llin-step120-knowledge-complete-cpt-1epoch-20260911')],env)
                run('evaluation',[sys.executable,str(code/'run_logistics_cpt_curve_8x.py'),
                    '--evaluate',str(OUT/'llin-step120-knowledge-complete-cpt-1epoch-20260911'),'--output',str(OUT/'evaluation')],evaluation_env(env))
                ...
                save(status,dict(status='complete',training_epochs=1,training_steps=614,benchmark_requests=5016))
