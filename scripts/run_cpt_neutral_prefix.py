"""P1-only 915-call diagnostic. Preserve completed batches; never retry reservations."""
import argparse
import hashlib
import json
import os
...
OUT=ROOT/'runs/llin-neutral-prefix-20260923-01'
...
def prepared(package):
    ...
    assert reg['max_calls']==915 and reg['training_allowed'] is False and packet['training_allowed'] is False
    ...
    assert len(packet['requests'])==915 and len({r['id'] for r in packet['requests']})==915 and len(packet['reuse'])==305
...
def main():
    ...
    if a.worker:
        ...
        freeze=dict(packet_sha256=reg['packet_sha256'],prompt_token_sha256=digest(prompts),calls=915)
    ...
    with (ROOT/'runs/.logistics-exam-cpt.lock').open('a') as lock:
        ...
        save(OUT/'preflight.safe.json',dict(checked_at=time.time(),free_bytes=shutil.disk_usage(OUT).free,memory_mb=used,
            model_matches=True,code_matches=True,historical_token_prompts_match=305,max_prompt_tokens=max(len(p['prompt_token_ids']) for p in prompts)))
        ...
        try:
            ...
            assert read(OUT/'p1/completed.safe.json')['calls']==915
            save(status,dict(state='complete_pending_local_verification',calls=915,training_runs=0))
