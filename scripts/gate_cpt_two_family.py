...
from prepare_cpt_two_family_train import validate_budgets,DEV_SHA,DEV_MESSAGES_SHA
...
def gate(prepared,out):
    ...
    for arm in ('D','T'):
        d=prepared/arm
        assert sha(d/'dev.parquet')==DEV_SHA and sha(d/'dev.messages.private.jsonl')==DEV_MESSAGES_SHA
    ...
    result=dict(installed_loader_passed=True,baseline_model_and_runtime_match=True,prompts_match=True,
                budgets=budgets,same_sampler_order=True,sampler_sha256=hashlib.sha256(json.dumps(orders['D']).encode()).hexdigest(),
                budget_sha256=sha(prepared/'budget.safe.json'),training_allowed=False,installed_training_inputs_ready=True,trainer_seed=1,engine_seed=1,sampler_seed=1)
