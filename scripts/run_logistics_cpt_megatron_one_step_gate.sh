...
RUN_NAME="${RUN_NAME:-logistics-cpt-megatron-one-step-gate-20260903-01}"
...
for path in "${MODEL_PATH}/config.json" "${MODEL_PATH}/tokenizer.json" "${SOURCE_MODEL_DIST_CKPT}/.metadata" "${TRAIN_FILE}"; do
  if [[ ! -e "${path}" ]]; then
    printf 'required logistics CPT gate input missing: %s\n' "${path}" >&2
    exit 2
  fi
done
...
purpose=throwaway_all_token_causal_lm_forward_backward_gate
...
record_selection=single_longest_token_block
mtp_enable=false
...
torchrun --standalone --nnodes=1 --nproc_per_node="${NPROC}" \
  ...
  data.val_files=null \
  +data.text_key=text \
  +data.sort_by_token_count_desc=true \
