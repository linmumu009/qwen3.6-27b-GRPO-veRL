...
RUN_NAME="${RUN_NAME:-cpt-step29-one-update-restore-probe}"
...
TOTAL_EXPOSURES="${TOTAL_EXPOSURES:-8}"
TOTAL_STEPS="${TOTAL_STEPS:-232}"
CHECKPOINT_STEPS="${CHECKPOINT_STEPS:-29}"
LEARNING_RATE="${LEARNING_RATE:-5e-7}"
MIN_LEARNING_RATE="${MIN_LEARNING_RATE:-1e-7}"
# Two warmup steps over the fixed 232-step schedule.
WARMUP_RATIO="${WARMUP_RATIO:-0.008620689655172414}"
...
if [[ "${TRAIN_BATCH_SIZE}" -ne 4 || "${STEPS_PER_EXPOSURE}" -ne 29 || \
      "${TOTAL_EXPOSURES}" -ne 8 || "${TOTAL_STEPS}" -ne 232 || "${CHECKPOINT_STEPS}" -ne 29 ]]; then
  printf '8x curve requires batch=4, steps_per_exposure=29, exposures=8, total_steps=232, checkpoint_steps=29\n' >&2
  exit 2
fi
...
[[ "${actual_hash}" == 975dc8719da2ff6ba0f2eb5db4faa03cb72f2f1ee36c14ed5e35460d4ca4c1c2 ]] || exit 2
...
purpose=one_update_full_checkpoint_restore_probe
...
checkpoint_initialization=model_optimizer_scheduler_rng_dataloader_step29
optimizer_state=restore_step29_then_one_update_discard
optimizer_reset_at_step_58=false
optimizer_placement=device_side_matching_step120
dataloader_state=restore_step29_normalize_finished_epoch_in_memory
...
expected_replay_sequence_tokens_with_eos=9768
train_batch_size=${TRAIN_BATCH_SIZE}
total_training_steps=${TOTAL_STEPS}
evaluation_checkpoints=none
evaluation_exposures=none
learning_rate=${LEARNING_RATE}
minimum_learning_rate=${MIN_LEARNING_RATE}
warmup_ratio=${WARMUP_RATIO}
expected_effective_warmup_steps=2
lr_decay_style=cosine_across_all_232_steps
...
torchrun --standalone --nnodes=1 --nproc_per_node="${NPROC}" \
  ...
  "${SCRIPT_DIR}/run_cpt_resume_probe.py" \
  ...
  "trainer.resume_from_path=${PROJECT_ROOT}/runs/cpt-controlled-storage-20260910/llin/cpt-controlled-20260910/curve8x/checkpoints/global_step_29" \
