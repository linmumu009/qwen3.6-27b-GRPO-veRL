...
DATA_DIR="${DATA_DIR:-${PROJECT_ROOT}/data/native_disjoint_first_error_candidates_20260813}"
DATA_FILE="${DATA_FILE:-${DATA_DIR}/native_disjoint_first_error_candidates.parquet}"
DATA_CONTRACT="${DATA_CONTRACT:-${DATA_DIR}/native_first_error_candidate_contract.json}"
RUN_NAME="${RUN_NAME:?RUN_NAME is required}"
OUTPUT_DIR="${OUTPUT_DIR:-${PROJECT_ROOT}/runs/${RUN_NAME}}"
MODEL_LABEL="${MODEL_LABEL:-step120_on_native_first_error_candidates}"
EXPECTED_PAIRS="${EXPECTED_PAIRS:-7}"
...
if (( TP * PP * CP != NPROC )); then
  printf 'invalid topology for native candidate margin screen\n' >&2
  exit 2
fi
for path in "${MODEL_PATH}/config.json" "${MODEL_DIST_CKPT}/.metadata" "${DATA_FILE}" "${DATA_CONTRACT}"; do
  if [[ ! -e "${path}" ]]; then
    printf 'native candidate margin input missing: %s\n' "${path}" >&2
    exit 2
  fi
done
...
read -r PAIRS ROWS CANDIDATE_ONLY MAY_TRAIN TRAINING_ALLOWED < <(python3 - "${DATA_CONTRACT}" <<'PY'
import json
import sys
...
print(
    int(value["pairs"]),
    int(value["rows"]),
    str(value["candidate_only"]).lower(),
    str(value["may_be_used_as_training_data"]).lower(),
    str(value["training_allowed"]).lower(),
)
PY
)
if (( PAIRS != EXPECTED_PAIRS || ROWS != 2 * PAIRS )) || \
   [[ "${CANDIDATE_ONLY}" != true || "${MAY_TRAIN}" != false || "${TRAINING_ALLOWED}" != false ]]; then
  printf 'native candidate frozen contract failed: pairs=%s rows=%s candidate=%s may_train=%s training=%s\n' \
    "${PAIRS}" "${ROWS}" "${CANDIDATE_ONLY}" "${MAY_TRAIN}" "${TRAINING_ALLOWED}" >&2
  exit 2
fi
...
purpose=step120_forward_only_screen_on_native_real_first_error_candidates
...
torchrun --standalone --nnodes=1 --nproc_per_node="${NPROC}" \
  ...
  trainer.project_name=llin-native-disjoint-first-error-screen \
