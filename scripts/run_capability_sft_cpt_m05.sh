...
OUT=/opt/llin-capability-sft-cpt-20260909-01
...
umask 077
[[ ! -e "$OUT" && -f "$SOURCE/.metadata" && -f "$BASE/config.json" ]] || exit 2
...
printf 'preparing\n' > "$OUT/status.txt"
python3 "$ROOT/scripts/prepare_capability_sft_training.py" --root "$ROOT" --model "$BASE" --out "$OUT/data"
printf 'training\n' > "$OUT/status.txt"
torchrun --standalone --nnodes=1 --nproc_per_node=16 --log-dir="$OUT/torchrun_logs" --redirects=3 --tee=0 \
 ...
 trainer.experiment_name=capability-low-exposure 'trainer.logger=["console"]' \
 trainer.total_epochs=1 trainer.total_training_steps=44 trainer.save_freq=44 trainer.test_freq=44 \
 trainer.resume_mode=disable trainer.max_ckpt_to_keep=1 trainer.nnodes=1 trainer.n_gpus_per_node=16
printf 'exporting\n' > "$OUT/status.txt"
python3 "$ROOT/scripts/export_megatron_dist_to_hf.py" --actor-checkpoint "$OUT/checkpoints/global_step_44" \
 --base-model "$BASE" --output-dir "$OUT/hf_export" > "$OUT/export.log" 2>&1
printf 'evaluating_domain\n' > "$OUT/status.txt"
MODEL_PATH="$OUT/hf_export" MODEL_LABEL=capability_sft_cpt_44 CASES_PATH="$ROOT/runs/logistics-cpt-diagnostics-20260904/private/public_eval/frozen_cases_source.jsonl" \
...
python3 "$ROOT/scripts/summarize_mcq_repeats.py" --repeat "$OUT/evaluation.jsonl" --repeat "$OUT/evaluation.repeat2.jsonl" \
 ...
 --model-label capability_sft_cpt_44 --private-output "$OUT/majority.private.jsonl" --safe-output "$OUT/majority.safe.json"
printf 'domain_complete_other_regressions_pending\n' > "$OUT/status.txt"
