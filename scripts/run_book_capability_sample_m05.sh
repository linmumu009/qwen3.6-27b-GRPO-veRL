...
OUT="$ROOT/runs/book-capability-probe-20260909-01"
...
python3 "$ROOT/scripts/probe_book_capability_sample.py" prepare --root "$ROOT" --out "$OUT"
for model in step120 cpt; do
 python3 "$ROOT/scripts/probe_book_capability_sample.py" run --root "$ROOT" --out "$OUT" --model "$model"
done
python3 "$ROOT/scripts/probe_book_capability_sample.py" grade --root "$ROOT" --out "$OUT" --api-config "$ROOT/private/chat_api_config.user.json"
