...
python3 "$ROOT/scripts/run_logistics_strategy_diagnostic.py" prepare --root "$ROOT" --out "$OUT"
for model in step120 cpt; do
 python3 "$ROOT/scripts/run_logistics_strategy_diagnostic.py" run --root "$ROOT" --out "$OUT" --model "$model"
done
python3 "$ROOT/scripts/run_logistics_strategy_diagnostic.py" summarize --root "$ROOT" --out "$OUT"
