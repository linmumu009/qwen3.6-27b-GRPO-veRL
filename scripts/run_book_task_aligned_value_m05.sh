...
python3 "$ROOT/scripts/probe_book_task_aligned_value.py" prepare --root "$ROOT" --out "$OUT"
for model in step120 cpt; do
 python3 "$ROOT/scripts/probe_book_task_aligned_value.py" run --root "$ROOT" --out "$OUT" --model "$model"
done
