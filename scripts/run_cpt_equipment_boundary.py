"""Frozen close-equipment boundary probe; evaluation only."""
import argparse
import hashlib
import json
import os
from pathlib import Path
from cpt_equipment_boundary import sha,save,digest,specs,prompt,summarize,SEED
...
def run(packet_dir,model_manifest,out,label):
    ...
    try:
        with (ROOT/'runs/.logistics-exam-cpt.lock').open('a') as lock:
            ...
            assert prompts[:128]==prompts[128:]
            save(out/'registration.safe.json',dict(model_label=label,model=str(model),model_manifest_sha256=sha(model_manifest),
                packet_sha256=sha(path),runner_sha256=sha(Path(__file__)),protocol_sha256=sha(Path(__file__).with_name('cpt_equipment_boundary.py')),
                seed=SEED,temperature=0,thinking=False,tp=8,max_model_len=8192,max_tokens=96,max_num_seqs=16,chunk=8,
                calls=256,repeats=2,identical_batches_repeated=True,training_allowed=False))
            ...
            with (out/'predictions.private.jsonl').open('x',encoding='utf-8') as f:
                for first in range(0,len(plan),8):
                    ...
                    f.flush();status('running',completed=len(raw),total=256)
