"""Reparse P3 raw outputs against the frozen packet; optionally rebuild tokens."""
import argparse
...
def verify(packet,run,arithmetic,tokenizer=None):
    ...
    result={};predictions={}
    for stage in ('baseline','baseline_repeat','post'):
        base=run/stage
        assert base.exists()
        ...
        expected_model='/workspace/llin-verl-grpo/runs/llin-step120-opensource-20260825-02/hf_export_step120_opensource' if stage!='post' else '/workspace/llin-verl-grpo/runs/llin-transfer-p3-run-20260917-01/training/llin-step120-p3-hf'
    ...
    if 'post' in result:
        ...
        stored=json.loads((run/'comparison.safe.json').read_text())
        for stage,key in [('baseline','versus_main'),('baseline_repeat','versus_repeat')]:
            view=comparison(predictions[stage],predictions['post'])
            ...
            assert view==stored[key]
            assert json.loads((run/stage/'prompts.safe.json').read_text())==json.loads((run/'post/prompts.safe.json').read_text())
        from run_cpt_transfer_p3 import check_arithmetic
        arith_items=load_items(arithmetic/'cases.private.jsonl')
        from audit_cpt_p3_arithmetic import audit
        assert audit(arithmetic)['passed']
        scores={}
        for stage in ('arithmetic_base','arithmetic_p2','arithmetic_post'):
            directory=run/stage;scores[stage]=check_arithmetic(arithmetic,directory)
            summary=json.loads((directory/'summary.safe.json').read_text())
            for k,v in dict(temperature=0,seed=1024,max_tokens=96,max_model_len=8192,tp=8,max_num_seqs=32,thinking=False,repeats=1,items=60).items():assert summary[k]==v
            expected={'arithmetic_base':'/workspace/llin-verl-grpo/runs/llin-step120-opensource-20260825-02/hf_export_step120_opensource',
                'arithmetic_p2':'/workspace/llin-verl-grpo/runs/llin-transfer-p2-run-20260917-01/training/llin-step120-p2-hf',
                'arithmetic_post':'/workspace/llin-verl-grpo/runs/llin-transfer-p3-run-20260917-01/training/llin-step120-p3-hf'}
            assert summary['model']==expected[stage]
            prompts=json.loads((directory/'prompts.safe.json').read_text())
            assert prompts==json.loads((run/'arithmetic_base/prompts.safe.json').read_text())
            if tokenizer:
                for item,prompt in zip(arith_items,prompts):
                    text=tok.apply_chat_template(build_messages(item),tokenize=False,add_generation_prompt=True,enable_thinking=False)
                    ids=tok.encode(text,add_special_tokens=False)
                    assert prompt==dict(source_id=item.source_id,text_sha256=hashlib.sha256(text.encode()).hexdigest(),token_sha256=hashlib.sha256(json.dumps(ids,separators=(',',':')).encode()).hexdigest(),tokens=len(ids))
        floor=max(scores[k]['correct'] for k in ('arithmetic_base','arithmetic_p2'))
        arithmetic_passed=scores['arithmetic_post']['correct']>=floor
        assert scores==stored['arithmetic'] and stored['arithmetic_floor']==floor and stored['arithmetic_passed']==arithmetic_passed
        passed=arithmetic_passed and all(all(stored[k]['prospective_signal_gates'].values()) for k in ('versus_main','versus_repeat'))
        assert stored['next_action']==('broader_data_and_confirmation_review' if passed else 'stop_content_replacement_route_review')
        assert stored['retention_floor']==52 and not stored['formal_benchmark_evaluated'] and not stored['sealed_evaluated'] and not stored['promotion']
        result['comparison']=stored
        budget=json.loads((run/'data_audit.safe.json').read_text())
        training=json.loads((run/'training_audit.safe.json').read_text())
        assert training['steps']==105 and training['sequence_tokens']==budget['budgets']['train']['sequence_tokens']==74573
    ...
    result.update(packet_sha256=sha(packet/'tasks.private.jsonl'),cases_sha256=sha(packet/'cases.private.jsonl'),
        raw_outputs_verified=180*len(predictions)+180,token_prompts_independently_rebuilt=180*len(predictions)+180 if tokenizer else 0,
...
if __name__=='__main__':
    ...
    for k in ('packet','run','arithmetic','out'):p.add_argument('--'+k,type=Path,required=True)
    p.add_argument('--tokenizer',type=Path);a=p.parse_args();result=verify(a.packet,a.run,a.arithmetic,a.tokenizer)
