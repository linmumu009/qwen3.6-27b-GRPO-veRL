...
export TRAIN_FILE="${TRAIN_FILE:?TRAIN_FILE required}"
EXPECTED_CONTENT_TOKENS="${EXPECTED_CONTENT_TOKENS:?EXPECTED_CONTENT_TOKENS required}"
EXPECTED_TRAIN_SHA="${EXPECTED_TRAIN_SHA:?EXPECTED_TRAIN_SHA required}"
RUN_NAME="${RUN_NAME:-llin-cpt-knowledge-complete-one-epoch-20260911}"
...
STEPS_PER_EXPOSURE="${STEPS_PER_EXPOSURE:-614}"
TOTAL_EXPOSURES="${TOTAL_EXPOSURES:-1}"
TOTAL_STEPS="${TOTAL_STEPS:-2}"
CHECKPOINT_STEPS="${CHECKPOINT_STEPS:-2}"
LEARNING_RATE="${LEARNING_RATE:-1e-5}"
MIN_LEARNING_RATE="${MIN_LEARNING_RATE:-1e-6}"
# Thirteen warmup steps over the fixed 614-step schedule.
WARMUP_RATIO="${WARMUP_RATIO:-0.021172638436482084}"
...
[[ "$TRAIN_BATCH_SIZE" == 8 && "$TOTAL_EXPOSURES" == 1 ]] || exit 2
[[ "$TOTAL_STEPS" == 2 || "$TOTAL_STEPS" == 614 ]] || exit 2
[[ "$CHECKPOINT_STEPS" == "$TOTAL_STEPS" ]] || exit 2
export LLIN_LORA_SOURCE="$SOURCE_MODEL_DIST_CKPT"
export LLIN_LORA_AUDIT="$OUTPUT_DIR/weight_audit"
...
export PYTHONPATH="${CODE_ROOT}:${PROJECT_ROOT}:${MEGATRON_BRIDGE_ROOT}:${PROJECT_ROOT}/runtime:/verl:${PYTHONPATH:-}"
umask 077

actual_hash="$(sha256sum "${TRAIN_FILE}" | cut -d ' ' -f 1)"
[[ "${actual_hash}" == "${EXPECTED_TRAIN_SHA}" ]] || exit 2
mkdir -p "${OUTPUT_DIR}/torchrun_logs"
python3 "${SCRIPT_DIR}/check_logistics_cpt_dataset.py" \
  --train-file "${TRAIN_FILE}" \
  --model-path "${MODEL_PATH}" \
  --max-length "${MAX_LENGTH}" \
  --expected-records 4912 \
...
purpose=lora_cpt_compatibility_or_registered_epoch
source_checkpoint=${SOURCE_CHECKPOINT}
checkpoint_initialization=model_only_dist_ckpt
optimizer_state=fresh
train_file_sha256=${EXPECTED_TRAIN_SHA}
records=4912
planned_exposures=1
content_tokens=1868012
sequence_tokens=1872924
batch_size=8
total_training_steps=${TOTAL_STEPS}
learning_rate=${LEARNING_RATE}
minimum_learning_rate=${MIN_LEARNING_RATE}
expected_warmup_steps=13
adam_beta1=0.9
adam_beta2=0.999
raw_text_causal_lm=true
chat_template_applied=false
validation_documents_not_trained=0
save_contents=model_optimizer_extra_and_dataloader
promotion_allowed=false
EOF

torchrun --standalone --nnodes=1 --nproc_per_node="${NPROC}" \
  --log-dir="${OUTPUT_DIR}/torchrun_logs" --redirects=3 --tee=0 \
  "${SCRIPT_DIR}/run_lora_cpt_trainer.py" \
  ...
  data.num_workers=0 \
  "data.custom_cls.path=${SCRIPT_DIR}/qwen36_causal_lm_dataset.py" \
  ...
  model.lora_rank=0 \
  ++model.lora.type=lora ++model.lora.rank=64 ++model.lora.alpha=128 ++model.lora.dropout=0.0 \
  '++model.lora.target_modules=[language_model.*.linear_qkv,language_model.*.linear_proj,language_model.*.linear_fc1,language_model.*.linear_fc2]' \
  ...
  +optim.override_optimizer_config.adam_beta2=0.999 \
  optim.clip_grad=1.0 \
