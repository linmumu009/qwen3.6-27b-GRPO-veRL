...
ROLLING_WINDOW_MAX_MULTIPLIER="${ROLLING_WINDOW_MAX_MULTIPLIER:-1.0}"
...
MONITOR_FIRST_CARD="${MONITOR_FIRST_CARD:-0}"
MONITOR_NUM_CARDS="${MONITOR_NUM_CARDS:-8}"
MAX_ATTEMPTS="${MAX_ATTEMPTS:-3}"
RETRY_DELAY_SECONDS="${RETRY_DELAY_SECONDS:-60}"

export PROJECT_ROOT RUN_NAME OUTPUT_DIR MODEL MODEL_LABEL POLICY_STEP REASONING_EFFORT DATASET EXPECTED_TASKS
export SAMPLES_PER_TASK TASK_BATCH_SIZE MAX_NUM_SEQS AGENT_WORKERS
export TENSOR_PARALLEL_SIZE DATA_PARALLEL_SIZE ROLLOUT_NPUS
...
nohup bash -c '
  ...
  while (( attempt <= MAX_ATTEMPTS )); do
    ...
    python3 "${PROJECT_ROOT}/scripts/run_runtime_parity_verl_standalone.py" \
      ...
      --rolling-window-max-multiplier "${ROLLING_WINDOW_MAX_MULTIPLIER}" \
...
if [[ "${MONITOR_NPU}" == "1" ]]; then
  nohup python3 "${PROJECT_ROOT}/scripts/monitor_npu_utilization.py" \
    ...
    --first-card "${MONITOR_FIRST_CARD}" \
    --num-cards "${MONITOR_NUM_CARDS}" \
