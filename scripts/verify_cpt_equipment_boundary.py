...
from cpt_equipment_boundary import sha,read,save,digest,specs,prompt,summarize,SEED
from run_cpt_equipment_boundary import TOKENIZER_SHA,MODELS
...
def verify(packet_dir,package,run,tokenizer):
    ...
    for name in ('cpt_equipment_boundary.py','run_cpt_equipment_boundary.py'):assert sha(Path(__file__).with_name(name))==sha(package/'scripts'/name)
    ...
    for label in MODELS:
        ...
        checks=dict(model_label=label,model=MODELS[label].as_posix(),model_manifest_sha256=sha(package/(label+'.model.safe.json')),
            packet_sha256=sha(packet_file),runner_sha256=sha(package/'scripts/run_cpt_equipment_boundary.py'),protocol_sha256=sha(package/'scripts/cpt_equipment_boundary.py'),
            seed=SEED,temperature=0,thinking=False,tp=8,max_model_len=8192,max_tokens=96,max_num_seqs=16,chunk=8,calls=256,repeats=2,identical_batches_repeated=True,training_allowed=False)
        assert reg==checks
        ...
        for i,(s,r,d) in enumerate(zip(plan,raw,summary['per_call'])):
            assert (r['prompt_token_sha256'],r['prompt_tokens'])==tokens[i%128]
    ...
    return dict(status='completed_verified_local',new_calls=768,packet_sha256=sha(packet_file),models=summaries,
