...
TOTAL_STEPS="${TOTAL_STEPS:-614}"
CHECKPOINT_STEPS="${CHECKPOINT_STEPS:-614}"
...
if [[ "${TRAIN_BATCH_SIZE}" -ne 8 || "${TOTAL_STEPS}" -ne 614 || "${TOTAL_EXPOSURES}" -ne 1 || "${CHECKPOINT_STEPS}" -ne 614 ]]; then
  printf 'Logistics corpus requires batch8, exactly614 steps, one epoch, save614\n' >&2
  exit 2
fi
...
purpose=knowledge_complete_all_13_sources_plus_reviewed_cpt_one_epoch
