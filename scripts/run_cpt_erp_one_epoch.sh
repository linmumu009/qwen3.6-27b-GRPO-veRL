...
RUN_NAME="${RUN_NAME:-llin-cpt-erp-one-epoch-20260910}"
...
STEPS_PER_EXPOSURE="${STEPS_PER_EXPOSURE:-47}"
TOTAL_EXPOSURES="${TOTAL_EXPOSURES:-1}"
TOTAL_STEPS="${TOTAL_STEPS:-47}"
CHECKPOINT_STEPS="${CHECKPOINT_STEPS:-47}"
LEARNING_RATE="${LEARNING_RATE:-5e-7}"
MIN_LEARNING_RATE="${MIN_LEARNING_RATE:-1e-7}"
# Two warmup steps over the fixed 47-step schedule.
WARMUP_RATIO="${WARMUP_RATIO:-0.0425531914893617}"
...
if [[ "${TRAIN_BATCH_SIZE}" -ne 1 || "${TOTAL_STEPS}" -ne 47 || "${TOTAL_EXPOSURES}" -ne 1 || "${CHECKPOINT_STEPS}" -ne 47 ]]; then
  printf 'ERP seed corpus requires batch1, exactly47 steps, one epoch, save47\n' >&2
  exit 2
fi
...
purpose=erp_seed_cpt_one_epoch
...
records=47
planned_exposures=1
content_tokens=32468
sequence_tokens=32515
batch_size=1
total_training_steps=47
...
validation_documents_not_trained=3
