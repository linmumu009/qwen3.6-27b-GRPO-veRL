...
[[ "${SFT_ARM:?}" =~ ^(K|R)$ && "${SFT_LR:?}" == 5e-07 ]] || exit 4
[[ "${SFT_STEPS:?}" -ge 106 && "$SFT_STEPS" -le 111 ]] || exit 4
STEPS="$SFT_STEPS"
...
torchrun --standalone --nnodes=1 --nproc_per_node=16 --log-dir="$OUT/torchrun_logs" --redirects=3 --tee=0 \
 "$CODE/train_cpt_mechanism_sft.py" \
 ...
 engine=megatron engine.seed=1 engine.tensor_model_parallel_size=4 engine.pipeline_model_parallel_size=2 engine.context_parallel_size=2 \
 ...
 trainer.seed=1 trainer.resume_mode=disable trainer.max_ckpt_to_keep=1 trainer.nnodes=1 trainer.n_gpus_per_node=16
printf 'exporting\n' > "$OUT/status.txt"
python3 "$CODE/export_megatron_dist_to_hf.py" --actor-checkpoint "$OUT/checkpoints/global_step_$STEPS" \
 --base-model "$BASE" --output-dir "$OUT/${SFT_MODEL_NAME:-llin-step120-p4-hf}" > "$OUT/export.log" 2>&1
