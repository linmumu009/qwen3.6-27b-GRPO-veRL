...
RUN_NAME="${RUN_NAME:-llin-qwen38-approved43-tiered-v1-formal-$(date +%Y%m%d-%H%M%S)}"
...
if len(train) != 172 or set(train_ids) != approved_ids:
    raise SystemExit("formal schedule must contain exactly approved43 x four exposures")
if set(collections.Counter(train_ids).values()) != {4}:
    raise SystemExit("every approved task must occur exactly four times")
...
cat > "${OUTPUT_DIR}/audit/formal_contract.safe.txt" <<EOF
contract=qwen38-approved43-tiered-query-cost-formal-v1
...
nominal_groups=${TOTAL_NOMINAL_GROUPS}
nominal_batches=${TOTAL_NOMINAL_BATCHES}
...
checkpoint_policy=final_only_one_persistent_model
...
TOTAL_TRAINING_STEPS="${TOTAL_NOMINAL_BATCHES}" \
TOTAL_ROLLOUT_GROUPS="${TOTAL_NOMINAL_GROUPS}" \
...
SAVE_FREQ="${TOTAL_NOMINAL_BATCHES}" \
bash "${PROJECT_ROOT}/scripts/run_pi_grpo_fully_async_tp4_pp2_cp2.sh" \
  ...
  trainer.test_freq="${TOTAL_NOMINAL_BATCHES}" \
  ...
  trainer.save_freq="${TOTAL_NOMINAL_BATCHES}" \
