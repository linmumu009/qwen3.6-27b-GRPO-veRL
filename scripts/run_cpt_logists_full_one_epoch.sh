...
RUN_NAME="${RUN_NAME:-llin-cpt-logists-full-one-epoch-20260911}"
...
STEPS_PER_EXPOSURE="${STEPS_PER_EXPOSURE:-523}"
TOTAL_EXPOSURES="${TOTAL_EXPOSURES:-1}"
TOTAL_STEPS="${TOTAL_STEPS:-523}"
CHECKPOINT_STEPS="${CHECKPOINT_STEPS:-523}"
LEARNING_RATE="${LEARNING_RATE:-5e-7}"
MIN_LEARNING_RATE="${MIN_LEARNING_RATE:-1e-7}"
# Eleven warmup steps over the fixed 523-step schedule.
WARMUP_RATIO="${WARMUP_RATIO:-0.021032504780114723}"
...
if [[ "${TRAIN_BATCH_SIZE}" -ne 9 || "${TOTAL_STEPS}" -ne 523 || "${TOTAL_EXPOSURES}" -ne 1 || "${CHECKPOINT_STEPS}" -ne 523 ]]; then
  printf 'Logistics corpus requires batch9, exactly523 steps, one epoch, save523\n' >&2
  exit 2
fi
...
purpose=logists_all_13_sources_cpt_one_epoch
...
records=4707
planned_exposures=1
content_tokens=1844881
sequence_tokens=1849588
batch_size=9
total_training_steps=523
learning_rate=${LEARNING_RATE}
minimum_learning_rate=${MIN_LEARNING_RATE}
expected_warmup_steps=11
adam_beta1=0.9
adam_beta2=0.999
