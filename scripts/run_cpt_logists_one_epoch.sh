...
RUN_NAME="${RUN_NAME:-llin-cpt-logists-one-epoch-20260910}"
...
STEPS_PER_EXPOSURE="${STEPS_PER_EXPOSURE:-377}"
TOTAL_EXPOSURES="${TOTAL_EXPOSURES:-1}"
TOTAL_STEPS="${TOTAL_STEPS:-377}"
CHECKPOINT_STEPS="${CHECKPOINT_STEPS:-377}"
LEARNING_RATE="${LEARNING_RATE:-5e-7}"
MIN_LEARNING_RATE="${MIN_LEARNING_RATE:-1e-7}"
# Eight warmup steps over the fixed 377-step schedule.
WARMUP_RATIO="${WARMUP_RATIO:-0.021220159151193633}"
...
if [[ "${TRAIN_BATCH_SIZE}" -ne 8 || "${TOTAL_STEPS}" -ne 377 || "${TOTAL_EXPOSURES}" -ne 1 || "${CHECKPOINT_STEPS}" -ne 377 ]]; then
  printf 'Logistics corpus requires batch8, exactly377 steps, one epoch, save377\n' >&2
  exit 2
fi
...
records=3016
planned_exposures=1
content_tokens=1180181
sequence_tokens=1183197
batch_size=8
total_training_steps=377
learning_rate=${LEARNING_RATE}
minimum_learning_rate=${MIN_LEARNING_RATE}
expected_warmup_steps=8
adam_beta1=0.9
adam_beta2=0.999
raw_text_causal_lm=true
chat_template_applied=false
validation_documents_not_trained=161
