...
在交付包根目录运行：
```bash
bash Train/agentic-GRPO/run.sh --check
...
数据：datasets/agentic-GRPO/train.jsonl和val.jsonl；入口自动转换Parquet。
输出：outputs/agentic-GRPO/时间编号/，包括driver.log、exit_code、preflight.json、resolved_config.yaml、status.json和checkpoints。
